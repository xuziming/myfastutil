 ...
 * @see HashCommon
 */
public class FloatOpenHashBigSet extends AbstractFloatSet implements java.io.Serializable, Cloneable, Hash, Size64 {
	...
	public FloatOpenHashBigSet(final long expected, final float f) {
		...
		maxFill = maxFill(n, f);
		key = FloatBigArrays.newBigArray(n);
		initMasks();
	}
	...
	public boolean add(final float k) {
		...
		if ((Float.floatToIntBits(k) == 0)) {
			...
			final long h = it.unimi.dsi.fastutil.HashCommon.mix((long) (it.unimi.dsi.fastutil.HashCommon.float2int(k)));
			// The starting point.
			if (!(Float
					.floatToIntBits(curr = key[base = (int) ((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int) (h
							& segmentMask)]) == 0)) {
				if ((Float.floatToIntBits(curr) == Float.floatToIntBits(k)))
					return false;
				while (!(Float
						.floatToIntBits(curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0))
								& baseMask][displ]) == 0))
					if ((Float.floatToIntBits(curr) == Float.floatToIntBits(k)))
						return false;
			}
		...
		for (;;) {
			...
			for (;;) {
				...
				slot = it.unimi.dsi.fastutil.HashCommon
						.mix((long) (it.unimi.dsi.fastutil.HashCommon.float2int(BigArrays.get(key, pos)))) & mask;
	...
	public boolean remove(final float k) {
		...
		int displ, base;
		// The starting point.
		if ((Float.floatToIntBits(curr = key[base = (int) ((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int) (h
				& segmentMask)]) == 0))
			return false;
		if ((Float.floatToIntBits(curr) == Float.floatToIntBits(k)))
			return removeEntry(base, displ);
		while (true) {
			if ((Float.floatToIntBits(curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0))
					& baseMask][displ]) == 0))
				return false;
			if ((Float.floatToIntBits(curr) == Float.floatToIntBits(k)))
				return removeEntry(base, displ);
	...
	private class SetIterator implements FloatIterator {
		...
		public float nextFloat() {
			...
			for (;;) {
				if (displ == 0 && base <= 0) {
					...
					last = Long.MIN_VALUE;
					return wrapped.getFloat(-(--base) - 1);
				}
				if (displ-- == 0)
		...
		private final void shiftKeys(long pos) {
			...
			for (;;) {
				...
				for (;;) {
					...
					slot = it.unimi.dsi.fastutil.HashCommon
							.mix((long) (it.unimi.dsi.fastutil.HashCommon.float2int(curr))) & mask;
					if (last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos)
						break;
					pos = (pos + 1) & mask;
				}
		...
		public void remove() {
			...
			if (last == n)
				...
				FloatOpenHashBigSet.this.remove(wrapped.getFloat(-base - 1));
	...
	protected void rehash(final long newN) {
		...
		for (long i = realSize(); i-- != 0;) {
			...
			k = key[base][displ];
			h = it.unimi.dsi.fastutil.HashCommon.mix((long) (it.unimi.dsi.fastutil.HashCommon.float2int(k)));
			// The starting point.
			if (!(Float.floatToIntBits(
					newKey[b = (int) ((h & mask) >>> BigArrays.SEGMENT_SHIFT)][d = (int) (h & newSegmentMask)]) == 0))
				while (!(Float.floatToIntBits(
	...
	public int hashCode() {
		...
		for (long j = realSize(); j-- != 0;) {
			while ((Float.floatToIntBits(key[base][displ]) == 0))
				base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0));
			h += it.unimi.dsi.fastutil.HashCommon.float2int(key[base][displ]);
			base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0));
		}
		return h;
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		maxFill = maxFill(n, f);
		final float[][] key = this.key = FloatBigArrays.newBigArray(n);
		initMasks();
		long h;
		float k;
		int base, displ;
		for (long i = size; i-- != 0;) {
			...
			if ((Float.floatToIntBits(k) == 0))
				...
				h = it.unimi.dsi.fastutil.HashCommon.mix((long) (it.unimi.dsi.fastutil.HashCommon.float2int(k)));
				if (!(Float.floatToIntBits(key[base = (int) ((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int) (h
						& segmentMask)]) == 0))
					while (!(Float.floatToIntBits(key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0))
							& baseMask][displ]) == 0))
						;
				key[base][displ] = k;
			}
