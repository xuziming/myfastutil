...
public final class FloatBigArrays {
	...
	public static void quickSort(final float[][] x, final long from, final long to, final FloatComparator comp) {
		...
		final float v = BigArrays.get(x, m);
		...
		while (offsetPos > 0) {
			...
			for (long i = length; i-- != 0;)
				BigArrays.set(digit, i,
						(byte) (((fixFloat(BigArrays.get(a, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
			for (long i = length; i-- != 0;)
	...
	private static void selectionSort(final float[][] a, final float[][] b, final long from, final long to) {
		for (long i = from; i < to - 1; i++) {
			...
			for (long j = i + 1; j < to; j++)
				if ((Float.compare((BigArrays.get(a, j)), (BigArrays.get(a, m))) < 0)
						|| (Float.compare((BigArrays.get(a, j)), (BigArrays.get(a, m))) == 0)
								&& (Float.compare((BigArrays.get(b, j)), (BigArrays.get(b, m))) < 0))
					m = j;
			if (m != i) {
	...
	public static void radixSort(final float[][] a, final float[][] b, final long from, final long to) {
		...
		while (offsetPos > 0) {
			...
			for (long i = length; i-- != 0;)
				BigArrays.set(digit, i,
						(byte) (((fixFloat(BigArrays.get(k, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
			for (long i = length; i-- != 0;)
				count[BigArrays.get(digit, i) & 0xFF]++;
			// Compute cumulative distribution and push non-singleton keys on stack.
			int lastUsed = -1;
	...
	private static void insertionSortIndirect(final long[][] perm, final float[][] a, final float[][] b,
			final long from, final long to) {
		for (long i = from; ++i < to;) {
			long t = BigArrays.get(perm, i);
			long j = i;
			for (long u = BigArrays.get(perm, j - 1); (Float.compare((BigArrays.get(a, t)), (BigArrays.get(a, u))) < 0)
					|| (Float.compare((BigArrays.get(a, t)), (BigArrays.get(a, u))) == 0)
							&& (Float.compare((BigArrays.get(b, t)), (BigArrays.get(b, u))) < 0); u = BigArrays
									.get(perm, --j - 1)) {
	 ...
	 */
	public static void radixSortIndirect(final long[][] perm, final float[][] a, final float[][] b,
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final long[][] perm, final float[][] a, final float[][] b, final long from,
		...
		while (stackPos > 0) {
			...
			for (long i = first + length; i-- != first;)
				count[(fixFloat(BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)]++;
			...
			if (stable) {
				for (long i = first + length; i-- != first;)
					BigArrays.set(support,
							--pos[(fixFloat(BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK
									^ signMask)],
				...
				for (long i = first, d; i <= end; i += count[c], count[c] = 0) {
					long t = BigArrays.get(perm, i);
					c = (fixFloat(BigArrays.get(k, t)) >>> shift & DIGIT_MASK ^ signMask);
