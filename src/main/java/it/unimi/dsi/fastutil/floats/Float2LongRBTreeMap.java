 ...
 *
 */
public class Float2LongRBTreeMap extends AbstractFloat2LongSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Float2LongRBTreeMap(final Comparator<? super Float> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Float2LongRBTreeMap(final Map<? extends Float, ? extends Long> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2LongRBTreeMap(final SortedMap<Float, Long> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Float2LongRBTreeMap(final Float2LongMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2LongRBTreeMap(final Float2LongSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2LongRBTreeMap(final float[] k, final long v[], final Comparator<? super Float> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Float2LongRBTreeMap(final float[] k, final long v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2LongRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2LongMap.Entry>() {
				final Comparator<? super Float2LongMap.Entry> comparator = (Comparator<Float2LongMap.Entry>) (x,
						y) -> Float2LongRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2LongSortedMap implements java.io.Serializable {
		...
		final boolean in(final float k) {
			return (bottom || Float2LongRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2LongRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2LongMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Long))
							return false;
						final Float2LongRBTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2LongRBTreeMap.this.containsKey(k);
		}
		...
		public long get(final float k) {
			final Float2LongRBTreeMap.Entry e;
			final float kk = k;
		...
		public long put(final float k, final long v) {
			...
			final long oldValue = Float2LongRBTreeMap.this.put(k, v);
		...
		public long remove(final float k) {
			...
			final long oldValue = Float2LongRBTreeMap.this.remove(k);
		...
		@Override
		public float firstFloatKey() {
			Float2LongRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2LongRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2LongRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2LongRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
