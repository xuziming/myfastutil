 ...
 */
public class FloatRBTreeSet extends AbstractFloatSortedSet implements java.io.Serializable, Cloneable, FloatSortedSet {
	...
	private class SetIterator implements FloatListIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			FloatRBTreeSet.this.remove(curr.key);
			curr = null;
		}
	}
	...
	private final class Subset extends AbstractFloatSortedSet implements java.io.Serializable, FloatSortedSet {
		...
		final boolean in(final float k) {
			return (bottom || FloatRBTreeSet.this.compare(k, from) >= 0)
					&& (top || FloatRBTreeSet.this.compare(k, to) < 0);
		}

		@Override

		public boolean contains(final float k) {
			return in(k) && FloatRBTreeSet.this.contains(k);
		}
		...
		@Override
		public float firstFloat() {
			FloatRBTreeSet.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloat() {
			FloatRBTreeSet.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && FloatRBTreeSet.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && FloatRBTreeSet.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	}
