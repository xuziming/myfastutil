 ...
 *
 */
public class Float2BooleanRBTreeMap extends AbstractFloat2BooleanSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2BooleanRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Float2BooleanMap.Entry> float2BooleanEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2BooleanMap.Entry>() {
				final Comparator<? super Float2BooleanMap.Entry> comparator = (Comparator<Float2BooleanMap.Entry>) (x,
						y) -> Float2BooleanRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
				...
				@Override
				public ObjectSortedSet<Float2BooleanMap.Entry> subSet(Float2BooleanMap.Entry from,
						Float2BooleanMap.Entry to) {
					return subMap(from.getFloatKey(), to.getFloatKey()).float2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2BooleanMap.Entry> headSet(Float2BooleanMap.Entry to) {
					return headMap(to.getFloatKey()).float2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2BooleanMap.Entry> tailSet(Float2BooleanMap.Entry from) {
					return tailMap(from.getFloatKey()).float2BooleanEntrySet();
				}
			};
		return entries;
	}
	...
	private final class Submap extends AbstractFloat2BooleanSortedMap implements java.io.Serializable {
		...
		final boolean in(final float k) {
			return (bottom || Float2BooleanRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2BooleanRBTreeMap.this.compare(k, to) < 0);
		}
		...
		public ObjectSortedSet<Float2BooleanMap.Entry> float2BooleanEntrySet() {
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2BooleanMap.Entry>() {
					...
					@Override
					public ObjectBidirectionalIterator<Float2BooleanMap.Entry> iterator(
							final Float2BooleanMap.Entry from) {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Float2BooleanRBTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2BooleanRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public float firstFloatKey() {
			Float2BooleanRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2BooleanRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2BooleanRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2BooleanRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
