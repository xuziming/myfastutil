 ...
 * iterator}.
 */
public class Float2ObjectAVLTreeMap<V> extends AbstractFloat2ObjectSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2ObjectAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2ObjectMap.Entry<V>>() {
				final Comparator<? super Float2ObjectMap.Entry<V>> comparator = (Comparator<Float2ObjectMap.Entry<V>>) (
						x, y) -> Float2ObjectAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2ObjectSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final float k) {
			return (bottom || Float2ObjectAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2ObjectAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2ObjectMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Float))
							return false;
						final Float2ObjectAVLTreeMap.Entry<V> f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2ObjectAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public float firstFloatKey() {
			Float2ObjectAVLTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2ObjectAVLTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2ObjectAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2ObjectAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
