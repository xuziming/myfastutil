...
public interface Float2ReferenceMap<V> extends Float2ReferenceFunction<V>, Map<Float, V> {
	 ...
	 */
	interface FastEntrySet<V> extends ObjectSet<Float2ReferenceMap.Entry<V>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Float2ReferenceMap.Entry<V>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final float key, final java.util.function.DoubleFunction<? extends V> mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsentPartial(final float key, final Float2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default V computeIfPresent(final float key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final V newValue = remappingFunction.apply(Float.valueOf(key), (oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default V compute(final float key,
			final java.util.function.BiFunction<? super Float, ? super V, ? extends V> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final V oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final V newValue = remappingFunction.apply(Float.valueOf(key), contained ? (oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default V merge(final float key, final V value,
	...
	@Deprecated
	@Override
	default V computeIfAbsent(final Float key,
