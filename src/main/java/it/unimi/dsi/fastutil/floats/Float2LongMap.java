...
public interface Float2LongMap extends Float2LongFunction, Map<Float, Long> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Float2LongMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Float2LongMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final float key, final java.util.function.DoubleToLongFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsentNullable(final float key,
	 ...
	 * @since 8.0.0
	 */
	default long computeIfPresent(final float key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Long newValue = remappingFunction.apply(Float.valueOf(key), Long.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default long compute(final float key,
			final java.util.function.BiFunction<? super Float, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Long newValue = remappingFunction.apply(Float.valueOf(key), contained ? Long.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default long merge(final float key, final long value,
	...
	@Deprecated
	@Override
	default Long computeIfAbsent(final Float key,
	 ...
	 */
	interface Entry extends Map.Entry<Float, Long> {
		/**
