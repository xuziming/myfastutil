 ...
 * @see HashCommon
 */
public class Float2ByteLinkedOpenHashMap extends AbstractFloat2ByteSortedMap
	...
	public Float2ByteLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2ByteLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ByteLinkedOpenHashMap(final Map<? extends Float, ? extends Byte> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Float2ByteLinkedOpenHashMap(final Map<? extends Float, ? extends Byte> m) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ByteLinkedOpenHashMap(final Float2ByteMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Float2ByteLinkedOpenHashMap(final Float2ByteMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ByteLinkedOpenHashMap(final float[] k, final byte[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Float2ByteLinkedOpenHashMap(final float[] k, final byte[] v) {
	 ...
	 */
	public byte getAndMoveToFirst(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public byte getAndMoveToLast(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToFirst(final float k, final byte v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToLast(final float k, final byte v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				float curr;
				final float[] key = Float2ByteLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Float2ByteMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		@Override
		public ObjectSortedSet<Float2ByteMap.Entry> subSet(Float2ByteMap.Entry fromElement,
				Float2ByteMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2ByteLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2ByteLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Float2ByteMap.Entry> iterator(final Float2ByteMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Float2ByteMap.Entry> fastIterator(final Float2ByteMap.Entry from) {
		...
		public void forEach(final Consumer<? super Float2ByteMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractFloat2ByteMap.BasicEntry(key[curr], value[curr]));
			}
		}
	...
	@Override
	public FastSortedEntrySet float2ByteEntrySet() {
		if (entries == null)
