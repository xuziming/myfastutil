...
public interface Float2BooleanMap extends Float2BooleanFunction, Map<Float, Boolean> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Float2BooleanMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Float2BooleanMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsent(final float key, final java.util.function.DoublePredicate mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsentNullable(final float key,
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfPresent(final float key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Boolean newValue = remappingFunction.apply(Float.valueOf(key), Boolean.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default boolean compute(final float key,
			final java.util.function.BiFunction<? super Float, ? super Boolean, ? extends Boolean> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final boolean oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Boolean newValue = remappingFunction.apply(Float.valueOf(key),
	 ...
	 * @since 8.0.0
	 */
	default boolean merge(final float key, final boolean value,
	...
	@Deprecated
	@Override
	default Boolean computeIfAbsent(final Float key,
	 ...
	 */
	interface Entry extends Map.Entry<Float, Boolean> {
		/**
