...
public final class FloatSortedSets {
	...
	public static class Singleton extends FloatSets.Singleton
			implements FloatSortedSet, java.io.Serializable, Cloneable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		public FloatSortedSet subSet(final float from, final float to) {
			if (compare(from, element) <= 0 && compare(element, to) < 0)
				return this;
			return EMPTY_SET;
		}
	 ...
	 * element. The returned sorted set is serializable and cloneable.
	 *
	...
	/**
	 * Returns a type-specific immutable sorted set containing only the specified
	 * element, and using a specified comparator. The returned sorted set is
	...
	public static class SynchronizedSortedSet extends FloatSets.SynchronizedSet
			implements FloatSortedSet, java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		@Override
		public FloatSortedSet subSet(final float from, final float to) {
			return new SynchronizedSortedSet(sortedSet.subSet(from, to), sync);
		}

		@Override
		public FloatSortedSet headSet(final float to) {
			return new SynchronizedSortedSet(sortedSet.headSet(to), sync);
		}

		@Override
		public FloatSortedSet tailSet(final float from) {
			return new SynchronizedSortedSet(sortedSet.tailSet(from), sync);
		}
		...
		public FloatBidirectionalIterator iterator(final float from) {
			return sortedSet.iterator(from);
		}

		@Override
		public float firstFloat() {
			synchronized (sync) {
				return sortedSet.firstFloat();
			}
		}
	...
	public static class UnmodifiableSortedSet extends FloatSets.UnmodifiableSet
		...
		@Override
		public FloatSortedSet subSet(final float from, final float to) {
			return new UnmodifiableSortedSet(sortedSet.subSet(from, to));
		}

		@Override
		public FloatSortedSet headSet(final float to) {
			return new UnmodifiableSortedSet(sortedSet.headSet(to));
		}

		@Override
		public FloatSortedSet tailSet(final float from) {
			return new UnmodifiableSortedSet(sortedSet.tailSet(from));
		}

		@Override
		public FloatBidirectionalIterator iterator() {
			return FloatIterators.unmodifiable(sortedSet.iterator());
		}

		@Override
		public FloatBidirectionalIterator iterator(final float from) {
			return FloatIterators.unmodifiable(sortedSet.iterator(from));
		}
