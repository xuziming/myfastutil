...
public class Float2CharLinkedOpenHashMap extends AbstractFloat2CharSortedMap
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Float2CharMap.Entry> implements FastSortedEntrySet {
		...
		@Override
		public ObjectSortedSet<Float2CharMap.Entry> subSet(Float2CharMap.Entry fromElement,
				Float2CharMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2CharLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2CharLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Float2CharMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractFloat2CharMap.BasicEntry(key[curr], value[curr]));
			}
		}
