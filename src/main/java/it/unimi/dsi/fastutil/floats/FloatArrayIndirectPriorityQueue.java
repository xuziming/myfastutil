...
public class FloatArrayIndirectPriorityQueue implements FloatIndirectPriorityQueue {
	 ...
	 * @param c        the comparator used in this queue, or {@code null} for the
	 *                 natural order.
	 */
	 ...
	 *                 natural order.
	 */
	public FloatArrayIndirectPriorityQueue(final float[] refArray, final int[] a, int size, final FloatComparator c) {
		this(refArray, 0, c);
		this.array = a;
		this.size = size;
	}
	 ...
	 * @param size     the number of elements to be included in the queue.
	 */
	public FloatArrayIndirectPriorityQueue(final float[] refArray, final int[] a, int size) {
	...
	private int findFirst() {
		...
		int i = size;
		int firstIndex = --i;
		float first = refArray[array[firstIndex]];
		if (c == null)
			while (i-- != 0) {
				if ((Float.compare((refArray[array[i]]), (first)) < 0))
	...
	private int findLast() {
		int i = size;
		int lastIndex = --i;
		float last = refArray[array[lastIndex]];
		if (c == null) {
			while (i-- != 0)
				if ((Float.compare((last), (refArray[array[i]])) < 0))
	 ...
	 * @throws IndexOutOfBoundsException if the given index is negative or larger
	 *                                   than the reference array length.
	 */
	...
	public void enqueue(int x) {
		...
		if (firstIndexValid) {
			if (c == null) {
				if ((Float.compare((refArray[x]), (refArray[array[firstIndex]])) < 0))
					firstIndex = size;
	 ...
	 * 
	 * @param a an array whose initial part will be filled with the frnot (must be
	 *          sized as least as the heap size).
	 * @return the number of elements of the front.
	 */
	@Override
	public int front(int[] a) {
		final float top = refArray[array[findFirst()]];
		int i = size, c = 0;
		while (i-- != 0)
			if ((Float.floatToIntBits(top) == Float.floatToIntBits(refArray[array[i]])))
				a[c++] = array[i];
		return c;
	}
