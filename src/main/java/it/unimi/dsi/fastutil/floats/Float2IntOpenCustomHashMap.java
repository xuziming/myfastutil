 ...
 * @see HashCommon
 */
public class Float2IntOpenCustomHashMap extends AbstractFloat2IntMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Float2IntOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Float2IntOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2IntOpenCustomHashMap(final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2IntOpenCustomHashMap(final Map<? extends Float, ? extends Integer> m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2IntOpenCustomHashMap(final Map<? extends Float, ? extends Integer> m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2IntOpenCustomHashMap(final Float2IntMap m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Float2IntOpenCustomHashMap(final Float2IntMap m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2IntOpenCustomHashMap(final float[] k, final int[] v, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Float2IntOpenCustomHashMap(final float[] k, final int[] v,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsent(final float k, final java.util.function.DoubleToIntFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsentNullable(final float k,
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfPresent(final float k,
			final java.util.function.BiFunction<? super Float, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Integer newValue = remappingFunction.apply(Float.valueOf(k), Integer.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public int compute(final float k,
			final java.util.function.BiFunction<? super Float, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Integer newValue = remappingFunction.apply(Float.valueOf(k),
	...
	/** {@inheritDoc} */
	@Override
	public int merge(final float k, final int v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final float key[] = Float2IntOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2IntOpenCustomHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2IntMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Float2IntOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2IntOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Float2IntMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractFloat2IntMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Float.floatToIntBits(key[pos]) == 0))
					consumer.accept(new AbstractFloat2IntMap.BasicEntry(key[pos], value[pos]));
		}
