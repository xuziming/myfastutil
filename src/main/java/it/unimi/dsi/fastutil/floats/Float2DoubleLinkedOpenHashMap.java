 ...
 * @see HashCommon
 */
public class Float2DoubleLinkedOpenHashMap extends AbstractFloat2DoubleSortedMap
	...
	public Float2DoubleLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2DoubleLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Float2DoubleLinkedOpenHashMap(final Map<? extends Float, ? extends Double> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Float2DoubleLinkedOpenHashMap(final Map<? extends Float, ? extends Double> m) {
	 ...
	 * @param f the load factor.
	 */
	public Float2DoubleLinkedOpenHashMap(final Float2DoubleMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Float2DoubleLinkedOpenHashMap(final Float2DoubleMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2DoubleLinkedOpenHashMap(final float[] k, final double[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Float2DoubleLinkedOpenHashMap(final float[] k, final double[] v) {
	 ...
	 */
	public double getAndMoveToFirst(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public double getAndMoveToLast(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public double putAndMoveToFirst(final float k, final double v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public double putAndMoveToLast(final float k, final double v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				float curr;
				final float[] key = Float2DoubleLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Float2DoubleMap.Entry>
		...
		@Override
		public ObjectSortedSet<Float2DoubleMap.Entry> subSet(Float2DoubleMap.Entry fromElement,
				Float2DoubleMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		 ...
		 */
		@Override
		public ObjectListIterator<Float2DoubleMap.Entry> iterator(final Float2DoubleMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Float2DoubleMap.Entry> fastIterator(final Float2DoubleMap.Entry from) {
		...
		public void forEach(final Consumer<? super Float2DoubleMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractFloat2DoubleMap.BasicEntry(key[curr], value[curr]));
			}
		}
