 ...
 * iterator}.
 */
public class Float2ReferenceAVLTreeMap<V> extends AbstractFloat2ReferenceSortedMap<V>
	 ...
	 */
	public Float2ReferenceAVLTreeMap(final Comparator<? super Float> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Float2ReferenceAVLTreeMap(final Map<? extends Float, ? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ReferenceAVLTreeMap(final SortedMap<Float, V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Float2ReferenceAVLTreeMap(final Float2ReferenceMap<? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ReferenceAVLTreeMap(final Float2ReferenceSortedMap<V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ReferenceAVLTreeMap(final float[] k, final V v[], final Comparator<? super Float> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Float2ReferenceAVLTreeMap(final float[] k, final V v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2ReferenceAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Float2ReferenceMap.Entry<V>> float2ReferenceEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2ReferenceMap.Entry<V>>() {
				final Comparator<? super Float2ReferenceMap.Entry<V>> comparator = (Comparator<Float2ReferenceMap.Entry<V>>) (
						x,
						y) -> Float2ReferenceAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
				...
				@Override
				public ObjectBidirectionalIterator<Float2ReferenceMap.Entry<V>> iterator(
						final Float2ReferenceMap.Entry<V> from) {
				...
				public boolean contains(final Object o) {
					...
					if (e.getKey() == null || !(e.getKey() instanceof Float))
						return false;
					final Entry<V> f = findKey(((Float) (e.getKey())).floatValue());
					return e.equals(f);
				}
				...
				@Override
				public ObjectSortedSet<Float2ReferenceMap.Entry<V>> subSet(Float2ReferenceMap.Entry<V> from,
						Float2ReferenceMap.Entry<V> to) {
					return subMap(from.getFloatKey(), to.getFloatKey()).float2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2ReferenceMap.Entry<V>> headSet(Float2ReferenceMap.Entry<V> to) {
					return headMap(to.getFloatKey()).float2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2ReferenceMap.Entry<V>> tailSet(Float2ReferenceMap.Entry<V> from) {
					return tailMap(from.getFloatKey()).float2ReferenceEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractFloat2ReferenceSortedMap<V> implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final float k) {
			return (bottom || Float2ReferenceAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2ReferenceAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Float))
							return false;
						final Float2ReferenceAVLTreeMap.Entry<V> f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2ReferenceAVLTreeMap.this.containsKey(k);
		}
		...
		public V get(final float k) {
			final Float2ReferenceAVLTreeMap.Entry<V> e;
			final float kk = k;
		...
		public V put(final float k, final V v) {
			...
			final V oldValue = Float2ReferenceAVLTreeMap.this.put(k, v);
		...
		public V remove(final float k) {
			...
			final V oldValue = Float2ReferenceAVLTreeMap.this.remove(k);
		...
		@Override
		public float firstFloatKey() {
			Float2ReferenceAVLTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2ReferenceAVLTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2ReferenceAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2ReferenceAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Float2ReferenceAVLTreeMap<V> clone() {
		...
		try {
			c = (Float2ReferenceAVLTreeMap<V>) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
	...
	private Entry<V> readTree(final java.io.ObjectInputStream s, final int n, final Entry<V> pred, final Entry<V> succ)
		...
		if (n == 1) {
			final Entry<V> top = new Entry<>(s.readFloat(), (V) s.readObject());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
		if (n == 2) {
			 ...
			 */
			final Entry<V> top = new Entry<>(s.readFloat(), (V) s.readObject());
			top.right(new Entry<>(s.readFloat(), (V) s.readObject()));
			top.right.pred(top);
			top.balance(1);
			top.pred(pred);
			top.right.succ(succ);
