 ...
 * iterator}.
 */
public class Float2LongAVLTreeMap extends AbstractFloat2LongSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Float2LongAVLTreeMap(final Comparator<? super Float> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Float2LongAVLTreeMap(final Map<? extends Float, ? extends Long> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2LongAVLTreeMap(final SortedMap<Float, Long> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Float2LongAVLTreeMap(final Float2LongMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2LongAVLTreeMap(final Float2LongSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2LongAVLTreeMap(final float[] k, final long v[], final Comparator<? super Float> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Float2LongAVLTreeMap(final float[] k, final long v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry extends AbstractFloat2LongMap.BasicEntry implements Cloneable {
		 ...
		 * @param v a value.
		 */
		Entry(final float k, final long v) {
			super(k, v);
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2LongAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	 ...
	 * This class can iterate in both directions on a threaded tree.
	 */
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Float2LongMap.Entry> {
		EntryIterator() {
		}
	...
	public ObjectSortedSet<Float2LongMap.Entry> float2LongEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2LongMap.Entry>() {
				final Comparator<? super Float2LongMap.Entry> comparator = (Comparator<Float2LongMap.Entry>) (x,
						y) -> Float2LongAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
				...
				public ObjectSortedSet<Float2LongMap.Entry> subSet(Float2LongMap.Entry from, Float2LongMap.Entry to) {
					return subMap(from.getFloatKey(), to.getFloatKey()).float2LongEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2LongMap.Entry> headSet(Float2LongMap.Entry to) {
					return headMap(to.getFloatKey()).float2LongEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2LongMap.Entry> tailSet(Float2LongMap.Entry from) {
					return tailMap(from.getFloatKey()).float2LongEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractFloat2LongSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final float k) {
			return (bottom || Float2LongAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2LongAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2LongMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Long))
							return false;
						final Float2LongAVLTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
					...
					@Override
					public ObjectSortedSet<Float2LongMap.Entry> subSet(Float2LongMap.Entry from,
							Float2LongMap.Entry to) {
						return subMap(from.getFloatKey(), to.getFloatKey()).float2LongEntrySet();
					}
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2LongAVLTreeMap.this.containsKey(k);
		}
		...
		public long get(final float k) {
			final Float2LongAVLTreeMap.Entry e;
			final float kk = k;
		...
		public long put(final float k, final long v) {
			...
			final long oldValue = Float2LongAVLTreeMap.this.put(k, v);
		...
		public long remove(final float k) {
			...
			final long oldValue = Float2LongAVLTreeMap.this.remove(k);
		...
		@Override
		public float firstFloatKey() {
			Float2LongAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2LongAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2LongAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2LongAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry readTree(final java.io.ObjectInputStream s, final int n, final Entry pred, final Entry succ)
		...
		if (n == 1) {
			final Entry top = new Entry(s.readFloat(), s.readLong());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
