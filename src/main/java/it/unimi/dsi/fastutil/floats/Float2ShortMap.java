...
public interface Float2ShortMap extends Float2ShortFunction, Map<Float, Short> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Float2ShortMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Float2ShortMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	...
	default boolean containsValue(final Object value) {
		return value == null ? false : containsValue(((Short) (value)).shortValue());
	}
	 ...
	 * @since 8.0.0
	 */
	default short getOrDefault(final float key, final short defaultValue) {
		final short v;
	 ...
	 * @since 8.0.0
	 */
	default short putIfAbsent(final float key, final short value) {
		final short v = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default boolean remove(final float key, final short value) {
		final short curValue = get(key);
	 ...
	 * @since 8.0.0
	 */
	default boolean replace(final float key, final short oldValue, final short newValue) {
		final short curValue = get(key);
	 ...
	 * @since 8.0.0
	 */
	default short replace(final float key, final short value) {
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final float key, final java.util.function.DoubleToIntFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = get(key);
		if (v != defaultReturnValue() || containsKey(key))
			return v;
		short newValue = it.unimi.dsi.fastutil.SafeMath.safeIntToShort(mappingFunction.applyAsInt(key));
		put(key, newValue);
		return newValue;
	}
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsentNullable(final float key,
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsentPartial(final float key, final Float2ShortFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default short computeIfPresent(final float key,
			final java.util.function.BiFunction<? super Float, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Short newValue = remappingFunction.apply(Float.valueOf(key), Short.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default short compute(final float key,
			final java.util.function.BiFunction<? super Float, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Short newValue = remappingFunction.apply(Float.valueOf(key), contained ? Short.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
		}
		final short newVal = (newValue).shortValue();
		put(key, newVal);
		return newVal;
	}
	 ...
	 * @since 8.0.0
	 */
	default short merge(final float key, final short value,
			final java.util.function.BiFunction<? super Short, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
		final short newValue;
		if (oldValue != drv || containsKey(key)) {
			final Short mergedValue = remappingFunction.apply(Short.valueOf(oldValue), Short.valueOf(value));
			if (mergedValue == null) {
				remove(key);
				return drv;
			}
	...
	@Deprecated
	@Override
	default Short computeIfAbsent(final Float key,
			final java.util.function.Function<? super Float, ? extends Short> mappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Float, Short> {
		/**
		 ...
		 */
		short setValue(final short value);
