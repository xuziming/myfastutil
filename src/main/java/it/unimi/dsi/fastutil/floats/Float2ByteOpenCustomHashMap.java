 ...
 * @see HashCommon
 */
public class Float2ByteOpenCustomHashMap extends AbstractFloat2ByteMap
	...
	public Float2ByteOpenCustomHashMap(final int expected, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ByteOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ByteOpenCustomHashMap(final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ByteOpenCustomHashMap(final Map<? extends Float, ? extends Byte> m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ByteOpenCustomHashMap(final Map<? extends Float, ? extends Byte> m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ByteOpenCustomHashMap(final Float2ByteMap m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ByteOpenCustomHashMap(final Float2ByteMap m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ByteOpenCustomHashMap(final float[] k, final byte[] v, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Float2ByteOpenCustomHashMap(final float[] k, final byte[] v,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	...
	private int find(final float k) {
		...
		if ((Float.floatToIntBits(
				curr = key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(k))) & mask]) == 0))
			return -(pos + 1);
	...
	private void insert(final int pos, final float k, final byte v) {
		if (pos == n)
	...
	public byte getOrDefault(final float k, final byte defaultValue) {
		if ((strategy.equals((k), (0))))
	...
	/** {@inheritDoc} */
	@Override
	public byte putIfAbsent(final float k, final byte v) {
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
		insert(-pos - 1, k, v);
	...
	public boolean remove(final float k, final byte v) {
		if ((strategy.equals((k), (0)))) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean replace(final float k, final byte oldValue, final byte v) {
		final int pos = find(k);
	...
	/** {@inheritDoc} */
	@Override
	public byte replace(final float k, final byte v) {
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final byte oldValue = value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsent(final float k, final java.util.function.DoubleToIntFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsentNullable(final float k,
			final java.util.function.DoubleFunction<? extends Byte> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfPresent(final float k,
			final java.util.function.BiFunction<? super Float, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Byte newValue = remappingFunction.apply(Float.valueOf(k), Byte.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public byte compute(final float k,
			final java.util.function.BiFunction<? super Float, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Byte newValue = remappingFunction.apply(Float.valueOf(k), pos >= 0 ? Byte.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public byte merge(final float k, final byte v,
	...
	final class MapEntry implements Float2ByteMap.Entry, Map.Entry<Float, Byte> {
		...
		public boolean equals(final Object o) {
			...
			return (strategy.equals((key[index]), ((e.getKey()).floatValue())))
					&& ((value[index]) == ((e.getValue()).byteValue()));
		}
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final float key[] = Float2ByteOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2ByteOpenCustomHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ByteMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Float2ByteOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2ByteOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		/** {@inheritDoc} */
		@Override
		public void forEach(final Consumer<? super Float2ByteMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractFloat2ByteMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Float.floatToIntBits(key[pos]) == 0))
					consumer.accept(new AbstractFloat2ByteMap.BasicEntry(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Float2ByteMap.Entry> consumer) {
	...
	protected void rehash(final int newN) {
		...
		for (int j = realSize(); j-- != 0;) {
			while ((Float.floatToIntBits(key[--i]) == 0))
				;
			if (!(Float.floatToIntBits(
					newKey[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(key[i]))) & mask]) == 0))
