 ...
 * @see HashCommon
 */
public class Float2ShortOpenCustomHashMap extends AbstractFloat2ShortMap
	...
	public Float2ShortOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ShortOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ShortOpenCustomHashMap(final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ShortOpenCustomHashMap(final Map<? extends Float, ? extends Short> m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ShortOpenCustomHashMap(final Map<? extends Float, ? extends Short> m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ShortOpenCustomHashMap(final Float2ShortMap m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Float2ShortOpenCustomHashMap(final Float2ShortMap m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ShortOpenCustomHashMap(final float[] k, final short[] v, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Float2ShortOpenCustomHashMap(final float[] k, final short[] v,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsent(final float k, final java.util.function.DoubleToIntFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsentNullable(final float k,
			final java.util.function.DoubleFunction<? extends Short> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfPresent(final float k,
			final java.util.function.BiFunction<? super Float, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Short newValue = remappingFunction.apply(Float.valueOf(k), Short.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public short compute(final float k,
			final java.util.function.BiFunction<? super Float, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Short newValue = remappingFunction.apply(Float.valueOf(k), pos >= 0 ? Short.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public short merge(final float k, final short v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2ShortOpenCustomHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ShortMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Float2ShortOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2ShortOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Float2ShortMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractFloat2ShortMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Float.floatToIntBits(key[pos]) == 0))
					consumer.accept(new AbstractFloat2ShortMap.BasicEntry(key[pos], value[pos]));
		}
