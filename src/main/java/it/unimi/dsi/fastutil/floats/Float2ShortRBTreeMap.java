 ...
 *
 */
public class Float2ShortRBTreeMap extends AbstractFloat2ShortSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Float2ShortRBTreeMap(final Comparator<? super Float> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Float2ShortRBTreeMap(final Map<? extends Float, ? extends Short> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ShortRBTreeMap(final SortedMap<Float, Short> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Float2ShortRBTreeMap(final Float2ShortMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ShortRBTreeMap(final Float2ShortSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ShortRBTreeMap(final float[] k, final short v[], final Comparator<? super Float> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Float2ShortRBTreeMap(final float[] k, final short v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2ShortRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Float2ShortMap.Entry> float2ShortEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2ShortMap.Entry>() {
				final Comparator<? super Float2ShortMap.Entry> comparator = (Comparator<Float2ShortMap.Entry>) (x,
						y) -> Float2ShortRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
				...
				@Override
				public ObjectSortedSet<Float2ShortMap.Entry> subSet(Float2ShortMap.Entry from,
						Float2ShortMap.Entry to) {
					return subMap(from.getFloatKey(), to.getFloatKey()).float2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2ShortMap.Entry> headSet(Float2ShortMap.Entry to) {
					return headMap(to.getFloatKey()).float2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2ShortMap.Entry> tailSet(Float2ShortMap.Entry from) {
					return tailMap(from.getFloatKey()).float2ShortEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractFloat2ShortSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final float k) {
			return (bottom || Float2ShortRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2ShortRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Float2ShortRBTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2ShortRBTreeMap.this.containsKey(k);
		}
		...
		public short get(final float k) {
			final Float2ShortRBTreeMap.Entry e;
			final float kk = k;
		...
		public short put(final float k, final short v) {
			...
			final short oldValue = Float2ShortRBTreeMap.this.put(k, v);
		...
		public short remove(final float k) {
			...
			final short oldValue = Float2ShortRBTreeMap.this.remove(k);
		...
		@Override
		public float firstFloatKey() {
			Float2ShortRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2ShortRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2ShortRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2ShortRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Float2ShortRBTreeMap clone() {
		...
		try {
			c = (Float2ShortRBTreeMap) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
