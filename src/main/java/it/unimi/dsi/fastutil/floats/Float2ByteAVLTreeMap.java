 ...
 * iterator}.
 */
public class Float2ByteAVLTreeMap extends AbstractFloat2ByteSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Float2ByteAVLTreeMap(final Comparator<? super Float> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Float2ByteAVLTreeMap(final Map<? extends Float, ? extends Byte> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ByteAVLTreeMap(final SortedMap<Float, Byte> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Float2ByteAVLTreeMap(final Float2ByteMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ByteAVLTreeMap(final Float2ByteSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ByteAVLTreeMap(final float[] k, final byte v[], final Comparator<? super Float> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Float2ByteAVLTreeMap(final float[] k, final byte v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2ByteAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2ByteMap.Entry>() {
				final Comparator<? super Float2ByteMap.Entry> comparator = (Comparator<Float2ByteMap.Entry>) (x,
						y) -> Float2ByteAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2ByteSortedMap implements java.io.Serializable {
		...
		final boolean in(final float k) {
			return (bottom || Float2ByteAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2ByteAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2ByteMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Byte))
							return false;
						final Float2ByteAVLTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2ByteAVLTreeMap.this.containsKey(k);
		}
		...
		public byte get(final float k) {
			final Float2ByteAVLTreeMap.Entry e;
			final float kk = k;
		...
		public byte put(final float k, final byte v) {
			...
			final byte oldValue = Float2ByteAVLTreeMap.this.put(k, v);
		...
		public byte remove(final float k) {
			...
			final byte oldValue = Float2ByteAVLTreeMap.this.remove(k);
		...
		@Override
		public float firstFloatKey() {
			Float2ByteAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2ByteAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2ByteAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2ByteAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
