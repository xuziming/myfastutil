 ...
 * @see HashCommon
 */
public class Float2ReferenceOpenHashMap<V> extends AbstractFloat2ReferenceMap<V>
	...
	public Float2ReferenceOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2ReferenceOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ReferenceOpenHashMap(final Map<? extends Float, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Float2ReferenceOpenHashMap(final Map<? extends Float, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ReferenceOpenHashMap(final Float2ReferenceMap<V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Float2ReferenceOpenHashMap(final Float2ReferenceMap<V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ReferenceOpenHashMap(final float[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Float2ReferenceOpenHashMap(final float[] k, final V[] v) {
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfAbsent(final float k, final java.util.function.DoubleFunction<? extends V> mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfPresent(final float k,
			final java.util.function.BiFunction<? super Float, ? super V, ? extends V> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final V newValue = remappingFunction.apply(Float.valueOf(k), (value[pos]));
		if (newValue == null) {
			if ((Float.floatToIntBits(k) == 0))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public V compute(final float k,
			final java.util.function.BiFunction<? super Float, ? super V, ? extends V> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final V newValue = remappingFunction.apply(Float.valueOf(k), pos >= 0 ? (value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((Float.floatToIntBits(k) == 0))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public V merge(final float k, final V v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final float key[] = Float2ReferenceOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2ReferenceOpenHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2ReferenceOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2ReferenceOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		/** {@inheritDoc} */
		@Override
		public void forEach(final Consumer<? super Float2ReferenceMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractFloat2ReferenceMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Float.floatToIntBits(key[pos]) == 0))
					consumer.accept(new AbstractFloat2ReferenceMap.BasicEntry<V>(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Float2ReferenceMap.Entry<V>> consumer) {
