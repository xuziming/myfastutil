 ...
 * @see HashCommon
 */
public class Float2LongLinkedOpenHashMap extends AbstractFloat2LongSortedMap
	...
	public Float2LongLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2LongLinkedOpenHashMap(final int expected) {
	 ...
	 */
	public long getAndMoveToFirst(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public long getAndMoveToLast(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public long putAndMoveToFirst(final float k, final long v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public long putAndMoveToLast(final float k, final long v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				float curr;
				final float[] key = Float2LongLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Float2LongMap.Entry> implements FastSortedEntrySet {
		...
		@Override
		public ObjectSortedSet<Float2LongMap.Entry> subSet(Float2LongMap.Entry fromElement,
				Float2LongMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2LongLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2LongLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Float2LongMap.Entry> iterator(final Float2LongMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Float2LongMap.Entry> fastIterator(final Float2LongMap.Entry from) {
		...
		public void forEach(final Consumer<? super Float2LongMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractFloat2LongMap.BasicEntry(key[curr], value[curr]));
			}
		}
