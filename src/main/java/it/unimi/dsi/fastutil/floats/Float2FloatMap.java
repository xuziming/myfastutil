...
public interface Float2FloatMap extends Float2FloatFunction, Map<Float, Float> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Float2FloatMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Float2FloatMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final float key, final java.util.function.DoubleUnaryOperator mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsentNullable(final float key,
	 ...
	 * @since 8.0.0
	 */
	default float computeIfPresent(final float key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Float newValue = remappingFunction.apply(Float.valueOf(key), Float.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default float compute(final float key,
			final java.util.function.BiFunction<? super Float, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Float newValue = remappingFunction.apply(Float.valueOf(key), contained ? Float.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default float merge(final float key, final float value,
			final java.util.function.BiFunction<? super Float, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
	...
	@Deprecated
	@Override
	default Float computeIfAbsent(final Float key,
	 ...
	 */
	interface Entry extends Map.Entry<Float, Float> {
		/**
