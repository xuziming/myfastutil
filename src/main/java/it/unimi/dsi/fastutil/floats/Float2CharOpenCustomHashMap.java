 ...
 * @see HashCommon
 */
public class Float2CharOpenCustomHashMap extends AbstractFloat2CharMap
	...
	public Float2CharOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Float2CharOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2CharOpenCustomHashMap(final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2CharOpenCustomHashMap(final Map<? extends Float, ? extends Character> m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2CharOpenCustomHashMap(final Map<? extends Float, ? extends Character> m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Float2CharOpenCustomHashMap(final Float2CharMap m, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Float2CharOpenCustomHashMap(final Float2CharMap m,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2CharOpenCustomHashMap(final float[] k, final char[] v, final float f,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Float2CharOpenCustomHashMap(final float[] k, final char[] v,
			final it.unimi.dsi.fastutil.floats.FloatHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfAbsent(final float k, final java.util.function.DoubleToIntFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfAbsentNullable(final float k,
			final java.util.function.DoubleFunction<? extends Character> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfPresent(final float k,
			final java.util.function.BiFunction<? super Float, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Character newValue = remappingFunction.apply(Float.valueOf(k), Character.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public char compute(final float k,
			final java.util.function.BiFunction<? super Float, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Character newValue = remappingFunction.apply(Float.valueOf(k),
	...
	/** {@inheritDoc} */
	@Override
	public char merge(final float k, final char v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2CharOpenCustomHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2CharMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Float2CharOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2CharOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Float2CharMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractFloat2CharMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Float.floatToIntBits(key[pos]) == 0))
					consumer.accept(new AbstractFloat2CharMap.BasicEntry(key[pos], value[pos]));
		}
