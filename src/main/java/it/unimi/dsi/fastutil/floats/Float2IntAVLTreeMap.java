 ...
 * iterator}.
 */
public class Float2IntAVLTreeMap extends AbstractFloat2IntSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2IntAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2IntMap.Entry>() {
				final Comparator<? super Float2IntMap.Entry> comparator = (Comparator<Float2IntMap.Entry>) (x,
						y) -> Float2IntAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2IntSortedMap implements java.io.Serializable {
		...
		final boolean in(final float k) {
			return (bottom || Float2IntAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2IntAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2IntMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Integer))
							return false;
						final Float2IntAVLTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2IntAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public float firstFloatKey() {
			Float2IntAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2IntAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2IntAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2IntAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
