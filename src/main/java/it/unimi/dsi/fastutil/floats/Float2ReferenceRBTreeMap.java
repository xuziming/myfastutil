 ...
 *
 */
public class Float2ReferenceRBTreeMap<V> extends AbstractFloat2ReferenceSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2ReferenceRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2ReferenceMap.Entry<V>>() {
				final Comparator<? super Float2ReferenceMap.Entry<V>> comparator = (Comparator<Float2ReferenceMap.Entry<V>>) (
						x,
						y) -> Float2ReferenceRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2ReferenceSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final float k) {
			return (bottom || Float2ReferenceRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2ReferenceRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Float))
							return false;
						final Float2ReferenceRBTreeMap.Entry<V> f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2ReferenceRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public float firstFloatKey() {
			Float2ReferenceRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2ReferenceRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2ReferenceRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2ReferenceRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Float2ReferenceRBTreeMap<V> clone() {
		...
		try {
			c = (Float2ReferenceRBTreeMap<V>) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
