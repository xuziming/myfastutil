 ...
 * iterator}.
 */
public class Float2ShortAVLTreeMap extends AbstractFloat2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2ShortAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2ShortMap.Entry>() {
				final Comparator<? super Float2ShortMap.Entry> comparator = (Comparator<Float2ShortMap.Entry>) (x,
						y) -> Float2ShortAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2ShortSortedMap implements java.io.Serializable {
		...
		final boolean in(final float k) {
			return (bottom || Float2ShortAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2ShortAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Float2ShortAVLTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2ShortAVLTreeMap.this.containsKey(k);
		}
		...
		public short put(final float k, final short v) {
			...
			final short oldValue = Float2ShortAVLTreeMap.this.put(k, v);
		...
		@Override
		public float firstFloatKey() {
			Float2ShortAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2ShortAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2ShortAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2ShortAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
