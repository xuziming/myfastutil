...
public class Float2ObjectOpenHashMap<V> extends AbstractFloat2ObjectMap<V>
	...
	public Float2ObjectOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2ObjectOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ObjectOpenHashMap(final Map<? extends Float, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Float2ObjectOpenHashMap(final Map<? extends Float, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ObjectOpenHashMap(final Float2ObjectMap<V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Float2ObjectOpenHashMap(final Float2ObjectMap<V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ObjectOpenHashMap(final float[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Float2ObjectOpenHashMap(final float[] k, final V[] v) {
	...
	public boolean containsValue(final Object v) {
		...
		if (containsNullKey && java.util.Objects.equals(value[n], v))
			return true;
		for (int i = n; i-- != 0;)
			if (!(Float.floatToIntBits(key[i]) == 0) && java.util.Objects.equals(value[i], v))
				return true;
		return false;
	}
	...
	public boolean remove(final float k, final Object v) {
		...
		if ((Float.floatToIntBits(
				curr = key[pos = it.unimi.dsi.fastutil.HashCommon.mix(it.unimi.dsi.fastutil.HashCommon.float2int(k))
						& mask]) == 0))
			return false;
		if ((Float.floatToIntBits(k) == Float.floatToIntBits(curr)) && java.util.Objects.equals(v, value[pos])) {
			removeEntry(pos);
			return true;
		}
		while (true) {
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final float key[] = Float2ObjectOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2ObjectOpenHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2ObjectOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			float curr;
			final float[] key = Float2ObjectOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Float2ObjectMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractFloat2ObjectMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Float.floatToIntBits(key[pos]) == 0))
					consumer.accept(new AbstractFloat2ObjectMap.BasicEntry<V>(key[pos], value[pos]));
		}
