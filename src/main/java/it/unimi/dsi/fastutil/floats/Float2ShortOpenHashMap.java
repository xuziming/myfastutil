...
public class Float2ShortOpenHashMap extends AbstractFloat2ShortMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Float2ShortOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2ShortOpenHashMap(final int expected) {
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final float key[] = Float2ShortOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2ShortOpenHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2ShortMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2ShortOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2ShortOpenHashMap.this.key;
			int pos;
			// The starting point.
