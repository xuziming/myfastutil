 ...
 *
 */
public class Float2DoubleRBTreeMap extends AbstractFloat2DoubleSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2DoubleMap.Entry>() {
				final Comparator<? super Float2DoubleMap.Entry> comparator = (Comparator<Float2DoubleMap.Entry>) (x,
						y) -> Float2DoubleRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2DoubleSortedMap implements java.io.Serializable {
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2DoubleMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Double))
							return false;
						final Float2DoubleRBTreeMap.Entry f = findKey(((Float) (e.getKey())).floatValue());
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2DoubleRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2DoubleRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
