...
public class Float2BooleanAVLTreeMap extends AbstractFloat2BooleanSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2BooleanMap.Entry>() {
				final Comparator<? super Float2BooleanMap.Entry> comparator = (Comparator<Float2BooleanMap.Entry>) (x,
						y) -> Float2BooleanAVLTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
	...
	private final class Submap extends AbstractFloat2BooleanSortedMap implements java.io.Serializable {
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2BooleanAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2BooleanAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
