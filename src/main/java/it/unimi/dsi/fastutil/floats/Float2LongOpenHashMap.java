 ...
 * @see HashCommon
 */
public class Float2LongOpenHashMap extends AbstractFloat2LongMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Float2LongOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2LongOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Float2LongOpenHashMap(final Map<? extends Float, ? extends Long> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Float2LongOpenHashMap(final Map<? extends Float, ? extends Long> m) {
	 ...
	 * @param f the load factor.
	 */
	public Float2LongOpenHashMap(final Float2LongMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Float2LongOpenHashMap(final Float2LongMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2LongOpenHashMap(final float[] k, final long[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Float2LongOpenHashMap(final float[] k, final long[] v) {
	...
	private int find(final float k) {
		...
		if ((Float.floatToIntBits(
				curr = key[pos = it.unimi.dsi.fastutil.HashCommon.mix(it.unimi.dsi.fastutil.HashCommon.float2int(k))
						& mask]) == 0))
			return -(pos + 1);
		...
		while (true) {
			if ((Float.floatToIntBits(curr = key[pos = (pos + 1) & mask]) == 0))
				return -(pos + 1);
			if ((Float.floatToIntBits(k) == Float.floatToIntBits(curr)))
				return pos;
		}
	...
	private void insert(final int pos, final float k, final long v) {
		if (pos == n)
	...
	/** {@inheritDoc} */
	@Override
	public long putIfAbsent(final float k, final long v) {
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
		insert(-pos - 1, k, v);
	...
	public boolean remove(final float k, final long v) {
		if ((Float.floatToIntBits(k) == 0)) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean replace(final float k, final long oldValue, final long v) {
		final int pos = find(k);
	...
	/** {@inheritDoc} */
	@Override
	public long replace(final float k, final long v) {
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final long oldValue = value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsent(final float k, final java.util.function.DoubleToLongFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsentNullable(final float k,
			final java.util.function.DoubleFunction<? extends Long> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfPresent(final float k,
			final java.util.function.BiFunction<? super Float, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Long newValue = remappingFunction.apply(Float.valueOf(k), Long.valueOf(value[pos]));
		if (newValue == null) {
			if ((Float.floatToIntBits(k) == 0))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public long compute(final float k,
			final java.util.function.BiFunction<? super Float, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Long newValue = remappingFunction.apply(Float.valueOf(k), pos >= 0 ? Long.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((Float.floatToIntBits(k) == 0))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public long merge(final float k, final long v,
	...
	final class MapEntry implements Float2LongMap.Entry, Map.Entry<Float, Long> {
		...
		public boolean equals(final Object o) {
			...
			return (Float.floatToIntBits(key[index]) == Float.floatToIntBits((e.getKey()).floatValue()))
					&& ((value[index]) == ((e.getValue()).longValue()));
		}

		@Override
		public int hashCode() {
			return it.unimi.dsi.fastutil.HashCommon.float2int(key[index])
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final float key[] = Float2LongOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
					// We are just enumerating elements from the wrapped list.
					last = Integer.MIN_VALUE;
					final float k = wrapped.getFloat(-pos - 1);
					int p = it.unimi.dsi.fastutil.HashCommon.mix(it.unimi.dsi.fastutil.HashCommon.float2int(k)) & mask;
		...
		private void shiftKeys(int pos) {
			...
			for (;;) {
				...
				for (;;) {
					...
					slot = it.unimi.dsi.fastutil.HashCommon.mix(it.unimi.dsi.fastutil.HashCommon.float2int(curr))
							& mask;
					if (last <= pos ? last >= slot || slot > pos : last >= slot && slot > pos)
						break;
					pos = (pos + 1) & mask;
		...
		public void remove() {
			...
			if (last == n) {
				...
				Float2LongOpenHashMap.this.remove(wrapped.getFloat(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Float2LongMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2LongOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2LongOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Float2LongMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractFloat2LongMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Float.floatToIntBits(key[pos]) == 0))
					consumer.accept(new AbstractFloat2LongMap.BasicEntry(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Float2LongMap.Entry> consumer) {
	...
	protected void rehash(final int newN) {
		...
		for (int j = realSize(); j-- != 0;) {
			while ((Float.floatToIntBits(key[--i]) == 0))
				;
			if (!(Float.floatToIntBits(newKey[pos = it.unimi.dsi.fastutil.HashCommon
					.mix(it.unimi.dsi.fastutil.HashCommon.float2int(key[i])) & mask]) == 0))
	...
	public int hashCode() {
		...
		if (containsNullKey)
			h += it.unimi.dsi.fastutil.HashCommon.long2int(value[n]);
		return h;
	}
