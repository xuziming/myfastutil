 ...
 *
 */
public class Float2ObjectRBTreeMap<V> extends AbstractFloat2ObjectSortedMap<V>
	 ...
	 */
	public Float2ObjectRBTreeMap(final Comparator<? super Float> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Float2ObjectRBTreeMap(final Map<? extends Float, ? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ObjectRBTreeMap(final SortedMap<Float, V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Float2ObjectRBTreeMap(final Float2ObjectMap<? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Float2ObjectRBTreeMap(final Float2ObjectSortedMap<V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ObjectRBTreeMap(final float[] k, final V v[], final Comparator<? super Float> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Float2ObjectRBTreeMap(final float[] k, final V v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry<V> extends AbstractFloat2ObjectMap.BasicEntry<V> implements Cloneable {
		...
		@Override
		public int hashCode() {
			return it.unimi.dsi.fastutil.HashCommon.float2int(key) ^ ((value) == null ? 0 : (value).hashCode());
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Float2ObjectRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Float2ObjectMap.Entry<V>> float2ObjectEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Float2ObjectMap.Entry<V>>() {
				final Comparator<? super Float2ObjectMap.Entry<V>> comparator = (Comparator<Float2ObjectMap.Entry<V>>) (
						x, y) -> Float2ObjectRBTreeMap.this.actualComparator.compare(x.getFloatKey(), y.getFloatKey());
				...
				@Override
				public ObjectBidirectionalIterator<Float2ObjectMap.Entry<V>> iterator(
						final Float2ObjectMap.Entry<V> from) {
				...
				@Override
				public ObjectSortedSet<Float2ObjectMap.Entry<V>> subSet(Float2ObjectMap.Entry<V> from,
						Float2ObjectMap.Entry<V> to) {
					return subMap(from.getFloatKey(), to.getFloatKey()).float2ObjectEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2ObjectMap.Entry<V>> headSet(Float2ObjectMap.Entry<V> to) {
					return headMap(to.getFloatKey()).float2ObjectEntrySet();
				}

				@Override
				public ObjectSortedSet<Float2ObjectMap.Entry<V>> tailSet(Float2ObjectMap.Entry<V> from) {
					return tailMap(from.getFloatKey()).float2ObjectEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractFloat2ObjectSortedMap<V> implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final float k) {
			return (bottom || Float2ObjectRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Float2ObjectRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Float2ObjectMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Float))
							return false;
						final Float2ObjectRBTreeMap.Entry<V> f = findKey(((Float) (e.getKey())).floatValue());
		...
		public boolean containsKey(final float k) {
			return in(k) && Float2ObjectRBTreeMap.this.containsKey(k);
		}
		...
		public V get(final float k) {
			final Float2ObjectRBTreeMap.Entry<V> e;
			final float kk = k;
		...
		public V put(final float k, final V v) {
			...
			final V oldValue = Float2ObjectRBTreeMap.this.put(k, v);
		...
		public V remove(final float k) {
			...
			final V oldValue = Float2ObjectRBTreeMap.this.remove(k);
		...
		@Override
		public float firstFloatKey() {
			Float2ObjectRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public float lastFloatKey() {
			Float2ObjectRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Float2ObjectRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Float2ObjectRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
