...
public class FloatBigArrayBigList extends AbstractFloatBigList
	 ...
	 */
	protected FloatBigArrayBigList(final float a[][], boolean dummy) {
		this.a = a;
	}
	...
	public FloatBigArrayBigList(final long capacity) {
		...
		if (capacity == 0)
			a = FloatBigArrays.EMPTY_BIG_ARRAY;
		else
			a = FloatBigArrays.newBigArray(capacity);
	}
	...
	public FloatBigArrayBigList(final FloatCollection c) {
		this(c.size());
		for (FloatIterator i = c.iterator(); i.hasNext();)
			add(i.nextFloat());
	}
	...
	public void ensureCapacity(final long capacity) {
		if (capacity <= a.length || a == FloatBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
			return;
	...
	private void grow(long capacity) {
		...
		if (capacity <= oldLength)
			return;
		if (a != FloatBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
	...
	public long indexOf(final float k) {
		for (long i = 0; i < size; i++)
			if ((Float.floatToIntBits(k) == Float.floatToIntBits(BigArrays.get(a, i))))
				return i;
		return -1;
	}
	...
	public void size(final long size) {
		...
		if (size > this.size)
			BigArrays.fill(a, this.size, size, (0));
		this.size = size;
	}
	...
	public FloatBigArrayBigList clone() {
		FloatBigArrayBigList c = new FloatBigArrayBigList(size);
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		for (int i = 0; i < size; i++)
			s.writeFloat(BigArrays.get(a, i));
	}
