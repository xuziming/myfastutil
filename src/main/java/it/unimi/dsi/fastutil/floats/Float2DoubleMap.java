...
public interface Float2DoubleMap extends Float2DoubleFunction, Map<Float, Double> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Float2DoubleMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Float2DoubleMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final float key, final java.util.function.DoubleUnaryOperator mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsentNullable(final float key,
	 ...
	 * @since 8.0.0
	 */
	default double computeIfPresent(final float key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Double newValue = remappingFunction.apply(Float.valueOf(key), Double.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default double compute(final float key,
			final java.util.function.BiFunction<? super Float, ? super Double, ? extends Double> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final double oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Double newValue = remappingFunction.apply(Float.valueOf(key),
	 ...
	 * @since 8.0.0
	 */
	default double merge(final float key, final double value,
	...
	@Deprecated
	@Override
	default Double computeIfAbsent(final Float key,
	 ...
	 */
	interface Entry extends Map.Entry<Float, Double> {
		/**
