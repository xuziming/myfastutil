...
public interface Float2ByteMap extends Float2ByteFunction, Map<Float, Byte> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Float2ByteMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Float2ByteMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final float key, final java.util.function.DoubleToIntFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsentNullable(final float key,
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfPresent(final float key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Byte newValue = remappingFunction.apply(Float.valueOf(key), Byte.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default byte compute(final float key,
			final java.util.function.BiFunction<? super Float, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Byte newValue = remappingFunction.apply(Float.valueOf(key), contained ? Byte.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default byte merge(final float key, final byte value,
	...
	@Deprecated
	@Override
	default Byte computeIfAbsent(final Float key,
	 ...
	 */
	interface Entry extends Map.Entry<Float, Byte> {
		/**
