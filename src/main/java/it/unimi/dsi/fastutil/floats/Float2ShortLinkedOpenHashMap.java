 ...
 * @see HashCommon
 */
public class Float2ShortLinkedOpenHashMap extends AbstractFloat2ShortSortedMap
	...
	public Float2ShortLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Float2ShortLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ShortLinkedOpenHashMap(final Map<? extends Float, ? extends Short> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Float2ShortLinkedOpenHashMap(final Map<? extends Float, ? extends Short> m) {
	 ...
	 * @param f the load factor.
	 */
	public Float2ShortLinkedOpenHashMap(final Float2ShortMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Float2ShortLinkedOpenHashMap(final Float2ShortMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Float2ShortLinkedOpenHashMap(final float[] k, final short[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Float2ShortLinkedOpenHashMap(final float[] k, final short[] v) {
	 ...
	 */
	public short getAndMoveToFirst(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public short getAndMoveToLast(final float k) {
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public short putAndMoveToFirst(final float k, final short v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public short putAndMoveToLast(final float k, final short v) {
		int pos;
		if ((Float.floatToIntBits(k) == 0)) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				float curr;
				final float[] key = Float2ShortLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Float2ShortMap.Entry>
			implements FastSortedEntrySet {
		@Override
		...
		@Override
		public ObjectSortedSet<Float2ShortMap.Entry> subSet(Float2ShortMap.Entry fromElement,
				Float2ShortMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if ((Float.floatToIntBits(k) == 0))
				return Float2ShortLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			float curr;
			final float[] key = Float2ShortLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Float2ShortMap.Entry> iterator(final Float2ShortMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Float2ShortMap.Entry> fastIterator(final Float2ShortMap.Entry from) {
		...
		public void forEach(final Consumer<? super Float2ShortMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractFloat2ShortMap.BasicEntry(key[curr], value[curr]));
			}
		}
