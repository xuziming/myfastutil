 ...
 *
 * <p>
 * In addition to commodity methods, this class contains {@link Swapper}-based
 * implementations of {@linkplain #quickSort(int, int, IntComparator, Swapper)
 * quicksort} and of a stable, in-place
 * {@linkplain #mergeSort(int, int, IntComparator, Swapper) mergesort}. These
 * generic sorting methods can be used to sort any kind of list, but they find
 * their natural usage, for instance, in sorting arrays in parallel.
 *
 * @see Arrays
 */
...
public class Arrays {
	 ...
	 *                                        greater than {@code arrayLength} or
	 *                                        negative.
	 */
	public static void ensureFromTo(final int arrayLength, final int from, final int to) {
		...
		if (to > arrayLength)
			throw new ArrayIndexOutOfBoundsException(
					"End index (" + to + ") is greater than array length (" + arrayLength + ")");
	}
	 ...
	 */
	private static void inPlaceMerge(final int from, int mid, final int to, final IntComparator comp,
	 ...
	 *
	 * @param from the index of the first element (inclusive) to be included in the
	 *             binary search.
	 * @param to   the index of the last element (exclusive) to be included in the
	 *             binary search.
	 ...
	 *         {@code [first..i)}, {@code comp.compare(j, pos)} is {@code true}.
	 */
	 ...
	 * @param comp the comparison function.
	 * @return The largest index i such that, for every j in the range
	 *         {@code [first..i)}, {@code comp.compare(pos, j)} is {@code false}.
	 */
	...
	/**
	 * Sorts the specified range of elements using the specified swapper and
	...
	public static void mergeSort(final int from, final int to, final IntComparator c, final Swapper swapper) {
		/*
		 * We retain the same method signature as quickSort. Given only a comparator and
		 * swapper we do not know how to copy and move elements from/to temporary
		 * arrays. Hence, in contrast to the JDK mergesorts this is an "in-place"
		 * mergesort, i.e. does not allocate any temporary arrays. A non-inplace
		 * mergesort would perhaps be faster in most cases, but would require
		...
		protected void compute() {
			...
			if (s > 1 && t > 1)
				invokeAll(new ForkJoinGenericQuickSort(from, from + s, comp, swapper),
						new ForkJoinGenericQuickSort(to - t, to, comp, swapper));
			else if (s > 1)
				invokeAll(new ForkJoinGenericQuickSort(from, from + s, comp, swapper));
			else
	...
	/**
	 * Sorts the specified range of elements using the specified swapper and
	 * according to the order induced by the specified comparator using a parallel
	 * quicksort.
	 *
	 * <p>
	 ...
	 *
	 */
	public static void parallelQuickSort(final int from, final int to, final IntComparator comp,
			final Swapper swapper) {
	...
	/**
	 * Sorts the specified range of elements using the specified swapper and
	 * according to the order induced by the specified comparator using parallel
	 * quicksort.
	 *
	 * <p>
	 * The sorting algorithm is a tuned quicksort adapted from Jon L. Bentley and M.
