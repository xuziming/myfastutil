 ...
 * @see HashCommon
 */
public class Reference2ShortOpenHashMap<K> extends AbstractReference2ShortMap<K>
	...
	public Reference2ShortOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2ShortOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ShortOpenHashMap(final Map<? extends K, ? extends Short> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2ShortOpenHashMap(final Map<? extends K, ? extends Short> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ShortOpenHashMap(final Reference2ShortMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2ShortOpenHashMap(final Reference2ShortMap<K> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2ShortOpenHashMap(final K[] k, final short[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2ShortOpenHashMap(final K[] k, final short[] v) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeShortIfAbsent(final K k, final java.util.function.ToIntFunction<? super K> mappingFunction) {
	 ...
	 * the map
	 */
	final class MapEntry implements Reference2ShortMap.Entry<K>, Map.Entry<K, Short> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()))) && ((value[index]) == ((e.getValue()).shortValue()));
		}
	...
	private class MapIterator {
		 ...
		 */
		boolean mustReturnNullKey = Reference2ShortOpenHashMap.this.containsNullKey;
		/**
		...
		public int nextEntry() {
			...
			final K key[] = Reference2ShortOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Reference2ShortOpenHashMap.this.removeShort(wrapped.set(-pos - 1, null));
	...
	private class EntryIterator extends MapIterator implements ObjectIterator<Reference2ShortMap.Entry<K>> {
	...
	private class FastEntryIterator extends MapIterator implements ObjectIterator<Reference2ShortMap.Entry<K>> {
	...
	private final class MapEntrySet extends AbstractObjectSet<Reference2ShortMap.Entry<K>> implements FastEntrySet<K> {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2ShortOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2ShortOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		/** {@inheritDoc} */
		@Override
		public void forEach(final Consumer<? super Reference2ShortMap.Entry<K>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractReference2ShortMap.BasicEntry<K>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (null)))
					consumer.accept(new AbstractReference2ShortMap.BasicEntry<K>(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Reference2ShortMap.Entry<K>> consumer) {
	...
	protected void rehash(final int newN) {
		...
		for (int j = realSize(); j-- != 0;) {
			while (((key[--i]) == (null)))
				;
			if (!((newKey[pos = (it.unimi.dsi.fastutil.HashCommon.mix(System.identityHashCode(key[i])))
					& mask]) == (null)))
	...
	public Reference2ShortOpenHashMap<K> clone() {
		...
		try {
			c = (Reference2ShortOpenHashMap<K>) super.clone();
