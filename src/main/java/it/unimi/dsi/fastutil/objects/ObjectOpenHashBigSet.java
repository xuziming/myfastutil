...
public class ObjectOpenHashBigSet<K> extends AbstractObjectSet<K>
		implements java.io.Serializable, Cloneable, Hash, Size64 {
	...
	public boolean add(final K k) {
		...
		if (((k) == null)) {
			...
			// The starting point.
			if (!((curr = key[base = (int) ((h & mask) >>> BigArrays.SEGMENT_SHIFT)][displ = (int) (h
					& segmentMask)]) == null)) {
				if (((curr).equals(k)))
					return false;
				while (!((curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0))
						& baseMask][displ]) == null))
					if (((curr).equals(k)))
						return false;
			}
	...
	protected final void shiftKeys(long pos) {
		...
		for (;;) {
			...
			for (;;) {
				...
				slot = (it.unimi.dsi.fastutil.HashCommon.mix((long) ((BigArrays.get(key, pos)).hashCode()))) & mask;
	...
	public boolean remove(final Object k) {
		...
		while (true) {
			if (((curr = key[base = (base + ((displ = (displ + 1) & segmentMask) == 0 ? 1 : 0))
					& baseMask][displ]) == null))
				return false;
			if (((curr).equals(k)))
				return removeEntry(base, displ);
	...
	public K get(final Object k) {
		if (k == null)
			return null; // This is correct independently of the value of containsNull
		K curr;
		final K[][] key = this.key;
		final long h = (it.unimi.dsi.fastutil.HashCommon.mix((long) ((k).hashCode())));
		int displ, base;
