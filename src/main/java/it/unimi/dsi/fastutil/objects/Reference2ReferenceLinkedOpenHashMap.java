 ...
 * @see HashCommon
 */
public class Reference2ReferenceLinkedOpenHashMap<K, V> extends AbstractReference2ReferenceSortedMap<K, V>
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Reference2ReferenceMap.Entry<K, V>>
		...
		@Override
		public ObjectSortedSet<Reference2ReferenceMap.Entry<K, V>> subSet(
				Reference2ReferenceMap.Entry<K, V> fromElement, Reference2ReferenceMap.Entry<K, V> toElement) {
			throw new UnsupportedOperationException();
		}

		@Override
		public ObjectSortedSet<Reference2ReferenceMap.Entry<K, V>> headSet(
				Reference2ReferenceMap.Entry<K, V> toElement) {
			throw new UnsupportedOperationException();
		}

		@Override
		public ObjectSortedSet<Reference2ReferenceMap.Entry<K, V>> tailSet(
				Reference2ReferenceMap.Entry<K, V> fromElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2ReferenceLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2ReferenceLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2ReferenceMap.Entry<K, V>> iterator(
				final Reference2ReferenceMap.Entry<K, V> from) {
		...
		public void forEach(final Consumer<? super Reference2ReferenceMap.Entry<K, V>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractReference2ReferenceMap.BasicEntry<K, V>(key[curr], value[curr]));
			}
		}
	...
	@Override
	public FastSortedEntrySet<K, V> reference2ReferenceEntrySet() {
		if (entries == null)
	...
	public Reference2ReferenceLinkedOpenHashMap<K, V> clone() {
		...
		try {
			c = (Reference2ReferenceLinkedOpenHashMap<K, V>) super.clone();
