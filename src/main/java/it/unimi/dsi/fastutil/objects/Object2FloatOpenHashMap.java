...
public class Object2FloatOpenHashMap<K> extends AbstractObject2FloatMap<K>
	...
	public Object2FloatOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Object2FloatOpenHashMap(final int expected) {
	...
	public boolean remove(final Object k, final float v) {
		...
		if (((curr = key[pos = (it.unimi.dsi.fastutil.HashCommon.mix((k).hashCode())) & mask]) == null))
			return false;
		if (((k).equals(curr)) && (Float.floatToIntBits(v) == Float.floatToIntBits(value[pos]))) {
			removeEntry(pos);
			return true;
		}
		while (true) {
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final K key[] = Object2FloatOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Object2FloatOpenHashMap.this.removeFloat(wrapped.set(-pos - 1, null));
