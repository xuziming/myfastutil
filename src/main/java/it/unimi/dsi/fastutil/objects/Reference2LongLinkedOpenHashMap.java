 ...
 * @see HashCommon
 */
public class Reference2LongLinkedOpenHashMap<K> extends AbstractReference2LongSortedMap<K>
	...
	public Reference2LongLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2LongLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2LongLinkedOpenHashMap(final Map<? extends K, ? extends Long> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2LongLinkedOpenHashMap(final Map<? extends K, ? extends Long> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2LongLinkedOpenHashMap(final Reference2LongMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2LongLinkedOpenHashMap(final Reference2LongMap<K> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2LongLinkedOpenHashMap(final K[] k, final long[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2LongLinkedOpenHashMap(final K[] k, final long[] v) {
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				K curr;
				final K[] key = Reference2LongLinkedOpenHashMap.this.key;
	...
	private class EntryIterator extends MapIterator implements ObjectListIterator<Reference2LongMap.Entry<K>> {
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Reference2LongMap.Entry<K>> {
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Reference2LongMap.Entry<K>>
		...
		@Override
		public ObjectSortedSet<Reference2LongMap.Entry<K>> subSet(Reference2LongMap.Entry<K> fromElement,
				Reference2LongMap.Entry<K> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2LongLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2LongLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2LongMap.Entry<K>> iterator(final Reference2LongMap.Entry<K> from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2LongMap.Entry<K>> fastIterator(final Reference2LongMap.Entry<K> from) {
		...
		public void forEach(final Consumer<? super Reference2LongMap.Entry<K>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractReference2LongMap.BasicEntry<K>(key[curr], value[curr]));
			}
		}
	...
	@Override
	public FastSortedEntrySet<K> reference2LongEntrySet() {
		if (entries == null)
	...
	public Reference2LongLinkedOpenHashMap<K> clone() {
		...
		try {
			c = (Reference2LongLinkedOpenHashMap<K>) super.clone();
