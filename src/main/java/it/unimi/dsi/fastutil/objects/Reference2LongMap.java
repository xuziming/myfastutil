...
public interface Reference2LongMap<K> extends Reference2LongFunction<K>, Map<K, Long> {
	 ...
	 */
	interface FastEntrySet<K> extends ObjectSet<Reference2LongMap.Entry<K>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Reference2LongMap.Entry<K>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default long computeLongIfAbsentPartial(final K key, final Reference2LongFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final long v = getLong(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default long computeLongIfPresent(final K key,
	 ...
	 * @since 8.0.0
	 */
	default long computeLong(final K key,
			final java.util.function.BiFunction<? super K, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = getLong(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default long mergeLong(final K key, final long value,
	...
	@Deprecated
	@Override
	default Long merge(final K key, final Long value,
			final java.util.function.BiFunction<? super Long, ? super Long, ? extends Long> remappingFunction) {
