 ...
 *
 */
public class Object2ObjectRBTreeMap<K, V> extends AbstractObject2ObjectSortedMap<K, V>
	 ...
	 */
	public Object2ObjectRBTreeMap(final Comparator<? super K> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Object2ObjectRBTreeMap(final Map<? extends K, ? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Object2ObjectRBTreeMap(final SortedMap<K, V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Object2ObjectRBTreeMap(final Object2ObjectMap<? extends K, ? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Object2ObjectRBTreeMap(final Object2ObjectSortedMap<K, V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Object2ObjectRBTreeMap(final K[] k, final V v[], final Comparator<? super K> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Object2ObjectRBTreeMap(final K[] k, final V v[]) {
		this(k, v, null);
	}
	...
	private Entry<K, V> add(final K k) {
		...
		if (tree == null) { // The case of the empty tree is treated separately.
			...
			while (i > 0 && !nodePath[i].black()) {
				if (!dirPath[i - 1]) {
					Entry<K, V> y = nodePath[i - 1].right;
		...
		if (p.black()) {
			for (; i > 0; i--) {
				if (dirPath[i - 1] && !nodePath[i - 1].succ() || !dirPath[i - 1] && !nodePath[i - 1].pred()) {
					Entry<K, V> x = dirPath[i - 1] ? nodePath[i - 1].right : nodePath[i - 1].left;
					if (!x.black()) {
						x.black(true);
						break;
					}
				...
				if (!dirPath[i - 1]) {
					Entry<K, V> w = nodePath[i - 1].right;
					if (!w.black()) {
						w.black(true);
						nodePath[i - 1].black(false);
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Object2ObjectRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Object2ObjectMap.Entry<K, V>> object2ObjectEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Object2ObjectMap.Entry<K, V>>() {
				final Comparator<? super Object2ObjectMap.Entry<K, V>> comparator = (Comparator<Object2ObjectMap.Entry<K, V>>) (
						x, y) -> Object2ObjectRBTreeMap.this.actualComparator.compare(x.getKey(), y.getKey());
				...
				@Override
				public ObjectBidirectionalIterator<Object2ObjectMap.Entry<K, V>> iterator(
						final Object2ObjectMap.Entry<K, V> from) {
	...
	private final class Submap extends AbstractObject2ObjectSortedMap<K, V> implements java.io.Serializable {
		...
		final boolean in(final K k) {
			return (bottom || Object2ObjectRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Object2ObjectRBTreeMap.this.compare(k, to) < 0);
		}
				...
				entries = new AbstractObjectSortedSet<Object2ObjectMap.Entry<K, V>>() {
					...
					public boolean contains(final Object o) {
						if (!(o instanceof Map.Entry))
							return false;
						final Map.Entry<?, ?> e = (Map.Entry<?, ?>) o;
						final Object2ObjectRBTreeMap.Entry<K, V> f = findKey(((K) e.getKey()));
		...
		@SuppressWarnings("unchecked")
		public boolean containsKey(final Object k) {
			return in((K) k) && Object2ObjectRBTreeMap.this.containsKey(k);
		}
		...
		@SuppressWarnings("unchecked")
		public V get(final Object k) {
			final Object2ObjectRBTreeMap.Entry<K, V> e;
			final K kk = (K) k;
		...
		public V put(final K k, final V v) {
			...
			final V oldValue = Object2ObjectRBTreeMap.this.put(k, v);
		...
		public V remove(final Object k) {
			...
			final V oldValue = Object2ObjectRBTreeMap.this.remove(k);
		...
		@Override
		public K firstKey() {
			Object2ObjectRBTreeMap.Entry<K, V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public K lastKey() {
			Object2ObjectRBTreeMap.Entry<K, V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Object2ObjectRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Object2ObjectRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
