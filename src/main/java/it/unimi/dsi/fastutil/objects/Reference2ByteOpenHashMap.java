...
public class Reference2ByteOpenHashMap<K> extends AbstractReference2ByteMap<K>
	...
	public Reference2ByteOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2ByteOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ByteOpenHashMap(final Map<? extends K, ? extends Byte> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2ByteOpenHashMap(final Map<? extends K, ? extends Byte> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ByteOpenHashMap(final Reference2ByteMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2ByteOpenHashMap(final Reference2ByteMap<K> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2ByteOpenHashMap(final K[] k, final byte[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2ByteOpenHashMap(final K[] k, final byte[] v) {
	...
	final class MapEntry implements Reference2ByteMap.Entry<K>, Map.Entry<K, Byte> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()))) && ((value[index]) == ((e.getValue()).byteValue()));
		}
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final K key[] = Reference2ByteOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Reference2ByteOpenHashMap.this.removeByte(wrapped.set(-pos - 1, null));
	...
	private final class MapEntrySet extends AbstractObjectSet<Reference2ByteMap.Entry<K>> implements FastEntrySet<K> {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2ByteOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2ByteOpenHashMap.this.key;
			int pos;
			// The starting point.
