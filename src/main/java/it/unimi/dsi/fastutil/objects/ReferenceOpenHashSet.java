 ...
 * @see HashCommon
 */
public class ReferenceOpenHashSet<K> extends AbstractReferenceSet<K> implements java.io.Serializable, Cloneable, Hash {
	...
	private class SetIterator implements ObjectIterator<K> {
		...
		public void remove() {
			...
			if (last == n) {
				...
				ReferenceOpenHashSet.this.key[n] = (null);
			} else if (pos >= 0)
				shiftKeys(last);
			else {
				// We're removing wrapped entries.
				ReferenceOpenHashSet.this.remove(wrapped.set(-pos - 1, null));
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = size, pos; i-- != 0;) {
			...
			if (((k) == (null))) {
				...
				if (!((key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(System.identityHashCode(k))) & mask]) == (null)))
