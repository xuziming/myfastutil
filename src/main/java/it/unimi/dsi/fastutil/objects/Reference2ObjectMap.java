...
public interface Reference2ObjectMap<K, V> extends Reference2ObjectFunction<K, V>, Map<K, V> {
	 ...
	 */
	interface FastEntrySet<K, V> extends ObjectSet<Reference2ObjectMap.Entry<K, V>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Reference2ObjectMap.Entry<K, V>> consumer) {
			forEach(consumer);
		}
	}
