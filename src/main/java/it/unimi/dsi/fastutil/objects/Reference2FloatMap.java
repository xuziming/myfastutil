...
public interface Reference2FloatMap<K> extends Reference2FloatFunction<K>, Map<K, Float> {
	 ...
	 */
	interface FastEntrySet<K> extends ObjectSet<Reference2FloatMap.Entry<K>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Reference2FloatMap.Entry<K>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default float computeFloatIfAbsent(final K key,
	 ...
	 * @since 8.0.0
	 */
	default float computeFloatIfAbsentPartial(final K key, final Reference2FloatFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = getFloat(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default float computeFloatIfPresent(final K key,
	 ...
	 * @since 8.0.0
	 */
	default float computeFloat(final K key,
			final java.util.function.BiFunction<? super K, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = getFloat(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default float mergeFloat(final K key, final float value,
	...
	@Deprecated
	@Override
	default Float merge(final K key, final Float value,
			final java.util.function.BiFunction<? super Float, ? super Float, ? extends Float> remappingFunction) {
