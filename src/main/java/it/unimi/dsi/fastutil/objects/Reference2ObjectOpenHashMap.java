 ...
 * @see HashCommon
 */
public class Reference2ObjectOpenHashMap<K, V> extends AbstractReference2ObjectMap<K, V>
	...
	public Reference2ObjectOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2ObjectOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ObjectOpenHashMap(final Map<? extends K, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2ObjectOpenHashMap(final Map<? extends K, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ObjectOpenHashMap(final Reference2ObjectMap<K, V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2ObjectOpenHashMap(final Reference2ObjectMap<K, V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2ObjectOpenHashMap(final K[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2ObjectOpenHashMap(final K[] k, final V[] v) {
	...
	final class MapEntry implements Reference2ObjectMap.Entry<K, V>, Map.Entry<K, V> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()))) && java.util.Objects.equals(value[index], (e.getValue()));
		}

		@Override
		public int hashCode() {
			return (System.identityHashCode(key[index])) ^ ((value[index]) == null ? 0 : (value[index]).hashCode());
		}
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final K key[] = Reference2ObjectOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Reference2ObjectOpenHashMap.this.remove(wrapped.set(-pos - 1, null));
	...
	private final class MapEntrySet extends AbstractObjectSet<Reference2ObjectMap.Entry<K, V>>
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2ObjectOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			K curr;
			final K[] key = Reference2ObjectOpenHashMap.this.key;
			int pos;
			// The starting point.
