 ...
 * instance of this class was created {@linkplain #wrap(Object[],int) by
 * wrapping}, backing-array reallocations will be performed using reflection, so
 * that {@link #elements()} can return an array of the same type of the original
 * array: the comments about efficiency made in
 * {@link it.unimi.dsi.fastutil.objects.ObjectArrays} apply here. Moreover, you
 * must take into consideration that assignment to an array not of type
 * {@code Object[]} is slower due to type checking.
 *
 * <p>
...
public class ReferenceArrayList<K> extends AbstractReferenceList<K>
	...
	/**
	 * Whether the backing array was passed to {@code wrap()}. In this case, we must
	 * reallocate with the same type of array.
	 */
	 ...
	 */
	protected ReferenceArrayList(final K a[], boolean dummy) {
		this.a = a;
		this.wrapped = true;
	}
	 ...
	 * array will be of type {@link Object Object[]} (in spite of the declared
	 * return type).
	 *
	 * <p>
	 * <strong>Warning</strong>: This behaviour may cause (unfathomable) run-time
	 * errors if a method expects an array actually of type {@code K[]}, but this
	 * methods returns an array of type {@link Object Object[]}.
	 *
	...
	public static <K> ReferenceArrayList<K> wrap(final K a[], final int length) {
		...
		final ReferenceArrayList<K> l = new ReferenceArrayList<>(a, false);
		l.size = length;
		return l;
	}
	...
	private void grow(int capacity) {
		...
		if (wrapped)
			a = ObjectArrays.forceCapacity(a, capacity, size);
		else {
			final Object t[] = new Object[capacity];
			System.arraycopy(a, 0, t, 0, size);
			a = (K[]) t;
	...
	public void size(final int size) {
		if (size > a.length)
			a = ObjectArrays.forceCapacity(a, size, this.size);
		if (size > this.size)
	...
	public ReferenceArrayList<K> clone() {
		ReferenceArrayList<K> c = new ReferenceArrayList<>(size);
