...
public interface Reference2ByteMap<K> extends Reference2ByteFunction<K>, Map<K, Byte> {
	 ...
	 */
	interface FastEntrySet<K> extends ObjectSet<Reference2ByteMap.Entry<K>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Reference2ByteMap.Entry<K>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 */
	interface Entry<K> extends Map.Entry<K, Byte> {
		/**
