...
public interface Reference2ShortMap<K> extends Reference2ShortFunction<K>, Map<K, Short> {
	 ...
	 */
	interface FastEntrySet<K> extends ObjectSet<Reference2ShortMap.Entry<K>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Reference2ShortMap.Entry<K>> consumer) {
			forEach(consumer);
		}
	}
	...
	default short putIfAbsent(final K key, final short value) {
		final short v = getShort(key), drv = defaultReturnValue();
	...
	default boolean remove(final Object key, final short value) {
		final short curValue = getShort(key);
	 ...
	 * @since 8.0.0
	 */
	default short computeShortIfAbsent(final K key, final java.util.function.ToIntFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final short v = getShort(key);
	 ...
	 * @since 8.0.0
	 */
	default short computeShortIfPresent(final K key,
			final java.util.function.BiFunction<? super K, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = getShort(key), drv = defaultReturnValue();
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Short newValue = remappingFunction.apply((key), Short.valueOf(oldValue));
		if (newValue == null) {
			removeShort(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default short computeShort(final K key,
			final java.util.function.BiFunction<? super K, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = getShort(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Short newValue = remappingFunction.apply((key), contained ? Short.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				removeShort(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default short mergeShort(final K key, final short value,
	...
	@Deprecated
	@Override
	default Short merge(final K key, final Short value,
			final java.util.function.BiFunction<? super Short, ? super Short, ? extends Short> remappingFunction) {
	 ...
	 */
	interface Entry<K> extends Map.Entry<K, Short> {
		/**
