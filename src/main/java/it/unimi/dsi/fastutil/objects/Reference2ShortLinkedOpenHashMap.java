 ...
 * @see HashCommon
 */
public class Reference2ShortLinkedOpenHashMap<K> extends AbstractReference2ShortSortedMap<K>
	...
	public Reference2ShortLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2ShortLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ShortLinkedOpenHashMap(final Map<? extends K, ? extends Short> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2ShortLinkedOpenHashMap(final Map<? extends K, ? extends Short> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ShortLinkedOpenHashMap(final Reference2ShortMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2ShortLinkedOpenHashMap(final Reference2ShortMap<K> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2ShortLinkedOpenHashMap(final K[] k, final short[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2ShortLinkedOpenHashMap(final K[] k, final short[] v) {
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				K curr;
				final K[] key = Reference2ShortLinkedOpenHashMap.this.key;
	...
	private class EntryIterator extends MapIterator implements ObjectListIterator<Reference2ShortMap.Entry<K>> {
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Reference2ShortMap.Entry<K>> {
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Reference2ShortMap.Entry<K>>
		...
		@Override
		public ObjectSortedSet<Reference2ShortMap.Entry<K>> subSet(Reference2ShortMap.Entry<K> fromElement,
				Reference2ShortMap.Entry<K> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2ShortLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2ShortLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2ShortMap.Entry<K>> iterator(final Reference2ShortMap.Entry<K> from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2ShortMap.Entry<K>> fastIterator(final Reference2ShortMap.Entry<K> from) {
		...
		public void forEach(final Consumer<? super Reference2ShortMap.Entry<K>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractReference2ShortMap.BasicEntry<K>(key[curr], value[curr]));
			}
		}
