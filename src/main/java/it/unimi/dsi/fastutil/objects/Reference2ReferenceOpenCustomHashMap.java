...
public class Reference2ReferenceOpenCustomHashMap<K, V> extends AbstractReference2ReferenceMap<K, V>
	...
	public Reference2ReferenceOpenCustomHashMap(final int expected, final float f, final Strategy<? super K> strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Reference2ReferenceOpenCustomHashMap(final int expected, final Strategy<? super K> strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Reference2ReferenceOpenCustomHashMap(final Strategy<? super K> strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Reference2ReferenceOpenCustomHashMap(final Map<? extends K, ? extends V> m, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Reference2ReferenceOpenCustomHashMap(final Map<? extends K, ? extends V> m,
			final Strategy<? super K> strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Reference2ReferenceOpenCustomHashMap(final Reference2ReferenceMap<K, V> m, final float f,
			final Strategy<? super K> strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Reference2ReferenceOpenCustomHashMap(final Reference2ReferenceMap<K, V> m,
			final Strategy<? super K> strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2ReferenceOpenCustomHashMap(final K[] k, final V[] v, final float f,
			final Strategy<? super K> strategy) {
	 ...
	 *                                  lengths.
	 */
	public Reference2ReferenceOpenCustomHashMap(final K[] k, final V[] v, final Strategy<? super K> strategy) {
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final K key[] = Reference2ReferenceOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Reference2ReferenceOpenCustomHashMap.this.remove(wrapped.set(-pos - 1, null));
	...
	private final class MapEntrySet extends AbstractObjectSet<Reference2ReferenceMap.Entry<K, V>>
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (null))))
				return Reference2ReferenceOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2ReferenceOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Reference2ReferenceMap.Entry<K, V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractReference2ReferenceMap.BasicEntry<K, V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (null)))
					consumer.accept(new AbstractReference2ReferenceMap.BasicEntry<K, V>(key[pos], value[pos]));
		}
