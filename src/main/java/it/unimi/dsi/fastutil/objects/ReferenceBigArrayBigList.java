 ...
 */
public class ReferenceBigArrayBigList<K> extends AbstractReferenceBigList<K>
	 ...
	 */
	protected ReferenceBigArrayBigList(final K a[][], boolean dummy) {
		this.a = a;
		this.wrapped = true;
	}
	...
	public ReferenceBigArrayBigList(final long capacity) {
		if (capacity < 0)
	 ...
	 */
	public ReferenceBigArrayBigList(final ReferenceCollection<? extends K> c) {
		this(c.size());
	 ...
	 */
	public ReferenceBigArrayBigList(final ReferenceBigList<? extends K> l) {
		this(l.size64());
	 ...
	 */
	public ReferenceBigArrayBigList(final K a[][]) {
	 ...
	 */
	public ReferenceBigArrayBigList(final K a[][], final long offset, final long length) {
		this(length);
	 ...
	 */
	public ReferenceBigArrayBigList(final Iterator<? extends K> i) {
		this();
		while (i.hasNext())
			this.add((i.next()));
	}
	 ...
	 *          list.
	 */
	public ReferenceBigArrayBigList(final ObjectIterator<? extends K> i) {
		this();
		while (i.hasNext())
			this.add(i.next());
	}
	 ...
	 */
	public static <K> ReferenceBigArrayBigList<K> wrap(final K a[][], final long length) {
		if (length > BigArrays.length(a))
			throw new IllegalArgumentException("The specified length (" + length + ") is greater than the array size ("
					+ BigArrays.length(a) + ")");
		final ReferenceBigArrayBigList<K> l = new ReferenceBigArrayBigList<>(a, false);
		l.size = length;
		return l;
	}
	 ...
	 */
	public static <K> ReferenceBigArrayBigList<K> wrap(final K a[][]) {
	...
	public ObjectBigListIterator<K> listIterator(final long index) {
		...
		return new ObjectBigListIterator<K>() {
			...
			@Override
			public void add(K k) {
				ReferenceBigArrayBigList.this.add(pos++, k);
				last = -1;
			}
			...
			public void set(K k) {
				...
				ReferenceBigArrayBigList.this.set(last, k);
			}
			...
			public void remove() {
				if (last == -1)
					throw new IllegalStateException();
				ReferenceBigArrayBigList.this.remove(last);
				/*
	...
	public ReferenceBigArrayBigList<K> clone() {
		ReferenceBigArrayBigList<K> c = new ReferenceBigArrayBigList<>(size);
	 ...
	 *         big-array list.
	 */
	public boolean equals(final ReferenceBigArrayBigList<K> l) {
		if (l == this)
			return true;
		long s = size64();
		if (s != l.size64())
