...
public abstract class AbstractReferenceBigList<K> extends AbstractReferenceCollection<K>
	...
	public ObjectBigListIterator<K> listIterator(final long index) {
		...
		return new ObjectBigListIterator<K>() {
			...
			public boolean hasNext() {
				return pos < AbstractReferenceBigList.this.size64();
			}
			...
			public K next() {
				...
				return AbstractReferenceBigList.this.get(last = pos++);
			}
			...
			@Override
			public void add(K k) {
				AbstractReferenceBigList.this.add(pos++, k);
				last = -1;
			}
			...
			public void set(K k) {
				...
				AbstractReferenceBigList.this.set(last, k);
			}
			...
			public void remove() {
				if (last == -1)
					throw new IllegalStateException();
				AbstractReferenceBigList.this.remove(last);
				/*
	...
	public static class ReferenceSubList<K> extends AbstractReferenceBigList<K> implements java.io.Serializable {
