...
public class ReferenceOpenHashBigSet<K> extends AbstractReferenceSet<K>
	...
	protected final void shiftKeys(long pos) {
		...
		for (;;) {
			...
			for (;;) {
				...
				slot = (it.unimi.dsi.fastutil.HashCommon.mix((long) (System.identityHashCode(BigArrays.get(key, pos)))))
						& mask;
		...
		private final void shiftKeys(long pos) {
			...
			for (;;) {
				...
				if (pos < last) { // Wrapped entry.
					if (wrapped == null)
						wrapped = new ReferenceArrayList<>();
	...
	protected void rehash(final long newN) {
		...
		for (long i = realSize(); i-- != 0;) {
			...
			// The starting point.
			if (!((newKey[b = (int) ((h & mask) >>> BigArrays.SEGMENT_SHIFT)][d = (int) (h
					& newSegmentMask)]) == (null)))
