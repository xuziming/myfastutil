...
public class ObjectArrayIndirectPriorityQueue<K> implements IndirectPriorityQueue<K> {
	...
	private int findFirst() {
		...
		if (c == null)
			while (i-- != 0) {
				if ((((Comparable<K>) (refArray[array[i]])).compareTo(first) < 0))
	...
	private int findLast() {
		...
		if (c == null) {
			while (i-- != 0)
				if ((((Comparable<K>) (last)).compareTo(refArray[array[i]]) < 0))
	...
	public void enqueue(int x) {
		...
		if (firstIndexValid) {
			if (c == null) {
				if ((((Comparable<K>) (refArray[x])).compareTo(refArray[array[firstIndex]]) < 0))
					firstIndex = size;
	...
	public int front(int[] a) {
		...
		int i = size, c = 0;
		while (i-- != 0)
			if (((top).equals(refArray[array[i]])))
				a[c++] = array[i];
		return c;
	}
