...
public class ObjectHeapIndirectPriorityQueue<K> extends ObjectHeapSemiIndirectPriorityQueue<K> {
	...
	public ObjectHeapIndirectPriorityQueue(final K[] refArray, final int[] a, final int size,
		...
		ObjectIndirectHeaps.makeHeap(refArray, a, inv, size, c);
	}
	 ...
	 * @param size     the number of elements to be included in the queue.
	 */
	public ObjectHeapIndirectPriorityQueue(final K[] refArray, final int[] a, int size) {
	...
	public void enqueue(final int x) {
		...
		ObjectIndirectHeaps.upHeap(refArray, heap, inv, size, size - 1, c);
	}
	...
	public int dequeue() {
		...
		inv[result] = -1;
		if (size != 0)
			ObjectIndirectHeaps.downHeap(refArray, heap, inv, size, 0, c);
		return result;
	}
	...
	public void changed(final int index) {
		...
		final int newPos = ObjectIndirectHeaps.upHeap(refArray, heap, inv, size, pos, c);
		ObjectIndirectHeaps.downHeap(refArray, heap, inv, size, newPos, c);
	}

	/** Rebuilds this queue in a bottom-up fashion (in linear time). */
	@Override
	public void allChanged() {
		ObjectIndirectHeaps.makeHeap(refArray, heap, inv, size, c);
	}
	...
	public boolean remove(final int index) {
		...
		if (result < --size) {
			...
			final int newPos = ObjectIndirectHeaps.upHeap(refArray, heap, inv, size, result, c);
			ObjectIndirectHeaps.downHeap(refArray, heap, inv, size, newPos, c);
		}
		return true;
	}
