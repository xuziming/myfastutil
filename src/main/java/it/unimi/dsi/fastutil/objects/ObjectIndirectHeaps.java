...
public final class ObjectIndirectHeaps {
	...
	public static <K> int downHeap(final K[] refArray, final int[] heap, final int[] inv, final int size, int i,
		...
		if (c == null)
			while ((child = (i << 1) + 1) < size) {
				...
				if (right < size && (((Comparable<K>) (refArray[heap[right]])).compareTo(refArray[t]) < 0))
					t = heap[child = right];
				if ((((Comparable<K>) (E)).compareTo(refArray[t]) <= 0))
					break;
				heap[i] = t;
				inv[heap[i]] = i;
				i = child;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <K> int upHeap(final K[] refArray, final int[] heap, final int[] inv, final int size, int i,
			final Comparator<K> c) {
		assert i < size;
		final int e = heap[i];
		final K E = refArray[e];
		if (c == null)
			while (i != 0) {
				...
				if ((((Comparable<K>) (refArray[t])).compareTo(E) <= 0))
					break;
				heap[i] = t;
				inv[heap[i]] = i;
				i = parent;
	 ...
	 *                 order.
	 */
	public static <K> void makeHeap(final K[] refArray, final int offset, final int length, final int[] heap,
			final int[] inv, final Comparator<K> c) {
	 ...
	 *                 order.
	 */
	public static <K> void makeHeap(final K[] refArray, final int[] heap, final int[] inv, final int size,
			final Comparator<K> c) {
		int i = size >>> 1;
		while (i-- != 0)
