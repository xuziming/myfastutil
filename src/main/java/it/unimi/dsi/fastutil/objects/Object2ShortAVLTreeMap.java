 ...
 * iterator}.
 */
public class Object2ShortAVLTreeMap<K> extends AbstractObject2ShortSortedMap<K>
	...
	private static final class Entry<K> extends AbstractObject2ShortMap.BasicEntry<K> implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return java.util.Objects.equals(key, (e.getKey())) && ((value) == ((e.getValue()).shortValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Object2ShortAVLTreeMap.this.removeShort(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Object2ShortMap.Entry<K>> object2ShortEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Object2ShortMap.Entry<K>>() {
				final Comparator<? super Object2ShortMap.Entry<K>> comparator = (Comparator<Object2ShortMap.Entry<K>>) (
						x, y) -> Object2ShortAVLTreeMap.this.actualComparator.compare(x.getKey(), y.getKey());
				...
				@Override
				public ObjectBidirectionalIterator<Object2ShortMap.Entry<K>> iterator(
						final Object2ShortMap.Entry<K> from) {
				...
				@Override
				public ObjectSortedSet<Object2ShortMap.Entry<K>> subSet(Object2ShortMap.Entry<K> from,
						Object2ShortMap.Entry<K> to) {
					return subMap(from.getKey(), to.getKey()).object2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Object2ShortMap.Entry<K>> headSet(Object2ShortMap.Entry<K> to) {
					return headMap(to.getKey()).object2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Object2ShortMap.Entry<K>> tailSet(Object2ShortMap.Entry<K> from) {
					return tailMap(from.getKey()).object2ShortEntrySet();
				}
			};
		return entries;
	}
	...
	private final class Submap extends AbstractObject2ShortSortedMap<K> implements java.io.Serializable {
		...
		final boolean in(final K k) {
			return (bottom || Object2ShortAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Object2ShortAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Object2ShortMap.Entry<K>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Object2ShortAVLTreeMap.Entry<K> f = findKey(((K) e.getKey()));
		...
		@SuppressWarnings("unchecked")
		public boolean containsKey(final Object k) {
			return in((K) k) && Object2ShortAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public K firstKey() {
			Object2ShortAVLTreeMap.Entry<K> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public K lastKey() {
			Object2ShortAVLTreeMap.Entry<K> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Object2ShortAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Object2ShortAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry<K> readTree(final java.io.ObjectInputStream s, final int n, final Entry<K> pred, final Entry<K> succ)
		...
		if (n == 1) {
			final Entry<K> top = new Entry<>((K) s.readObject(), s.readShort());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
		if (n == 2) {
			 ...
			 */
			final Entry<K> top = new Entry<>((K) s.readObject(), s.readShort());
			top.right(new Entry<>((K) s.readObject(), s.readShort()));
			top.right.pred(top);
			top.balance(1);
			top.pred(pred);
			top.right.succ(succ);
