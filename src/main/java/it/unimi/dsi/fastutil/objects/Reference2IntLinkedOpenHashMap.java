 ...
 * @see HashCommon
 */
public class Reference2IntLinkedOpenHashMap<K> extends AbstractReference2IntSortedMap<K>
	...
	public Reference2IntLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2IntLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2IntLinkedOpenHashMap(final Map<? extends K, ? extends Integer> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2IntLinkedOpenHashMap(final Map<? extends K, ? extends Integer> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2IntLinkedOpenHashMap(final Reference2IntMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2IntLinkedOpenHashMap(final Reference2IntMap<K> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2IntLinkedOpenHashMap(final K[] k, final int[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2IntLinkedOpenHashMap(final K[] k, final int[] v) {
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				K curr;
				final K[] key = Reference2IntLinkedOpenHashMap.this.key;
	...
	private class EntryIterator extends MapIterator implements ObjectListIterator<Reference2IntMap.Entry<K>> {
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Reference2IntMap.Entry<K>> {
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Reference2IntMap.Entry<K>>
		...
		@Override
		public ObjectSortedSet<Reference2IntMap.Entry<K>> subSet(Reference2IntMap.Entry<K> fromElement,
				Reference2IntMap.Entry<K> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2IntLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2IntLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2IntMap.Entry<K>> iterator(final Reference2IntMap.Entry<K> from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2IntMap.Entry<K>> fastIterator(final Reference2IntMap.Entry<K> from) {
		...
		public void forEach(final Consumer<? super Reference2IntMap.Entry<K>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractReference2IntMap.BasicEntry<K>(key[curr], value[curr]));
			}
		}
	...
	@Override
	public FastSortedEntrySet<K> reference2IntEntrySet() {
		if (entries == null)
	...
	public Reference2IntLinkedOpenHashMap<K> clone() {
		...
		try {
			c = (Reference2IntLinkedOpenHashMap<K>) super.clone();
