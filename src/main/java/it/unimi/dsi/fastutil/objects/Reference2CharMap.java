...
public interface Reference2CharMap<K> extends Reference2CharFunction<K>, Map<K, Character> {
	 ...
	 */
	interface FastEntrySet<K> extends ObjectSet<Reference2CharMap.Entry<K>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Reference2CharMap.Entry<K>> consumer) {
			forEach(consumer);
		}
	}
	...
	default char putIfAbsent(final K key, final char value) {
		final char v = getChar(key), drv = defaultReturnValue();
	...
	default boolean remove(final Object key, final char value) {
		final char curValue = getChar(key);
	 ...
	 * @since 8.0.0
	 */
	default char computeCharIfAbsent(final K key, final java.util.function.ToIntFunction<? super K> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final char v = getChar(key);
	 ...
	 * @since 8.0.0
	 */
	default char computeCharIfPresent(final K key,
			final java.util.function.BiFunction<? super K, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final char oldValue = getChar(key), drv = defaultReturnValue();
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Character newValue = remappingFunction.apply((key), Character.valueOf(oldValue));
		if (newValue == null) {
			removeChar(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default char computeChar(final K key,
			final java.util.function.BiFunction<? super K, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final char oldValue = getChar(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Character newValue = remappingFunction.apply((key), contained ? Character.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				removeChar(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default char mergeChar(final K key, final char value,
	...
	@Deprecated
	@Override
	default Character merge(final K key, final Character value,
			final java.util.function.BiFunction<? super Character, ? super Character, ? extends Character> remappingFunction) {
	 ...
	 */
	interface Entry<K> extends Map.Entry<K, Character> {
		/**
