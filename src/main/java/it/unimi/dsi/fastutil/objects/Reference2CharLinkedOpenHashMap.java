...
public class Reference2CharLinkedOpenHashMap<K> extends AbstractReference2CharSortedMap<K>
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Reference2CharMap.Entry<K>>
		...
		@Override
		public ObjectSortedSet<Reference2CharMap.Entry<K>> subSet(Reference2CharMap.Entry<K> fromElement,
				Reference2CharMap.Entry<K> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2CharLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2CharLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Reference2CharMap.Entry<K>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractReference2CharMap.BasicEntry<K>(key[curr], value[curr]));
			}
		}
	...
	@Override
	public FastSortedEntrySet<K> reference2CharEntrySet() {
		if (entries == null)
	...
	public Reference2CharLinkedOpenHashMap<K> clone() {
		...
		try {
			c = (Reference2CharLinkedOpenHashMap<K>) super.clone();
