 ...
 * @see HashCommon
 */
public class Reference2ObjectLinkedOpenHashMap<K, V> extends AbstractReference2ObjectSortedMap<K, V>
	...
	public Reference2ObjectLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2ObjectLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ObjectLinkedOpenHashMap(final Map<? extends K, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2ObjectLinkedOpenHashMap(final Map<? extends K, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2ObjectLinkedOpenHashMap(final Reference2ObjectMap<K, V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2ObjectLinkedOpenHashMap(final Reference2ObjectMap<K, V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2ObjectLinkedOpenHashMap(final K[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2ObjectLinkedOpenHashMap(final K[] k, final V[] v) {
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				K curr;
				final K[] key = Reference2ObjectLinkedOpenHashMap.this.key;
	...
	private class EntryIterator extends MapIterator implements ObjectListIterator<Reference2ObjectMap.Entry<K, V>> {
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Reference2ObjectMap.Entry<K, V>> {
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Reference2ObjectMap.Entry<K, V>>
		...
		@Override
		public ObjectSortedSet<Reference2ObjectMap.Entry<K, V>> subSet(Reference2ObjectMap.Entry<K, V> fromElement,
				Reference2ObjectMap.Entry<K, V> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2ObjectLinkedOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			K curr;
			final K[] key = Reference2ObjectLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Reference2ObjectMap.Entry<K, V>> iterator(
				final Reference2ObjectMap.Entry<K, V> from) {
		...
		public void forEach(final Consumer<? super Reference2ObjectMap.Entry<K, V>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractReference2ObjectMap.BasicEntry<K, V>(key[curr], value[curr]));
			}
		}
