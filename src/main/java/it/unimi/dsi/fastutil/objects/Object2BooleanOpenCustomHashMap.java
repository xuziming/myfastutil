...
public class Object2BooleanOpenCustomHashMap<K> extends AbstractObject2BooleanMap<K>
	 ...
	 * @param strategy the strategy.
	 */
	public Object2BooleanOpenCustomHashMap(final Map<? extends K, ? extends Boolean> m, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Object2BooleanOpenCustomHashMap(final Map<? extends K, ? extends Boolean> m,
			final Strategy<? super K> strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Object2BooleanOpenCustomHashMap(final Object2BooleanMap<K> m, final float f,
			final Strategy<? super K> strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Object2BooleanOpenCustomHashMap(final K[] k, final boolean[] v, final float f,
			final Strategy<? super K> strategy) {
	...
	private final class MapEntrySet extends AbstractObjectSet<Object2BooleanMap.Entry<K>> implements FastEntrySet<K> {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (null))))
				return Object2BooleanOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Object2BooleanOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Object2BooleanMap.Entry<K>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractObject2BooleanMap.BasicEntry<K>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == null))
					consumer.accept(new AbstractObject2BooleanMap.BasicEntry<K>(key[pos], value[pos]));
		}
