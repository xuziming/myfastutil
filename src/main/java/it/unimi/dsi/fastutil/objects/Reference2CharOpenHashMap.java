...
public class Reference2CharOpenHashMap<K> extends AbstractReference2CharMap<K>
	...
	public Reference2CharOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Reference2CharOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2CharOpenHashMap(final Map<? extends K, ? extends Character> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Reference2CharOpenHashMap(final Map<? extends K, ? extends Character> m) {
	 ...
	 * @param f the load factor.
	 */
	public Reference2CharOpenHashMap(final Reference2CharMap<K> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Reference2CharOpenHashMap(final Reference2CharMap<K> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Reference2CharOpenHashMap(final K[] k, final char[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Reference2CharOpenHashMap(final K[] k, final char[] v) {
	...
	final class MapEntry implements Reference2CharMap.Entry<K>, Map.Entry<K, Character> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()))) && ((value[index]) == ((e.getValue()).charValue()));
		}
	...
	private class MapIterator {
		 ...
		 */
		boolean mustReturnNullKey = Reference2CharOpenHashMap.this.containsNullKey;
		/**
		...
		public int nextEntry() {
			...
			final K key[] = Reference2CharOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Reference2CharOpenHashMap.this.removeChar(wrapped.set(-pos - 1, null));
	...
	private final class MapEntrySet extends AbstractObjectSet<Reference2CharMap.Entry<K>> implements FastEntrySet<K> {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (null)))
				return Reference2CharOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			K curr;
			final K[] key = Reference2CharOpenHashMap.this.key;
			int pos;
			// The starting point.
	...
	public Reference2CharOpenHashMap<K> clone() {
		...
		try {
			c = (Reference2CharOpenHashMap<K>) super.clone();
