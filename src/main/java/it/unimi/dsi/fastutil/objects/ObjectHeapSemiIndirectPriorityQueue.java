...
public class ObjectHeapSemiIndirectPriorityQueue<K> implements IndirectPriorityQueue<K> {
	 ...
	 *                 natural order.
	 */
	public ObjectHeapSemiIndirectPriorityQueue(final K[] refArray, final int[] a, int size,
	...
	public void enqueue(int x) {
		...
		heap[size++] = x;
		ObjectSemiIndirectHeaps.upHeap(refArray, heap, size, size - 1, c);
	}
	...
	public int dequeue() {
		...
		heap[0] = heap[--size];
		if (size != 0)
			ObjectSemiIndirectHeaps.downHeap(refArray, heap, size, 0, c);
		return result;
	}
	...
	public void allChanged() {
		ObjectSemiIndirectHeaps.makeHeap(refArray, heap, size, c);
	}
	...
	public int front(final int[] a) {
		return c == null ? ObjectSemiIndirectHeaps.front(refArray, heap, size, a)
				: ObjectSemiIndirectHeaps.front(refArray, heap, size, a, c);
	}
