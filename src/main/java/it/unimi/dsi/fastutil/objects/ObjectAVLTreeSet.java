...
public class ObjectAVLTreeSet<K> extends AbstractObjectSortedSet<K>
		implements java.io.Serializable, Cloneable, ObjectSortedSet<K> {
	...
	public K get(final Object k) {
		final Entry<K> entry = findKey((K) k);
	...
	private static final class Entry<K> implements Cloneable {
		...
		public boolean equals(final Object o) {
			if (!(o instanceof Entry))
				return false;
			Entry<?> e = (Entry<?>) o;
			return java.util.Objects.equals(key, e.key);
		}
	...
	private class SetIterator implements ObjectListIterator<K> {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			ObjectAVLTreeSet.this.remove(curr.key);
			curr = null;
		}
	}
	 ...
	 */
	private final class Subset extends AbstractObjectSortedSet<K> implements java.io.Serializable, ObjectSortedSet<K> {
		...
		final boolean in(final K k) {
			return (bottom || ObjectAVLTreeSet.this.compare(k, from) >= 0)
					&& (top || ObjectAVLTreeSet.this.compare(k, to) < 0);
		}

		@SuppressWarnings("unchecked")
		@Override
		public boolean contains(final Object k) {
			return in((K) k) && ObjectAVLTreeSet.this.contains(k);
		}
		...
		@Override
		public K first() {
			ObjectAVLTreeSet.Entry<K> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public K last() {
			ObjectAVLTreeSet.Entry<K> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && ObjectAVLTreeSet.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && ObjectAVLTreeSet.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	}
	...
	private Entry<K> readTree(final java.io.ObjectInputStream s, final int n, final Entry<K> pred, final Entry<K> succ)
		...
		if (n == 1) {
			final Entry<K> top = new Entry<>((K) s.readObject());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
		if (n == 2) {
			 ...
			 */
			final Entry<K> top = new Entry<>((K) s.readObject());
			top.right(new Entry<>((K) s.readObject()));
			top.right.pred(top);
			top.balance(1);
			top.pred(pred);
			top.right.succ(succ);
