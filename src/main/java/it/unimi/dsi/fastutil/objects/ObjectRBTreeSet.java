 ...
 */
public class ObjectRBTreeSet<K> extends AbstractObjectSortedSet<K>
	...
	private class SetIterator implements ObjectListIterator<K> {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			ObjectRBTreeSet.this.remove(curr.key);
			curr = null;
		}
	}
	...
	private final class Subset extends AbstractObjectSortedSet<K> implements java.io.Serializable, ObjectSortedSet<K> {
		...
		final boolean in(final K k) {
			return (bottom || ObjectRBTreeSet.this.compare(k, from) >= 0)
					&& (top || ObjectRBTreeSet.this.compare(k, to) < 0);
		}

		@Override
		@SuppressWarnings("unchecked")
		public boolean contains(final Object k) {
			return in((K) k) && ObjectRBTreeSet.this.contains(k);
		}
		...
		@Override
		public K first() {
			ObjectRBTreeSet.Entry<K> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public K last() {
			ObjectRBTreeSet.Entry<K> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && ObjectRBTreeSet.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && ObjectRBTreeSet.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	}
