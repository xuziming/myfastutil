...
public class BooleanBigArrayBigList extends AbstractBooleanBigList
	...
	public boolean removeBoolean(final long index) {
		if (index >= size)
			throw new IndexOutOfBoundsException(
					"Index (" + index + ") is greater than or equal to list size (" + size + ")");
		final boolean old = BigArrays.get(a, index);
		size--;
		if (index != size)
	...
	public boolean set(final long index, final boolean k) {
		if (index >= size)
			throw new IndexOutOfBoundsException(
					"Index (" + index + ") is greater than or equal to list size (" + size + ")");
		boolean old = BigArrays.get(a, index);
		BigArrays.set(a, index, k);
		return old;
	}
	...
	public void size(final long size) {
		...
		if (size > this.size)
			BigArrays.fill(a, this.size, size, (false));
		this.size = size;
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		for (int i = 0; i < size; i++)
			s.writeBoolean(BigArrays.get(a, i));
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = 0; i < size; i++)
			BigArrays.set(a, i, s.readBoolean());
	}
}
