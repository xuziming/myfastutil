...
public final class BooleanArrays {
	...
	public static void quickSort(final boolean[] x, final boolean[] y, final int from, final int to) {
		...
		while (true) {
			int comparison, t;
			while (b <= c && (comparison = (t = (Boolean.compare((x[b]), (v)))) == 0 ? (Boolean.compare((y[b]), (w)))
					: t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			}
			while (c >= b && (comparison = (t = (Boolean.compare((x[c]), (v)))) == 0 ? (Boolean.compare((y[c]), (w)))
					: t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
	...
	protected static class ForkJoinQuickSort2 extends RecursiveAction {
		...
		protected void compute() {
			...
			while (true) {
				int comparison, t;
				while (b <= c
						&& (comparison = (t = (Boolean.compare((x[b]), (v)))) == 0 ? (Boolean.compare((y[b]), (w)))
								: t) <= 0) {
					if (comparison == 0)
						swap(x, y, a++, b);
					b++;
				...
				while (c >= b
						&& (comparison = (t = (Boolean.compare((x[c]), (v)))) == 0 ? (Boolean.compare((y[c]), (w)))
								: t) >= 0) {
					if (comparison == 0)
						swap(x, y, c, d--);
					c--;
