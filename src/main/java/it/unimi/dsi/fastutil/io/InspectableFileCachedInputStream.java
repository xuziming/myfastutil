...
/**
 * A {@linkplain RepositionableStream repositionable}
 * {@link MeasurableInputStream} based on cached data received by a
 * {@link WritableByteChannel} whose first bytes can be inspected directly.
 *
 * <p>
 ...
 * {@link #buffer} is full, the bytes are written to an <em>overflow file</em>.
 *
 * <p>
 * At any time, the stream of bytes written since creation (or since the last
 * {@link #clear()}) are available as a fully implemented
 * {@link MeasurableInputStream} which also implements
 ...
 * {@linkplain #length() length} of the {@link MeasurableInputStream}.
 *
 * <p>
 * The method {@link #close()} makes the {@link MeasurableInputStream} and
 * {@link WritableByteChannel} state-changing methods temporarily throw an
 * {@link IOException}, but does not otherwise modify the state of the stream.
 * You can {@linkplain #reopen() reopen} the stream later, or
 * {@linkplain #clear() clear} it. The method {@link #dispose()} can be used to
 ...
 * wrapping instances of this class with a {@link FastBufferedInputStream}, as
 * reads after the buffer has been exhausted will be performed directly on a
 * {@link RandomAccessFile}.
 */
public class InspectableFileCachedInputStream extends MeasurableInputStream
	...
	/**
	 * The inspection buffer. The first {@link #inspectable} bytes contain the first
	 * part of the input stream. The buffer is available for inspection, but users
	 * should not modify its content.
	 */
	...
	/**
	 * The overflow file used by this stream: it is created at construction time,
	 * and deleted on {@link #close()}.
	 */
	 ...
	 * {@link #inspectable} is equal to {@link #buffer buffer.length}, the length of
	 * the stream is {@link #inspectable} + {@link #writePosition}.
	 */
	 ...
	 * @param overflowFile the directory where the overflow file should be created,
	 *                     or {@code null} for the default temporary directory.
	 */
	public InspectableFileCachedInputStream(final int bufferSize, final File overflowFile) throws IOException {
		...
		if (overflowFile != null)
			...
			(this.overflowFile = File.createTempFile(getClass().getSimpleName(), "overflow")).deleteOnExit();
	...
	/**
	 * Creates a new instance with specified buffer size and default overflow-file
	 * directory.
	 *
	 * @param bufferSize the buffer size, in bytes.
	 */
	...
	/**
	 * Clears the content of this {@link InspectableFileCachedInputStream}, zeroing
	 * the length of the represented stream.
	 */
	 ...
	 * @return the number of bytes appended (i.e., {@link ByteBuffer#remaining()
	 ...
	 *
	 * @param size the new size; the final size is the maximum between the current
	 *             write position (i.e., the length of the represented stream minus
	 *             the length of the inspection buffer) and this value.
	 */
	...
	/**
	 * Disposes this stream, deleting the overflow file. After that, the stream is
	 * unusable.
	 */
	...
	public int read(final byte[] b, int offset, int length) throws IOException {
		...
		if (position < inspectable) {
			/*
			 * The first min(inspectable - readPosition, length) bytes should be taken from
			 * the buffer.
			 */
