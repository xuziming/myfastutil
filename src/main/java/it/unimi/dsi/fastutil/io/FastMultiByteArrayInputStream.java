...
/**
 * Simple, fast and repositionable byte array input stream that multiplexes its
 * content among several arrays.
 *
 * This class is significantly slower than {@link FastByteArrayInputStream}, but
 * it can hold 256 PiB of data. The relevant constructor is
 * {@link #FastMultiByteArrayInputStream(InputStream, long)}, which fetches a
 * stream and loads it into a sequence of byte arrays.
 *
 * @author Sebastiano Vigna
 * @author Paolo Boldi s
 */
...
public class FastMultiByteArrayInputStream extends MeasurableInputStream implements RepositionableStream {
	...
	/**
	 * The array of arrays backing the input stream, plus an additional {@code null}
	 * entry.
	 */
	public byte[][] array;
	...
	/**
	 * Creates a new multi-array input stream loading it from a measurable input
	 * stream.
	 *
	...
	public FastMultiByteArrayInputStream(final InputStream is, long size) throws IOException {
		...
		for (int i = 0; i < array.length - 1; i++) {
			...
			if (BinIO.loadBytes(is, array[i]) != array[i].length)
	...
	/**
	 * Creates a new multi-array input stream sharing the backing arrays of another
	 * multi-array input stream.
	 *
	 ...
	 *
	 * @return the minimum among the number of available bytes and
