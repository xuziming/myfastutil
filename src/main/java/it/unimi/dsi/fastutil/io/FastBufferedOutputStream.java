...
public class FastBufferedOutputStream extends MeasurableOutputStream implements RepositionableStream {
	...
	/**
	 * The number of buffer bytes available starting from {@link #pos} (it must be
	 * always equal to {@code buffer.length - pos}).
	 */
	protected int avail;
	...
	/**
	 * Creates a new fast buffered output stream by wrapping a given output stream
	 * with a given buffer.
	 *
	...
	/**
	 * Creates a new fast buffered ouptut stream by wrapping a given output stream
	 ...
	 *
	 * <p>
	 * Note that this method performs a {@link #flush()} before changing the
	 ...
	 *
	 * <p>
	 * Note that this method performs a {@link #flush()} before detecting the
	 * length.
	 *
	 * @return the length of the underlying output stream.
	 * @throws UnsupportedOperationException if the underlying output stream is not
