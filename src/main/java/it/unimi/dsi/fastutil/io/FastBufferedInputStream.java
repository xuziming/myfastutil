 ...
 * {@linkplain #skip(long) true skipping}, {@linkplain MeasurableStream
 * measurability}, {@linkplain RepositionableStream repositionability} and
 * {@linkplain #readLine(byte[], int, int, EnumSet) line reading} support.
 *
 * <p>
 ...
 * {@link java.io.BufferedInputStream}. The main features follow.
 *
 * <ul>
 * <li>
 * <p>
 ...
 * {@link RepositionableStream} and {@link MeasurableStream} interfaces. An
 * instance of this class will try to cast the underlying byte stream to a
 * {@link RepositionableStream} and to fetch by reflection the
 ...
 * {@link #position(long)} to reposition the stream. Much in the same way, an
 * instance of this class will try to cast the the underlying byte stream to a
 * {@link MeasurableStream}, and if this operation is successful, or if a
 * {@link java.nio.channels.FileChannel} can be detected, then
 * {@link #position()} and {@link #length()} will work as expected.
 *
 *
 * <li>
 * <p>
 ...
 * (see <a href="http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6222822">bug
 * 6222822</a>; don't be fooled by the &ldquo;closed, fixed&rdquo; label), this
 * class peeks at the underlying stream and if it is {@link System#in} it uses
 * repeated reads instead of calling {@link InputStream#skip(long)} on the
 * underlying stream; moreover, skips and reads are tried alternately, so to
 * guarantee that skipping less bytes than requested can be caused only by
 * reaching the end of file.
 *
 * <li>
 * <p>
 * This class keeps also track of the number of bytes read so far, so to be able
 * to implement {@link MeasurableStream#position()} independently of underlying
 * input stream.
 *
 * <li>
 * <p>
 * This class has limited support for
 * {@linkplain #readLine(byte[], int, int, EnumSet) &ldquo;reading a
 * line&rdquo;} (whatever that means) from the underlying input stream. You can
 * choose the set of {@linkplain FastBufferedInputStream.LineTerminator line
 * terminators} that delimit lines.
 *
 * </ul>
 *
 * <p>
 * <strong>Warning:</strong> Since {@code fastutil} 6.0.0, this class detects a
 * implementations of {@link MeasurableStream} instead of subclasses
 * {@code MeasurableInputStream} (which is deprecated).
 *
 * @since 4.4
 */
...
public class FastBufferedInputStream extends MeasurableInputStream implements RepositionableStream {
	 ...
	 * In particular, this will always represent the index (in the underlying input
	 * stream) of the first available byte in the buffer.
	 */
	...
	public FastBufferedInputStream(final InputStream is, final byte[] buffer) {
		...
		if (is instanceof MeasurableStream)
			measurableStream = (MeasurableStream) is;
	 ...
	 *
	 * @param is         an input stream to wrap.
	 * @param bufferSize the size in bytes of the internal buffer (greater than
	 *                   zero).
	 */
	 ...
	 *
	 * @return true if there are no characters in the internal buffer and the
	...
	/**
	 * Reads a line into the given byte array using {@linkplain #ALL_TERMINATORS all
	 * terminators}.
	 *
	 * @param array byte array where the next line will be stored.
	 * @return the number of bytes actually placed in {@code array}, or -1 at end of
	 *         file.
	 ...
	 *
	 * @param array       byte array where the next line will be stored.
	 * @param terminators a set containing the line termination sequences that we
	 *                    want to consider as valid.
	 * @return the number of bytes actually placed in {@code array}, or -1 at end of
	 *         file.
	 * @see #readLine(byte[], int, int, EnumSet)
	 */
	 ...
	 * {@linkplain #ALL_TERMINATORS all terminators}.
	 *
	 * @param array byte array where the next line will be stored.
	 ...
	 * (methods available to that purpose in old versions of Java have been
	 * mercilessly deprecated). Nonetheless, in several situations, such as when
	 * decoding network protocols or headers known to be ASCII, it is very useful to
	 * be able to read a line from a byte stream.
	 *
	 * <p>
	 * This method will attempt to read the next line into {@code array} starting at
	 * {@code off}, reading at most {@code len} bytes. The read, however, will be
	 * stopped by the end of file or when meeting a {@linkplain LineTerminator
	 * <em>line terminator</em>}. Of course, for this operation to be sensible the
	 * encoding of the text contained in the stream, if any, must not generate
	 * spurious carriage returns or line feeds. Note that the termination detection
	 * uses a maximisation criterion, so if you specify both
	 * {@link LineTerminator#CR} and {@link LineTerminator#CR_LF} meeting a pair
	 * CR/LF will consider the whole pair a terminator.
	 *
	 * <p>
	 * Terminators are <em>not</em> copied into <em>array</em> or included in the
	 * returned count. The returned integer can be used to check whether the line is
	 * complete: if it is smaller than {@code len}, then more bytes might be
	 * available, but note that this method (contrarily to
	 * {@link #read(byte[], int, int)}) can legitimately return zero when
	 * {@code len} is nonzero just because a terminator was found as the first
	 * character. Thus, the intended usage of this method is to call it on a given
	 * array, check whether {@code len} bytes have been read, and if so try again
	 ...
	 * {@code off} (inclusive) and ending at {@code start + Math.max(len, 0)}
	 * (exclusive).
	 *
	 * @param array       byte array where the next line will be stored.
	 * @param off         the first byte to use in {@code array}.
	 ...
	 *                    want to consider as valid.
	 * @return the number of bytes actually placed in {@code array}, or -1 at end of
	 *         file. Note that the returned number will be {@code len} if no line
	 *         termination sequence specified in {@code terminators} has been met
	 *         before scanning {@code len} byte, and if also we did not meet the end
	 *         of file.
	 */

	public int readLine(final byte[] array, final int off, final int len, final EnumSet<LineTerminator> terminators)
			throws IOException {
		ByteArrays.ensureOffsetLength(array, off, len);
		if (len == 0)
			return 0; // 0-length reads always return 0
		if (noMoreCharacters())
			return -1;
				...
				if (k == '\n') { // LF first
					...
					if (terminators.contains(LineTerminator.CR_LF)) {
						if (avail > 0) {
							...
							if (noMoreCharacters()) {
								// Not found a matching LF because of end of file, will return CR in buffer if
								// not a terminator
	...
	public void position(long newPosition) throws IOException {
		...
		/**
		 * Note that this check will succeed also in the case of an empty buffer and
		 * position == newPosition. This behaviour is intentional, as it delays
		 * buffering to when it is actually necessary and avoids useless class the
		 * underlying stream.
		 */
	 ...
	 * {@linkplain MeasurableStream measurable}.
	 *
	 * @return the length of the underlying input stream.
	 * @throws UnsupportedOperationException if the underlying input stream is not
	 *                                       {@linkplain MeasurableStream
	 *                                       measurable} and cannot provide a
	 *                                       {@link FileChannel}.
	 */
	...
	/**
	 * Skips over and discards the given number of bytes of data from this fast
	 * buffered input stream.
	 *
	 * <p>
	 * As explained in the {@linkplain FastBufferedInputStream class documentation},
	 * the semantics of {@link InputStream#skip(long)} is fatally flawed. This
	 * method provides additional semantics as follows: it will skip the provided
	 * number of bytes, unless the end of file has been reached.
	 *
	 * <p>
	 * Additionally, if the underlying input stream is {@link System#in} this method
	 * will use repeated reads instead of invoking {@link InputStream#skip(long)}.
	 *
	 * @param n the number of bytes to skip.
	 * @return the number of bytes actually skipped; it can be smaller than
	 *         {@code n} only if the end of file has been reached.
	 * @see InputStream#skip(long)
	 */
	...
	/**
	 * Resets the internal logic of this fast buffered input stream, clearing the
	 * buffer.
	 *
	 * <p>
	 * All buffering information is discarded, and the number of bytes read so far
	 * (and thus, also the {@linkplain #position() current position}) is adjusted to
	 * reflect this fact.
	 *
	 * <p>
	 * This method is mainly useful for re-reading files that have been overwritten
	 * externally.
	 */
	 ...
	 *
	 * @deprecated As of {@code fastutil} 5.0.4, replaced by {@link #flush()}. The
	 *             old semantics of this method does not contradict
	 *             {@link InputStream}'s contract, as the semantics of
	 *             {@link #reset()} is undefined if
	 *             {@link InputStream#markSupported()} returns false. On the other
	 *             hand, the name was really a poor choice.
	 */
	@Override
	@Deprecated
	public void reset() {
