...
public class Double2ShortOpenCustomHashMap extends AbstractDouble2ShortMap
	...
	public Double2ShortOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ShortOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ShortOpenCustomHashMap(final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ShortOpenCustomHashMap(final Map<? extends Double, ? extends Short> m, final float f,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ShortOpenCustomHashMap(final Map<? extends Double, ? extends Short> m,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ShortOpenCustomHashMap(final Double2ShortMap m, final float f,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ShortOpenCustomHashMap(final Double2ShortMap m,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Double2ShortOpenCustomHashMap(final double[] k, final short[] v, final float f,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Double2ShortOpenCustomHashMap(final double[] k, final short[] v,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsent(final double k, final java.util.function.DoubleToIntFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsentNullable(final double k,
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfPresent(final double k,
			final java.util.function.BiFunction<? super Double, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Short newValue = remappingFunction.apply(Double.valueOf(k), Short.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public short compute(final double k,
			final java.util.function.BiFunction<? super Double, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Short newValue = remappingFunction.apply(Double.valueOf(k), pos >= 0 ? Short.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public short merge(final double k, final short v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Double2ShortOpenCustomHashMap.this.remove(wrapped.getDouble(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Double2ShortMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Double2ShortOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			double curr;
			final double[] key = Double2ShortOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Double2ShortMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractDouble2ShortMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Double.doubleToLongBits(key[pos]) == 0))
					consumer.accept(new AbstractDouble2ShortMap.BasicEntry(key[pos], value[pos]));
		}
