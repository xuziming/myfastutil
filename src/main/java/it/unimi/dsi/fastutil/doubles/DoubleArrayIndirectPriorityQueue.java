...
public class DoubleArrayIndirectPriorityQueue implements DoubleIndirectPriorityQueue {
	...
	private int findFirst() {
		...
		if (c == null)
			while (i-- != 0) {
				if ((Double.compare((refArray[array[i]]), (first)) < 0))
	...
	private int findLast() {
		...
		if (c == null) {
			while (i-- != 0)
				if ((Double.compare((last), (refArray[array[i]])) < 0))
	...
	public void enqueue(int x) {
		...
		if (firstIndexValid) {
			if (c == null) {
				if ((Double.compare((refArray[x]), (refArray[array[firstIndex]])) < 0))
					firstIndex = size;
	...
	public int front(int[] a) {
		...
		int i = size, c = 0;
		while (i-- != 0)
			if ((Double.doubleToLongBits(top) == Double.doubleToLongBits(refArray[array[i]])))
				a[c++] = array[i];
		return c;
	}
