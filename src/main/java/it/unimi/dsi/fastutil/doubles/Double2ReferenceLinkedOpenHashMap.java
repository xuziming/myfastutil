 ...
 * @see HashCommon
 */
public class Double2ReferenceLinkedOpenHashMap<V> extends AbstractDouble2ReferenceSortedMap<V>
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Double2ReferenceMap.Entry<V>>
		...
		@Override
		public ObjectSortedSet<Double2ReferenceMap.Entry<V>> subSet(Double2ReferenceMap.Entry<V> fromElement,
				Double2ReferenceMap.Entry<V> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if ((Double.doubleToLongBits(k) == 0))
				return Double2ReferenceLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			double curr;
			final double[] key = Double2ReferenceLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Double2ReferenceMap.Entry<V>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractDouble2ReferenceMap.BasicEntry<V>(key[curr], value[curr]));
			}
		}
