...
public final class DoubleBigArrays {
	...
	private static final class BigArrayHashStrategy implements Hash.Strategy<double[][]>, java.io.Serializable {
	...
	public static void quickSort(final double[][] x, final long from, final long to, final DoubleComparator comp) {
		...
		final double v = BigArrays.get(x, m);
	...
	public static void radixSort(final double[][] a, final long from, final long to) {
		...
		while (offsetPos > 0) {
			...
			for (long i = length; i-- != 0;)
				BigArrays.set(digit, i,
						(byte) (((fixDouble(BigArrays.get(a, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
			for (long i = length; i-- != 0;)
	...
	private static void selectionSort(final double[][] a, final double[][] b, final long from, final long to) {
		for (long i = from; i < to - 1; i++) {
			...
			for (long j = i + 1; j < to; j++)
				if ((Double.compare((BigArrays.get(a, j)), (BigArrays.get(a, m))) < 0)
						|| (Double.compare((BigArrays.get(a, j)), (BigArrays.get(a, m))) == 0)
								&& (Double.compare((BigArrays.get(b, j)), (BigArrays.get(b, m))) < 0))
					m = j;
			if (m != i) {
	...
	public static void radixSort(final double[][] a, final double[][] b, final long from, final long to) {
		...
		while (offsetPos > 0) {
			...
			for (long i = length; i-- != 0;)
				BigArrays.set(digit, i,
						(byte) (((fixDouble(BigArrays.get(k, first + i)) >>> shift) & DIGIT_MASK) ^ signMask));
			for (long i = length; i-- != 0;)
				count[BigArrays.get(digit, i) & 0xFF]++;
			// Compute cumulative distribution and push non-singleton keys on stack.
			int lastUsed = -1;
	...
	private static void insertionSortIndirect(final long[][] perm, final double[][] a, final double[][] b,
			final long from, final long to) {
		for (long i = from; ++i < to;) {
			long t = BigArrays.get(perm, i);
			long j = i;
			for (long u = BigArrays.get(perm, j - 1); (Double.compare((BigArrays.get(a, t)), (BigArrays.get(a, u))) < 0)
					|| (Double.compare((BigArrays.get(a, t)), (BigArrays.get(a, u))) == 0)
							&& (Double.compare((BigArrays.get(b, t)), (BigArrays.get(b, u))) < 0); u = BigArrays
									.get(perm, --j - 1)) {
	 ...
	 */
	public static void radixSortIndirect(final long[][] perm, final double[][] a, final double[][] b,
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final long[][] perm, final double[][] a, final double[][] b, final long from,
		...
		final long[] count = new long[1 << DIGIT_BITS];
		final long[] pos = new long[1 << DIGIT_BITS];
		final long[][] support = stable ? it.unimi.dsi.fastutil.longs.LongBigArrays.newBigArray(BigArrays.length(perm))
				: null;
		while (stackPos > 0) {
			...
			for (long i = first + length; i-- != first;)
				count[(int) (fixDouble(BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)]++;
			...
			if (stable) {
				for (long i = first + length; i-- != first;)
					BigArrays.set(support,
							--pos[(int) (fixDouble(BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK
									^ signMask)],
				...
				for (long i = first, d; i <= end; i += count[c], count[c] = 0) {
					long t = BigArrays.get(perm, i);
					c = (int) (fixDouble(BigArrays.get(k, t)) >>> shift & DIGIT_MASK ^ signMask);
