 ...
 */
public class DoubleArrayPriorityQueue implements DoublePriorityQueue, java.io.Serializable {
	 ...
	 *
	 * @param c the comparator used in this queue, or {@code null} for the natural
	 *          order.
	 */
	...
	private int findFirst() {
		...
		if (c == null) {
			while (i-- != 0)
				if ((Double.compare((array[i]), (first)) < 0))
	...
	public void enqueue(double x) {
		if (size == array.length)
			array = DoubleArrays.grow(array, size + 1);
		if (firstIndexValid) {
			if (c == null) {
				if ((Double.compare((x), (array[firstIndex])) < 0))
					firstIndex = size;
