 ...
 * iterator}.
 */
public class Double2BooleanAVLTreeMap extends AbstractDouble2BooleanSortedMap
	 ...
	 */
	public Double2BooleanAVLTreeMap(final Comparator<? super Double> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Double2BooleanAVLTreeMap(final Map<? extends Double, ? extends Boolean> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Double2BooleanAVLTreeMap(final SortedMap<Double, Boolean> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Double2BooleanAVLTreeMap(final Double2BooleanMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Double2BooleanAVLTreeMap(final Double2BooleanSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Double2BooleanAVLTreeMap(final double[] k, final boolean v[], final Comparator<? super Double> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Double2BooleanAVLTreeMap(final double[] k, final boolean v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Double2BooleanAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Double2BooleanMap.Entry> double2BooleanEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2BooleanMap.Entry>() {
				final Comparator<? super Double2BooleanMap.Entry> comparator = (Comparator<Double2BooleanMap.Entry>) (x,
						y) -> Double2BooleanAVLTreeMap.this.actualComparator.compare(x.getDoubleKey(),
								y.getDoubleKey());
				...
				@Override
				public ObjectBidirectionalIterator<Double2BooleanMap.Entry> iterator(
						final Double2BooleanMap.Entry from) {
				...
				@Override
				public ObjectSortedSet<Double2BooleanMap.Entry> subSet(Double2BooleanMap.Entry from,
						Double2BooleanMap.Entry to) {
					return subMap(from.getDoubleKey(), to.getDoubleKey()).double2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Double2BooleanMap.Entry> headSet(Double2BooleanMap.Entry to) {
					return headMap(to.getDoubleKey()).double2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Double2BooleanMap.Entry> tailSet(Double2BooleanMap.Entry from) {
					return tailMap(from.getDoubleKey()).double2BooleanEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractDouble2BooleanSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final double k) {
			return (bottom || Double2BooleanAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Double2BooleanAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2BooleanMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Double2BooleanAVLTreeMap.Entry f = findKey(((Double) (e.getKey())).doubleValue());
		...
		public boolean containsKey(final double k) {
			return in(k) && Double2BooleanAVLTreeMap.this.containsKey(k);
		}
		...
		public boolean get(final double k) {
			final Double2BooleanAVLTreeMap.Entry e;
			final double kk = k;
		...
		public boolean put(final double k, final boolean v) {
			...
			final boolean oldValue = Double2BooleanAVLTreeMap.this.put(k, v);
		...
		public boolean remove(final double k) {
			...
			final boolean oldValue = Double2BooleanAVLTreeMap.this.remove(k);
		...
		@Override
		public double firstDoubleKey() {
			Double2BooleanAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public double lastDoubleKey() {
			Double2BooleanAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2BooleanAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2BooleanAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
