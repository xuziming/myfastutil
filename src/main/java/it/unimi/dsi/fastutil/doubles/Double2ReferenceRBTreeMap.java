...
public class Double2ReferenceRBTreeMap<V> extends AbstractDouble2ReferenceSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Double2ReferenceRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2ReferenceMap.Entry<V>>() {
				final Comparator<? super Double2ReferenceMap.Entry<V>> comparator = (Comparator<Double2ReferenceMap.Entry<V>>) (
						x, y) -> Double2ReferenceRBTreeMap.this.actualComparator.compare(x.getDoubleKey(),
								y.getDoubleKey());
	...
	private final class Submap extends AbstractDouble2ReferenceSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final double k) {
			return (bottom || Double2ReferenceRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Double2ReferenceRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Double))
							return false;
						final Double2ReferenceRBTreeMap.Entry<V> f = findKey(((Double) (e.getKey())).doubleValue());
		...
		public boolean containsKey(final double k) {
			return in(k) && Double2ReferenceRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public double firstDoubleKey() {
			Double2ReferenceRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public double lastDoubleKey() {
			Double2ReferenceRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2ReferenceRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2ReferenceRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Double2ReferenceRBTreeMap<V> clone() {
		...
		try {
			c = (Double2ReferenceRBTreeMap<V>) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
