...
public class Double2BooleanRBTreeMap extends AbstractDouble2BooleanSortedMap
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Double2BooleanRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2BooleanMap.Entry>() {
				final Comparator<? super Double2BooleanMap.Entry> comparator = (Comparator<Double2BooleanMap.Entry>) (x,
						y) -> Double2BooleanRBTreeMap.this.actualComparator.compare(x.getDoubleKey(), y.getDoubleKey());
	...
	private final class Submap extends AbstractDouble2BooleanSortedMap implements java.io.Serializable {
		...
		final boolean in(final double k) {
			return (bottom || Double2BooleanRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Double2BooleanRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2BooleanMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Double2BooleanRBTreeMap.Entry f = findKey(((Double) (e.getKey())).doubleValue());
		...
		public boolean containsKey(final double k) {
			return in(k) && Double2BooleanRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public double firstDoubleKey() {
			Double2BooleanRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public double lastDoubleKey() {
			Double2BooleanRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2BooleanRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2BooleanRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
