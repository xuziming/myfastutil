...
public class Double2LongAVLTreeMap extends AbstractDouble2LongSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2LongMap.Entry>() {
				final Comparator<? super Double2LongMap.Entry> comparator = (Comparator<Double2LongMap.Entry>) (x,
						y) -> Double2LongAVLTreeMap.this.actualComparator.compare(x.getDoubleKey(), y.getDoubleKey());
	...
	private final class Submap extends AbstractDouble2LongSortedMap implements java.io.Serializable {
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2LongMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Long))
							return false;
						final Double2LongAVLTreeMap.Entry f = findKey(((Double) (e.getKey())).doubleValue());
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2LongAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2LongAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
