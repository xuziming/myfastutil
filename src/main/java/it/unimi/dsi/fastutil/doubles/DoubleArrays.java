...
public final class DoubleArrays {
	...
	public static void quickSort(final double[] x, final double[] y, final int from, final int to) {
		...
		while (true) {
			int comparison, t;
			while (b <= c && (comparison = (t = (Double.compare((x[b]), (v)))) == 0 ? (Double.compare((y[b]), (w)))
					: t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			}
			while (c >= b && (comparison = (t = (Double.compare((x[c]), (v)))) == 0 ? (Double.compare((y[c]), (w)))
					: t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final double[] a, final int from, final int to,
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void parallelRadixSortIndirect(final int perm[], final double[] a, final int from, final int to,
			final boolean stable) {
		if (to - from < PARALLEL_RADIXSORT_NO_FORK) {
	...
	private static void insertionSortIndirect(final int[] perm, final double[] a, final double[] b, final int from,
			final int to) {
		for (int i = from; ++i < to;) {
			int t = perm[i];
			int j = i;
			for (int u = perm[j - 1]; (Double.compare((a[t]), (a[u])) < 0) || (Double.compare((a[t]), (a[u])) == 0)
					&& (Double.compare((b[t]), (b[u])) < 0); u = perm[--j - 1]) {
				perm[j] = u;
				if (from == j - 1) {
					--j;
					break;
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final double[] a, final double[] b, final int from,
