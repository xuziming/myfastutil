 ...
 * iterator}.
 */
public class Double2ReferenceAVLTreeMap<V> extends AbstractDouble2ReferenceSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Double2ReferenceAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Double2ReferenceMap.Entry<V>> double2ReferenceEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2ReferenceMap.Entry<V>>() {
				final Comparator<? super Double2ReferenceMap.Entry<V>> comparator = (Comparator<Double2ReferenceMap.Entry<V>>) (
						x, y) -> Double2ReferenceAVLTreeMap.this.actualComparator.compare(x.getDoubleKey(),
								y.getDoubleKey());
				...
				@Override
				public ObjectBidirectionalIterator<Double2ReferenceMap.Entry<V>> iterator(
						final Double2ReferenceMap.Entry<V> from) {
				...
				@Override
				public ObjectSortedSet<Double2ReferenceMap.Entry<V>> subSet(Double2ReferenceMap.Entry<V> from,
						Double2ReferenceMap.Entry<V> to) {
					return subMap(from.getDoubleKey(), to.getDoubleKey()).double2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Double2ReferenceMap.Entry<V>> headSet(Double2ReferenceMap.Entry<V> to) {
					return headMap(to.getDoubleKey()).double2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Double2ReferenceMap.Entry<V>> tailSet(Double2ReferenceMap.Entry<V> from) {
					return tailMap(from.getDoubleKey()).double2ReferenceEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractDouble2ReferenceSortedMap<V> implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final double k) {
			return (bottom || Double2ReferenceAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Double2ReferenceAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Double))
							return false;
						final Double2ReferenceAVLTreeMap.Entry<V> f = findKey(((Double) (e.getKey())).doubleValue());
		...
		public boolean containsKey(final double k) {
			return in(k) && Double2ReferenceAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public double firstDoubleKey() {
			Double2ReferenceAVLTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public double lastDoubleKey() {
			Double2ReferenceAVLTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2ReferenceAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2ReferenceAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
