 ...
 */
public class DoubleRBTreeSet extends AbstractDoubleSortedSet
		implements java.io.Serializable, Cloneable, DoubleSortedSet {
	 ...
	 */
	public DoubleRBTreeSet(final Comparator<? super Double> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public DoubleRBTreeSet(final Collection<? extends Double> c) {
		this();
		addAll(c);
	}
	 ...
	 */
	public DoubleRBTreeSet(final SortedSet<Double> s) {
		this(s.comparator());
		addAll(s);
	}
	 ...
	 */
	public DoubleRBTreeSet(final DoubleCollection c) {
		this();
		addAll(c);
	}
	 ...
	 */
	public DoubleRBTreeSet(final DoubleSortedSet s) {
		this(s.comparator());
		addAll(s);
	}
	 ...
	 */
	public DoubleRBTreeSet(final DoubleIterator i) {
		while (i.hasNext())
			add(i.nextDouble());
	}
	...
	public DoubleRBTreeSet(final Iterator<?> i) {
	 ...
	 * @param c      a {@link Comparator} (even better, a type-specific comparator).
	 */
	public DoubleRBTreeSet(final double[] a, final int offset, final int length, final Comparator<? super Double> c) {
		this(c);
	 ...
	 * @param length the number of elements to use.
	 */
	public DoubleRBTreeSet(final double[] a, final int offset, final int length) {
	 ...
	 */
	public DoubleRBTreeSet(final double[] a) {
		this();
		int i = a.length;
		while (i-- != 0)
			add(a[i]);
	 ...
	 * @param c a {@link Comparator} (even better, a type-specific comparator).
	 */
	public DoubleRBTreeSet(final double[] a, final Comparator<? super Double> c) {
		this(c);
		int i = a.length;
		while (i-- != 0)
			add(a[i]);
	...
	private class SetIterator implements DoubleListIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			DoubleRBTreeSet.this.remove(curr.key);
			curr = null;
		}
	}
	...
	private final class Subset extends AbstractDoubleSortedSet implements java.io.Serializable, DoubleSortedSet {
		...
		final boolean in(final double k) {
			return (bottom || DoubleRBTreeSet.this.compare(k, from) >= 0)
					&& (top || DoubleRBTreeSet.this.compare(k, to) < 0);
		}

		@Override

		public boolean contains(final double k) {
			return in(k) && DoubleRBTreeSet.this.contains(k);
		}
		...
		@Override
		public double firstDouble() {
			DoubleRBTreeSet.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public double lastDouble() {
			DoubleRBTreeSet.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && DoubleRBTreeSet.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && DoubleRBTreeSet.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	}
