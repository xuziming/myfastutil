...
public class Double2DoubleRBTreeMap extends AbstractDouble2DoubleSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2DoubleMap.Entry>() {
				final Comparator<? super Double2DoubleMap.Entry> comparator = (Comparator<Double2DoubleMap.Entry>) (x,
						y) -> Double2DoubleRBTreeMap.this.actualComparator.compare(x.getDoubleKey(), y.getDoubleKey());
	...
	private final class Submap extends AbstractDouble2DoubleSortedMap implements java.io.Serializable {
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2DoubleMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Double))
							return false;
						final Double2DoubleRBTreeMap.Entry f = findKey(((Double) (e.getKey())).doubleValue());
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2DoubleRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2DoubleRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
