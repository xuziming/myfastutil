 ...
 *
 */
public class Double2ObjectRBTreeMap<V> extends AbstractDouble2ObjectSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Double2ObjectRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2ObjectMap.Entry<V>>() {
				final Comparator<? super Double2ObjectMap.Entry<V>> comparator = (Comparator<Double2ObjectMap.Entry<V>>) (
						x,
						y) -> Double2ObjectRBTreeMap.this.actualComparator.compare(x.getDoubleKey(), y.getDoubleKey());
	...
	private final class Submap extends AbstractDouble2ObjectSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final double k) {
			return (bottom || Double2ObjectRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Double2ObjectRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2ObjectMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Double))
							return false;
						final Double2ObjectRBTreeMap.Entry<V> f = findKey(((Double) (e.getKey())).doubleValue());
		...
		public boolean containsKey(final double k) {
			return in(k) && Double2ObjectRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public double firstDoubleKey() {
			Double2ObjectRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public double lastDoubleKey() {
			Double2ObjectRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2ObjectRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2ObjectRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
