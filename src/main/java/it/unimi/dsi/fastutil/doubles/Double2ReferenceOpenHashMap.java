 ...
 * @see HashCommon
 */
public class Double2ReferenceOpenHashMap<V> extends AbstractDouble2ReferenceMap<V>
	...
	private class MapIterator {
		 ...
		 */
		boolean mustReturnNullKey = Double2ReferenceOpenHashMap.this.containsNullKey;
		/**
		...
		public void remove() {
			...
			if (last == n) {
				...
				Double2ReferenceOpenHashMap.this.remove(wrapped.getDouble(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Double2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		...
		public boolean contains(final Object o) {
			...
			if ((Double.doubleToLongBits(k) == 0))
				return Double2ReferenceOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			double curr;
			final double[] key = Double2ReferenceOpenHashMap.this.key;
			int pos;
			// The starting point.
	...
	public Double2ReferenceOpenHashMap<V> clone() {
		...
		try {
			c = (Double2ReferenceOpenHashMap<V>) super.clone();
