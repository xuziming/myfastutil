...
public class Double2ByteOpenCustomHashMap extends AbstractDouble2ByteMap
	...
	public Double2ByteOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ByteOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ByteOpenCustomHashMap(final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ByteOpenCustomHashMap(final Map<? extends Double, ? extends Byte> m, final float f,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ByteOpenCustomHashMap(final Map<? extends Double, ? extends Byte> m,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ByteOpenCustomHashMap(final Double2ByteMap m, final float f,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Double2ByteOpenCustomHashMap(final Double2ByteMap m,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Double2ByteOpenCustomHashMap(final double[] k, final byte[] v, final float f,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Double2ByteOpenCustomHashMap(final double[] k, final byte[] v,
			final it.unimi.dsi.fastutil.doubles.DoubleHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsent(final double k, final java.util.function.DoubleToIntFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsentNullable(final double k,
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfPresent(final double k,
			final java.util.function.BiFunction<? super Double, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Byte newValue = remappingFunction.apply(Double.valueOf(k), Byte.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public byte compute(final double k,
			final java.util.function.BiFunction<? super Double, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Byte newValue = remappingFunction.apply(Double.valueOf(k), pos >= 0 ? Byte.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public byte merge(final double k, final byte v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Double2ByteOpenCustomHashMap.this.remove(wrapped.getDouble(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Double2ByteMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Double2ByteOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			double curr;
			final double[] key = Double2ByteOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Double2ByteMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractDouble2ByteMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!(Double.doubleToLongBits(key[pos]) == 0))
					consumer.accept(new AbstractDouble2ByteMap.BasicEntry(key[pos], value[pos]));
		}
