 ...
 */
public class DoubleAVLTreeSet extends AbstractDoubleSortedSet
	...
	private final class Subset extends AbstractDoubleSortedSet implements java.io.Serializable, DoubleSortedSet {
		...
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && DoubleAVLTreeSet.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && DoubleAVLTreeSet.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	}
