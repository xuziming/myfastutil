...
public class Double2ShortAVLTreeMap extends AbstractDouble2ShortSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2ShortMap.Entry>() {
				final Comparator<? super Double2ShortMap.Entry> comparator = (Comparator<Double2ShortMap.Entry>) (x,
						y) -> Double2ShortAVLTreeMap.this.actualComparator.compare(x.getDoubleKey(), y.getDoubleKey());
	...
	private final class Submap extends AbstractDouble2ShortSortedMap implements java.io.Serializable {
		...
		final boolean in(final double k) {
			return (bottom || Double2ShortAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Double2ShortAVLTreeMap.this.compare(k, to) < 0);
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2ShortAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2ShortAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
