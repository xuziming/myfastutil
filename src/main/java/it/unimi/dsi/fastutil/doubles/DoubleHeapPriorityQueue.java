...
public class DoubleHeapPriorityQueue implements DoublePriorityQueue, java.io.Serializable {
	 ...
	 *
	 * @param collection a collection; its elements will be used to initialize the
	 *                   queue.
	 * @param c          the comparator used in this queue, or {@code null} for the
	 *                   natural order.
	 */
	...
	public DoubleHeapPriorityQueue(final Collection<? extends Double> collection, final DoubleComparator c) {
		...
		for (int i = 0; i < size; i++)
			heap[i] = ((Double) (iterator.next())).doubleValue();
	}
	...
	public void enqueue(double x) {
		if (size == heap.length)
			heap = DoubleArrays.grow(heap, size + 1);
		heap[size++] = x;
