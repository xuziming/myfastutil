 ...
 *
 */
public class Double2CharRBTreeMap extends AbstractDouble2CharSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Double2CharRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Double2CharMap.Entry>() {
				final Comparator<? super Double2CharMap.Entry> comparator = (Comparator<Double2CharMap.Entry>) (x,
						y) -> Double2CharRBTreeMap.this.actualComparator.compare(x.getDoubleKey(), y.getDoubleKey());
	...
	private final class Submap extends AbstractDouble2CharSortedMap implements java.io.Serializable {
		...
		final boolean in(final double k) {
			return (bottom || Double2CharRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Double2CharRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Double2CharMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Character))
							return false;
						final Double2CharRBTreeMap.Entry f = findKey(((Double) (e.getKey())).doubleValue());
		...
		public boolean containsKey(final double k) {
			return in(k) && Double2CharRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public double firstDoubleKey() {
			Double2CharRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public double lastDoubleKey() {
			Double2CharRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Double2CharRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Double2CharRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
