...
public interface Double2ShortMap extends Double2ShortFunction, Map<Double, Short> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Double2ShortMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Double2ShortMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final double key, final java.util.function.DoubleToIntFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsentNullable(final double key,
	 ...
	 * @since 8.0.0
	 */
	default short computeIfPresent(final double key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Short newValue = remappingFunction.apply(Double.valueOf(key), Short.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default short compute(final double key,
			final java.util.function.BiFunction<? super Double, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Short newValue = remappingFunction.apply(Double.valueOf(key), contained ? Short.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default short merge(final double key, final short value,
	...
	@Deprecated
	@Override
	default Short computeIfAbsent(final Double key,
			final java.util.function.Function<? super Double, ? extends Short> mappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Double, Short> {
		/**
