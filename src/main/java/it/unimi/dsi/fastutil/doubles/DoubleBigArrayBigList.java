...
public class DoubleBigArrayBigList extends AbstractDoubleBigList
	...
	public DoubleBigArrayBigList(final long capacity) {
		...
		if (capacity == 0)
			a = DoubleBigArrays.EMPTY_BIG_ARRAY;
		else
	...
	public DoubleBigArrayBigList() {
		a = DoubleBigArrays.DEFAULT_EMPTY_BIG_ARRAY; // We delay allocation
	}
	...
	public void ensureCapacity(final long capacity) {
		if (capacity <= a.length || a == DoubleBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
			return;
	...
	private void grow(long capacity) {
		...
		if (capacity <= oldLength)
			return;
		if (a != DoubleBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
	...
	public long indexOf(final double k) {
		for (long i = 0; i < size; i++)
			if ((Double.doubleToLongBits(k) == Double.doubleToLongBits(BigArrays.get(a, i))))
				return i;
		return -1;
	}
	...
	public double removeDouble(final long index) {
		if (index >= size)
			throw new IndexOutOfBoundsException(
					"Index (" + index + ") is greater than or equal to list size (" + size + ")");
		final double old = BigArrays.get(a, index);
		size--;
		if (index != size)
	...
	public double set(final long index, final double k) {
		if (index >= size)
			throw new IndexOutOfBoundsException(
					"Index (" + index + ") is greater than or equal to list size (" + size + ")");
		double old = BigArrays.get(a, index);
		BigArrays.set(a, index, k);
		return old;
	}
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		for (int i = 0; i < size; i++)
			s.writeDouble(BigArrays.get(a, i));
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		for (int i = 0; i < size; i++)
			BigArrays.set(a, i, s.readDouble());
	}
}
