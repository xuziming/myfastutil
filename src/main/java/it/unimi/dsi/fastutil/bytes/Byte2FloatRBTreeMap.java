 ...
 *
 */
public class Byte2FloatRBTreeMap extends AbstractByte2FloatSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Byte2FloatRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Byte2FloatMap.Entry>() {
				final Comparator<? super Byte2FloatMap.Entry> comparator = (Comparator<Byte2FloatMap.Entry>) (x,
						y) -> Byte2FloatRBTreeMap.this.actualComparator.compare(x.getByteKey(), y.getByteKey());
	...
	private final class Submap extends AbstractByte2FloatSortedMap implements java.io.Serializable {
		...
		final boolean in(final byte k) {
			return (bottom || Byte2FloatRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Byte2FloatRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Byte2FloatMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Float))
							return false;
						final Byte2FloatRBTreeMap.Entry f = findKey(((Byte) (e.getKey())).byteValue());
		...
		public boolean containsKey(final byte k) {
			return in(k) && Byte2FloatRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public byte firstByteKey() {
			Byte2FloatRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public byte lastByteKey() {
			Byte2FloatRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Byte2FloatRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Byte2FloatRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
