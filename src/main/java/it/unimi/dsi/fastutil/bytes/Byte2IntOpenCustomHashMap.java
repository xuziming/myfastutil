 ...
 * @see HashCommon
 */
public class Byte2IntOpenCustomHashMap extends AbstractByte2IntMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Byte2IntOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2IntOpenCustomHashMap(final int expected, final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2IntOpenCustomHashMap(final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2IntOpenCustomHashMap(final Map<? extends Byte, ? extends Integer> m, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2IntOpenCustomHashMap(final Map<? extends Byte, ? extends Integer> m,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2IntOpenCustomHashMap(final Byte2IntMap m, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2IntOpenCustomHashMap(final Byte2IntMap m,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Byte2IntOpenCustomHashMap(final byte[] k, final int[] v, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Byte2IntOpenCustomHashMap(final byte[] k, final int[] v,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsent(final byte k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsentNullable(final byte k,
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfPresent(final byte k,
			final java.util.function.BiFunction<? super Byte, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Integer newValue = remappingFunction.apply(Byte.valueOf(k), Integer.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((byte) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public int compute(final byte k,
			final java.util.function.BiFunction<? super Byte, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Integer newValue = remappingFunction.apply(Byte.valueOf(k),
	...
	/** {@inheritDoc} */
	@Override
	public int merge(final byte k, final int v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Byte2IntOpenCustomHashMap.this.remove(wrapped.getByte(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Byte2IntMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((byte) 0))))
				return Byte2IntOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			byte curr;
			final byte[] key = Byte2IntOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Byte2IntMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractByte2IntMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((byte) 0)))
					consumer.accept(new AbstractByte2IntMap.BasicEntry(key[pos], value[pos]));
		}
