...
public class Byte2LongAVLTreeMap extends AbstractByte2LongSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Byte2LongMap.Entry>() {
				final Comparator<? super Byte2LongMap.Entry> comparator = (Comparator<Byte2LongMap.Entry>) (x,
						y) -> Byte2LongAVLTreeMap.this.actualComparator.compare(x.getByteKey(), y.getByteKey());
	...
	private final class Submap extends AbstractByte2LongSortedMap implements java.io.Serializable {
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Byte2LongAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Byte2LongAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
