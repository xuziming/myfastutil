 ...
 * @see HashCommon
 */
public class Byte2ReferenceOpenCustomHashMap<V> extends AbstractByte2ReferenceMap<V>
	...
	public Byte2ReferenceOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2ReferenceOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2ReferenceOpenCustomHashMap(final Map<? extends Byte, ? extends V> m, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2ReferenceOpenCustomHashMap(final Map<? extends Byte, ? extends V> m,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2ReferenceOpenCustomHashMap(final Byte2ReferenceMap<V> m, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2ReferenceOpenCustomHashMap(final Byte2ReferenceMap<V> m,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Byte2ReferenceOpenCustomHashMap(final byte[] k, final V[] v, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Byte2ReferenceOpenCustomHashMap(final byte[] k, final V[] v,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Byte2ReferenceOpenCustomHashMap.this.remove(wrapped.getByte(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Byte2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((byte) 0))))
				return Byte2ReferenceOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			byte curr;
			final byte[] key = Byte2ReferenceOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Byte2ReferenceMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractByte2ReferenceMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((byte) 0)))
					consumer.accept(new AbstractByte2ReferenceMap.BasicEntry<V>(key[pos], value[pos]));
		}
