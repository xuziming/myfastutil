 ...
 *
 */
public class Byte2BooleanRBTreeMap extends AbstractByte2BooleanSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Byte2BooleanRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Byte2BooleanMap.Entry>() {
				final Comparator<? super Byte2BooleanMap.Entry> comparator = (Comparator<Byte2BooleanMap.Entry>) (x,
						y) -> Byte2BooleanRBTreeMap.this.actualComparator.compare(x.getByteKey(), y.getByteKey());
	...
	private final class Submap extends AbstractByte2BooleanSortedMap implements java.io.Serializable {
		...
		final boolean in(final byte k) {
			return (bottom || Byte2BooleanRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Byte2BooleanRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Byte2BooleanMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Byte2BooleanRBTreeMap.Entry f = findKey(((Byte) (e.getKey())).byteValue());
		...
		public boolean containsKey(final byte k) {
			return in(k) && Byte2BooleanRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public byte firstByteKey() {
			Byte2BooleanRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public byte lastByteKey() {
			Byte2BooleanRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Byte2BooleanRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Byte2BooleanRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
