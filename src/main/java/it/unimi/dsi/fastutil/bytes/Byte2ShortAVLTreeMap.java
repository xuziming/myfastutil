 ...
 * iterator}.
 */
public class Byte2ShortAVLTreeMap extends AbstractByte2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Byte2ShortAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Byte2ShortMap.Entry>() {
				final Comparator<? super Byte2ShortMap.Entry> comparator = (Comparator<Byte2ShortMap.Entry>) (x,
						y) -> Byte2ShortAVLTreeMap.this.actualComparator.compare(x.getByteKey(), y.getByteKey());
	...
	private final class Submap extends AbstractByte2ShortSortedMap implements java.io.Serializable {
		...
		final boolean in(final byte k) {
			return (bottom || Byte2ShortAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Byte2ShortAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Byte2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Byte2ShortAVLTreeMap.Entry f = findKey(((Byte) (e.getKey())).byteValue());
		...
		public boolean containsKey(final byte k) {
			return in(k) && Byte2ShortAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public byte firstByteKey() {
			Byte2ShortAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public byte lastByteKey() {
			Byte2ShortAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Byte2ShortAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Byte2ShortAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
