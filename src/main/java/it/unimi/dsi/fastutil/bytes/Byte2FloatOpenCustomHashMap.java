...
public class Byte2FloatOpenCustomHashMap extends AbstractByte2FloatMap
	...
	public Byte2FloatOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2FloatOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2FloatOpenCustomHashMap(final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2FloatOpenCustomHashMap(final Map<? extends Byte, ? extends Float> m, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2FloatOpenCustomHashMap(final Map<? extends Byte, ? extends Float> m,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2FloatOpenCustomHashMap(final Byte2FloatMap m, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Byte2FloatOpenCustomHashMap(final Byte2FloatMap m,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Byte2FloatOpenCustomHashMap(final byte[] k, final float[] v, final float f,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Byte2FloatOpenCustomHashMap(final byte[] k, final float[] v,
			final it.unimi.dsi.fastutil.bytes.ByteHash.Strategy strategy) {
	...
	public boolean containsValue(final float v) {
		...
		if (containsNullKey && (Float.floatToIntBits(value[n]) == Float.floatToIntBits(v)))
			return true;
		for (int i = n; i-- != 0;)
			if (!((key[i]) == ((byte) 0)) && (Float.floatToIntBits(value[i]) == Float.floatToIntBits(v)))
				return true;
		return false;
	}
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfAbsent(final byte k, final java.util.function.IntToDoubleFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfAbsentNullable(final byte k,
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfPresent(final byte k,
			final java.util.function.BiFunction<? super Byte, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Float newValue = remappingFunction.apply(Byte.valueOf(k), Float.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((byte) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public float compute(final byte k,
			final java.util.function.BiFunction<? super Byte, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Float newValue = remappingFunction.apply(Byte.valueOf(k), pos >= 0 ? Float.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), ((byte) 0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public float merge(final byte k, final float v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final byte key[] = Byte2FloatOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Byte2FloatOpenCustomHashMap.this.remove(wrapped.getByte(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Byte2FloatMap.Entry> implements FastEntrySet {
		@Override
		...
		public void forEach(final Consumer<? super Byte2FloatMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractByte2FloatMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((byte) 0)))
					consumer.accept(new AbstractByte2FloatMap.BasicEntry(key[pos], value[pos]));
		}
