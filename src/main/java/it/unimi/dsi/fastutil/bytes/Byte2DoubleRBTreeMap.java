 ...
 *
 */
public class Byte2DoubleRBTreeMap extends AbstractByte2DoubleSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Byte2DoubleRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Byte2DoubleMap.Entry>() {
				final Comparator<? super Byte2DoubleMap.Entry> comparator = (Comparator<Byte2DoubleMap.Entry>) (x,
						y) -> Byte2DoubleRBTreeMap.this.actualComparator.compare(x.getByteKey(), y.getByteKey());
	...
	private final class Submap extends AbstractByte2DoubleSortedMap implements java.io.Serializable {
		...
		final boolean in(final byte k) {
			return (bottom || Byte2DoubleRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Byte2DoubleRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Byte2DoubleMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Double))
							return false;
						final Byte2DoubleRBTreeMap.Entry f = findKey(((Byte) (e.getKey())).byteValue());
		...
		public boolean containsKey(final byte k) {
			return in(k) && Byte2DoubleRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public byte firstByteKey() {
			Byte2DoubleRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public byte lastByteKey() {
			Byte2DoubleRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Byte2DoubleRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Byte2DoubleRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
