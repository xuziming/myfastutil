 ...
 * iterator}.
 */
public class Byte2ObjectAVLTreeMap<V> extends AbstractByte2ObjectSortedMap<V>
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Byte2ObjectMap.Entry<V>>() {
				final Comparator<? super Byte2ObjectMap.Entry<V>> comparator = (Comparator<Byte2ObjectMap.Entry<V>>) (x,
						y) -> Byte2ObjectAVLTreeMap.this.actualComparator.compare(x.getByteKey(), y.getByteKey());
	...
	private final class Submap extends AbstractByte2ObjectSortedMap<V> implements java.io.Serializable {
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Byte2ObjectMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Byte))
							return false;
						final Byte2ObjectAVLTreeMap.Entry<V> f = findKey(((Byte) (e.getKey())).byteValue());
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Byte2ObjectAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Byte2ObjectAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
