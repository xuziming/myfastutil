...
public final class ByteArrays {
	...
	public static void quickSort(final byte[] x, final byte[] y, final int from, final int to) {
		...
		while (true) {
			int comparison, t;
			while (b <= c
					&& (comparison = (t = (Byte.compare((x[b]), (v)))) == 0 ? (Byte.compare((y[b]), (w))) : t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			}
			while (c >= b
					&& (comparison = (t = (Byte.compare((x[c]), (v)))) == 0 ? (Byte.compare((y[c]), (w))) : t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
			}
	...
	protected static class ForkJoinQuickSort2 extends RecursiveAction {
		...
		protected void compute() {
			...
			while (true) {
				int comparison, t;
				while (b <= c && (comparison = (t = (Byte.compare((x[b]), (v)))) == 0 ? (Byte.compare((y[b]), (w)))
						: t) <= 0) {
					if (comparison == 0)
						swap(x, y, a++, b);
					b++;
				}
				while (c >= b && (comparison = (t = (Byte.compare((x[c]), (v)))) == 0 ? (Byte.compare((y[c]), (w)))
						: t) >= 0) {
					if (comparison == 0)
						swap(x, y, c, d--);
					c--;
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final byte a[], final int from, final int to) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final byte a[]) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final byte a[], final int from, final int to, ByteComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final byte a[], ByteComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final byte a[], final int from, final int to) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final byte a[]) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final byte a[], final int from, final int to, ByteComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final byte a[], ByteComparator comp) {
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void parallelRadixSortIndirect(final int perm[], final byte[] a, final int from, final int to,
			final boolean stable) {
		if (to - from < PARALLEL_RADIXSORT_NO_FORK) {
	...
	private static void insertionSortIndirect(final int[] perm, final byte[] a, final byte[] b, final int from,
			final int to) {
		for (int i = from; ++i < to;) {
			int t = perm[i];
			int j = i;
