...
public class ByteArrayList extends AbstractByteList implements RandomAccess, Cloneable, java.io.Serializable {
	 ...
	 */
	protected ByteArrayList(final byte a[], boolean dummy) {
		this.a = a;
	}
	...
	public void ensureCapacity(final int capacity) {
		if (capacity <= a.length || (a == ByteArrays.DEFAULT_EMPTY_ARRAY && capacity <= DEFAULT_INITIAL_CAPACITY))
			return;
	...
	private void grow(int capacity) {
		if (capacity <= a.length)
			return;
		if (a != ByteArrays.DEFAULT_EMPTY_ARRAY)
	...
	public void size(final int size) {
		if (size > a.length)
			a = ByteArrays.forceCapacity(a, size, this.size);
		if (size > this.size)
			Arrays.fill(a, this.size, size, ((byte) 0));
		this.size = size;
	}
	...
	public boolean removeAll(final Collection<?> c) {
		...
		int j = 0;
		for (int i = 0; i < size; i++)
			if (!c.contains(Byte.valueOf(a[i])))
				a[j++] = a[i];
		final boolean modified = size != j;
		size = j;
		return modified;
	...
	public ByteArrayList clone() {
		ByteArrayList c = new ByteArrayList(size);
