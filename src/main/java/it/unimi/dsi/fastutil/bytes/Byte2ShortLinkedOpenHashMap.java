...
public class Byte2ShortLinkedOpenHashMap extends AbstractByte2ShortSortedMap
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Byte2ShortMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		@Override
		public ObjectSortedSet<Byte2ShortMap.Entry> subSet(Byte2ShortMap.Entry fromElement,
				Byte2ShortMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((byte) 0)))
				return Byte2ShortLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			byte curr;
			final byte[] key = Byte2ShortLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Byte2ShortMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractByte2ShortMap.BasicEntry(key[curr], value[curr]));
			}
		}
