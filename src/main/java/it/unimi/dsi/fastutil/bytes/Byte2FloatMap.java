...
public interface Byte2FloatMap extends Byte2FloatFunction, Map<Byte, Float> {
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsentNullable(final byte key,
	 ...
	 * @since 8.0.0
	 */
	default float computeIfPresent(final byte key,
	 ...
	 * @since 8.0.0
	 */
	default float compute(final byte key,
			final java.util.function.BiFunction<? super Byte, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default float merge(final byte key, final float value,
	...
	@Deprecated
	@Override
	default Float computeIfAbsent(final Byte key,
