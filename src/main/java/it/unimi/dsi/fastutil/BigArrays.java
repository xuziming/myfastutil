 ...
 * an array-of-arrays, so a big array of integers is of type {@code int[][]}.
 * Note that {@link #SEGMENT_SIZE} has been chosen so that a single segment is
 * smaller than 2<sup>31</sup> bytes independently of the data type. It might be
 * enlarged in the future.
 *
 * <p>
 * If {@code a} is a big array, {@code a[0]}, {@code a[1]}, &hellip; are called
 * the <em>segments</em> of the big array. All segments, except possibly for the
 * last one, are of length {@link #SEGMENT_SIZE}. Given an index {@code i} into
 * a big array, there is an associated <em>{@linkplain #segment(long)
 * segment}</em> and an associated <em>{@linkplain #displacement(long)
 ...
 * {@link #get(int[][], long)} and {@link #set(int[][], long, int)}), but you
 * can also use the methods {@link #segment(long)}/{@link #displacement(long)}
 * to access entries manually.
 *
 * <p>
 * The intended usage of most of the methods of this class is that they will be
 * imported statically: for example,
 * 
 * <pre>
 ...
 *
 * <p>
 * Dynamic binding will take care of selecting the right method depending on the
 * array type.
 *
 ...
 * {@link #index(int, int) index(segment, displacement)}. You can also use an
 * additional long to keep track of the index.
 *
 * <p>
 ...
 * declare a literal array-of-array (e.g.,
 * <code>new int[][] { { 1, 2 } }</code>). Be warned, however, that this can
 * lead to creating illegal big arrays if for some reason (e.g., stress testing)
 * {@link #SEGMENT_SIZE} is set to a value smaller than the inner array length.
 *
...
public class BigArrays {
	/**
	 * The shift used to compute the segment associated with an index (equivalently,
	 * the logarithm of the segment size).
	 */
	public static final int SEGMENT_SHIFT = 27;
	/**
	 * The current size of a segment (2<sup>27</sup>) is the largest size that makes
	 * the physical memory allocation for a single segment strictly smaller than
	 * 2<sup>31</sup> bytes.
	 */
	 ...
	 *         {@link #displacement(long) displacement(index(segment, displacement))
	 *         == displacement}.
	 */
	 ...
	 *                                        greater than {@code bigArrayLength} or
	 *                                        negative.
	 */
	public static void ensureFromTo(final long bigArrayLength, final long from, final long to) {
		...
		if (to > bigArrayLength)
			throw new ArrayIndexOutOfBoundsException(
					"End index (" + to + ") is greater than big-array length (" + bigArrayLength + ")");
	}
	 ...
	 *                                        greater than {@code bigArrayLength}.
	 */
	public static void ensureOffsetLength(final long bigArrayLength, final long offset, final long length) {
		...
		if (offset + length > bigArrayLength)
			throw new ArrayIndexOutOfBoundsException(
					"Last index (" + (offset + length) + ") is greater than big-array length (" + bigArrayLength + ")");
	}
	 ...
	 *
	 * @param bigArrayLength a big-array length.
	 * @throws IllegalArgumentException if {@code length} is negative, or larger
	 *                                  than or equal to {@link #SEGMENT_SIZE} *
	...
	public static void ensureLength(final long bigArrayLength) {
		...
		if (bigArrayLength >= (long) Integer.MAX_VALUE << SEGMENT_SHIFT)
	 ...
	 * initial ranges are {@code [first, middle)} and {@code [middle, last)}, and
	 * the resulting range is {@code [first, last)}. Elements in the first input
	 * range will precede equal elements in the second.
	 */
	private static void inPlaceMerge(final long from, long mid, final long to, final LongComparator comp,
	 ...
	 * according to the order induced by the specified comparator using mergesort.
	 *
	 * <p>
	 ...
	 * running time is
	 * <i>O</i>(<var>n</var>&nbsp;(log&nbsp;<var>n</var>)<sup>2</sup>), but it does
	 * not allocate additional memory; as a result, it can be used as a generic
	 * sorting algorithm.
	 *
	...
	/**
	 * Sorts the specified range of elements using the specified big swapper and
	 * according to the order induced by the specified comparator using quicksort.
	 *
	 * <p>
	 ...
	 * @param array    a big array.
	 * @param length   the new minimum length for this big array.
	 * @param preserve the number of elements of the big array that must be
	 ...
	 *                                        {@code to}.
	 * @throws ArrayIndexOutOfBoundsException if {@code from} or {@code to} are
	 *                                        greater than the big array length or
	 *                                        negative.
	 */
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copy(final int[][] srcArray, final long srcPos, final int[][] destArray, final long destPos,
			long length) {
		if (destPos <= srcPos) {
			int srcSegment = segment(srcPos);
			int destSegment = segment(destPos);
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyFromBig(final int[][] srcArray, final long srcPos, final int[] destArray, int destPos,
			int length) {
		int srcSegment = segment(srcPos);
		int srcDispl = displacement(srcPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyToBig(final int[] srcArray, int srcPos, final int[][] destArray, final long destPos,
			long length) {
		int destSegment = segment(destPos);
		int destDispl = displacement(destPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copy(final long[][] srcArray, final long srcPos, final long[][] destArray, final long destPos,
			long length) {
		if (destPos <= srcPos) {
			int srcSegment = segment(srcPos);
			int destSegment = segment(destPos);
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyFromBig(final long[][] srcArray, final long srcPos, final long[] destArray, int destPos,
			int length) {
		int srcSegment = segment(srcPos);
		int srcDispl = displacement(srcPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyToBig(final long[] srcArray, int srcPos, final long[][] destArray, final long destPos,
			long length) {
		int destSegment = segment(destPos);
		int destDispl = displacement(destPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copy(final double[][] srcArray, final long srcPos, final double[][] destArray,
			final long destPos, long length) {
		if (destPos <= srcPos) {
			int srcSegment = segment(srcPos);
			int destSegment = segment(destPos);
			int srcDispl = displacement(srcPos);
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyFromBig(final double[][] srcArray, final long srcPos, final double[] destArray, int destPos,
			int length) {
		int srcSegment = segment(srcPos);
		int srcDispl = displacement(srcPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyToBig(final double[] srcArray, int srcPos, final double[][] destArray, final long destPos,
			long length) {
		int destSegment = segment(destPos);
		int destDispl = displacement(destPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copy(final boolean[][] srcArray, final long srcPos, final boolean[][] destArray,
			final long destPos, long length) {
		if (destPos <= srcPos) {
			int srcSegment = segment(srcPos);
			int destSegment = segment(destPos);
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyFromBig(final boolean[][] srcArray, final long srcPos, final boolean[] destArray,
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyToBig(final boolean[] srcArray, int srcPos, final boolean[][] destArray, final long destPos,
			long length) {
		int destSegment = segment(destPos);
		int destDispl = displacement(destPos);
		int l;
	...
	public static boolean[][] forceCapacity(final boolean[][] array, final long length, final long preserve) {
		...
		if (residual != 0) {
			for (int i = valid; i < baseLength - 1; i++)
				base[i] = new boolean[SEGMENT_SIZE];
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copy(final char[][] srcArray, final long srcPos, final char[][] destArray, final long destPos,
			long length) {
		if (destPos <= srcPos) {
			int srcSegment = segment(srcPos);
			int destSegment = segment(destPos);
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyFromBig(final char[][] srcArray, final long srcPos, final char[] destArray, int destPos,
			int length) {
		int srcSegment = segment(srcPos);
		int srcDispl = displacement(srcPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyToBig(final char[] srcArray, int srcPos, final char[][] destArray, final long destPos,
			long length) {
		int destSegment = segment(destPos);
		int destDispl = displacement(destPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copy(final float[][] srcArray, final long srcPos, final float[][] destArray, final long destPos,
			long length) {
		if (destPos <= srcPos) {
			int srcSegment = segment(srcPos);
			int destSegment = segment(destPos);
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyFromBig(final float[][] srcArray, final long srcPos, final float[] destArray, int destPos,
			int length) {
		int srcSegment = segment(srcPos);
		int srcDispl = displacement(srcPos);
		int l;
	 ...
	 * @param length    the number of elements to be copied.
	 */
	public static void copyToBig(final float[] srcArray, int srcPos, final float[][] destArray, final long destPos,
			long length) {
		int destSegment = segment(destPos);
		int destDispl = displacement(destPos);
		int l;
	...
	public static <K> K[][] forceCapacity(final K[][] array, final long length, final long preserve) {
		...
		if (residual != 0) {
			for (int i = valid; i < baseLength - 1; i++)
				base[i] = (K[]) java.lang.reflect.Array.newInstance(componentType.getComponentType(), SEGMENT_SIZE);
			base[baseLength - 1] = (K[]) java.lang.reflect.Array.newInstance(componentType.getComponentType(),
					residual);
		} else
			for (int i = valid; i < baseLength; i++)
				base[i] = (K[]) java.lang.reflect.Array.newInstance(componentType.getComponentType(), SEGMENT_SIZE);
	...
	public static <K> boolean equals(final K[][] a1, final K a2[][]) {
		...
		while (i-- != 0) {
			...
			j = t.length;
			while (j-- != 0)
				if (!java.util.Objects.equals(t[j], u[j]))
					return false;
		}
		return true;
	}
