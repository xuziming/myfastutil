 ...
 *
 */
public class Long2ReferenceRBTreeMap<V> extends AbstractLong2ReferenceSortedMap<V>
	 ...
	 */
	public Long2ReferenceRBTreeMap(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Long2ReferenceRBTreeMap(final Map<? extends Long, ? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ReferenceRBTreeMap(final SortedMap<Long, V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Long2ReferenceRBTreeMap(final Long2ReferenceMap<? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ReferenceRBTreeMap(final Long2ReferenceSortedMap<V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ReferenceRBTreeMap(final long[] k, final V v[], final Comparator<? super Long> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Long2ReferenceRBTreeMap(final long[] k, final V v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2ReferenceRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Long2ReferenceMap.Entry<V>> long2ReferenceEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2ReferenceMap.Entry<V>>() {
				final Comparator<? super Long2ReferenceMap.Entry<V>> comparator = (Comparator<Long2ReferenceMap.Entry<V>>) (
						x, y) -> Long2ReferenceRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
				...
				@Override
				public ObjectBidirectionalIterator<Long2ReferenceMap.Entry<V>> iterator(
						final Long2ReferenceMap.Entry<V> from) {
				...
				@Override
				public ObjectSortedSet<Long2ReferenceMap.Entry<V>> subSet(Long2ReferenceMap.Entry<V> from,
						Long2ReferenceMap.Entry<V> to) {
					return subMap(from.getLongKey(), to.getLongKey()).long2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2ReferenceMap.Entry<V>> headSet(Long2ReferenceMap.Entry<V> to) {
					return headMap(to.getLongKey()).long2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2ReferenceMap.Entry<V>> tailSet(Long2ReferenceMap.Entry<V> from) {
					return tailMap(from.getLongKey()).long2ReferenceEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractLong2ReferenceSortedMap<V> implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final long k) {
			return (bottom || Long2ReferenceRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2ReferenceRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Long))
							return false;
						final Long2ReferenceRBTreeMap.Entry<V> f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2ReferenceRBTreeMap.this.containsKey(k);
		}
		...
		public V get(final long k) {
			final Long2ReferenceRBTreeMap.Entry<V> e;
			final long kk = k;
		...
		public V put(final long k, final V v) {
			...
			final V oldValue = Long2ReferenceRBTreeMap.this.put(k, v);
		...
		public V remove(final long k) {
			...
			final V oldValue = Long2ReferenceRBTreeMap.this.remove(k);
		...
		@Override
		public long firstLongKey() {
			Long2ReferenceRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2ReferenceRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2ReferenceRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2ReferenceRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Long2ReferenceRBTreeMap<V> clone() {
		...
		try {
			c = (Long2ReferenceRBTreeMap<V>) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
