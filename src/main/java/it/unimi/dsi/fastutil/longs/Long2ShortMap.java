...
public interface Long2ShortMap extends Long2ShortFunction, Map<Long, Short> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Long2ShortMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Long2ShortMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final long key, final java.util.function.LongToIntFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsentNullable(final long key,
	 ...
	 * @since 8.0.0
	 */
	default short computeIfPresent(final long key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Short newValue = remappingFunction.apply(Long.valueOf(key), Short.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default short compute(final long key,
			final java.util.function.BiFunction<? super Long, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Short newValue = remappingFunction.apply(Long.valueOf(key), contained ? Short.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default short merge(final long key, final short value,
	...
	@Deprecated
	@Override
	default Short computeIfAbsent(final Long key,
			final java.util.function.Function<? super Long, ? extends Short> mappingFunction) {
