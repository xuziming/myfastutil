...
public class LongArrayList extends AbstractLongList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public void ensureCapacity(final int capacity) {
		if (capacity <= a.length || (a == LongArrays.DEFAULT_EMPTY_ARRAY && capacity <= DEFAULT_INITIAL_CAPACITY))
			return;
	...
	@Override
	public void sort(final LongComparator comp) {
		if (comp == null) {
			LongArrays.stableSort(a, 0, size);
		} else {
			LongArrays.stableSort(a, 0, size, comp);
		}
	}

	@Override
	public void unstableSort(final LongComparator comp) {
		if (comp == null) {
			LongArrays.unstableSort(a, 0, size);
		} else {
			LongArrays.unstableSort(a, 0, size, comp);
		}
	}
