...
public final class LongArrays {
	...
	public static void quickSort(final long[] x, final long[] y, final int from, final int to) {
		...
		while (true) {
			int comparison, t;
			while (b <= c
					&& (comparison = (t = (Long.compare((x[b]), (v)))) == 0 ? (Long.compare((y[b]), (w))) : t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			}
			while (c >= b
					&& (comparison = (t = (Long.compare((x[c]), (v)))) == 0 ? (Long.compare((y[c]), (w))) : t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
			}
	...
	protected static class ForkJoinQuickSort2 extends RecursiveAction {
		...
		protected void compute() {
			...
			while (true) {
				int comparison, t;
				while (b <= c && (comparison = (t = (Long.compare((x[b]), (v)))) == 0 ? (Long.compare((y[b]), (w)))
						: t) <= 0) {
					if (comparison == 0)
						swap(x, y, a++, b);
					b++;
				}
				while (c >= b && (comparison = (t = (Long.compare((x[c]), (v)))) == 0 ? (Long.compare((y[c]), (w)))
						: t) >= 0) {
					if (comparison == 0)
						swap(x, y, c, d--);
					c--;
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final long a[], final int from, final int to) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final long a[]) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final long a[], final int from, final int to, LongComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final long a[], LongComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final long a[], final int from, final int to) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final long a[]) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final long a[], final int from, final int to, LongComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final long a[], LongComparator comp) {
	...
	public static void radixSort(final long[] a, final int from, final int to) {
		...
		while (stackPos > 0) {
			...
			for (int i = first + length; i-- != first;)
				count[(int) ((a[i]) >>> shift & DIGIT_MASK ^ signMask)]++;
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final long[] a, final int from, final int to,
		...
		while (stackPos > 0) {
			...
			// Count keys.
			for (int i = first + length; i-- != first;)
				count[(int) ((a[perm[i]]) >>> shift & DIGIT_MASK ^ signMask)]++;
			// Compute cumulative distribution
			int lastUsed = -1;
			...
			if (stable) {
				for (int i = first + length; i-- != first;)
					support[--pos[(int) ((a[perm[i]]) >>> shift & DIGIT_MASK ^ signMask)]] = perm[i];
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void parallelRadixSortIndirect(final int perm[], final long[] a, final int from, final int to,
			final boolean stable) {
		if (to - from < PARALLEL_RADIXSORT_NO_FORK) {
	...
	private static void insertionSortIndirect(final int[] perm, final long[] a, final long[] b, final int from,
			final int to) {
		for (int i = from; ++i < to;) {
			int t = perm[i];
			int j = i;
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final long[] a, final long[] b, final int from, final int to,
			final boolean stable) {
		if (to - from < RADIXSORT_NO_REC) {
		...
		while (stackPos > 0) {
			...
			// Count keys.
			for (int i = first + length; i-- != first;)
				count[(int) ((k[perm[i]]) >>> shift & DIGIT_MASK ^ signMask)]++;
			// Compute cumulative distribution
			int lastUsed = -1;
			for (int i = 0, p = stable ? 0 : first; i < 1 << DIGIT_BITS; i++) {
				if (count[i] != 0)
			...
			if (stable) {
				for (int i = first + length; i-- != first;)
					support[--pos[(int) ((k[perm[i]]) >>> shift & DIGIT_MASK ^ signMask)]] = perm[i];
				System.arraycopy(support, 0, perm, first, length);
				for (int i = 0, p = first; i < 1 << DIGIT_BITS; i++) {
					if (level < maxLevel && count[i] > 1) {
						if (count[i] < RADIXSORT_NO_REC)
