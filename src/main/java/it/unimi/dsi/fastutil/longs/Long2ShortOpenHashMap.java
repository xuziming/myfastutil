 ...
 * @see HashCommon
 */
public class Long2ShortOpenHashMap extends AbstractLong2ShortMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Long2ShortOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Long2ShortOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Long2ShortOpenHashMap(final Map<? extends Long, ? extends Short> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Long2ShortOpenHashMap(final Map<? extends Long, ? extends Short> m) {
	 ...
	 * @param f the load factor.
	 */
	public Long2ShortOpenHashMap(final Long2ShortMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Long2ShortOpenHashMap(final Long2ShortMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ShortOpenHashMap(final long[] k, final short[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Long2ShortOpenHashMap(final long[] k, final short[] v) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsent(final long k, final java.util.function.LongToIntFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsentNullable(final long k,
			final java.util.function.LongFunction<? extends Short> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfPresent(final long k,
			final java.util.function.BiFunction<? super Long, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Short newValue = remappingFunction.apply(Long.valueOf(k), Short.valueOf(value[pos]));
		if (newValue == null) {
			if (((k) == (0)))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public short compute(final long k,
			final java.util.function.BiFunction<? super Long, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Short newValue = remappingFunction.apply(Long.valueOf(k), pos >= 0 ? Short.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if (((k) == (0)))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public short merge(final long k, final short v,
	...
	final class MapEntry implements Long2ShortMap.Entry, Map.Entry<Long, Short> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()).longValue())) && ((value[index]) == ((e.getValue()).shortValue()));
		}
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final long key[] = Long2ShortOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Long2ShortOpenHashMap.this.remove(wrapped.getLong(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ShortMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0)))
				return Long2ShortOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			long curr;
			final long[] key = Long2ShortOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Long2ShortMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractLong2ShortMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractLong2ShortMap.BasicEntry(key[pos], value[pos]));
		}
