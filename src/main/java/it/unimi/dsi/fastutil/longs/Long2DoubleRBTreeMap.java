 ...
 *
 */
public class Long2DoubleRBTreeMap extends AbstractLong2DoubleSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Long2DoubleRBTreeMap(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Long2DoubleRBTreeMap(final Map<? extends Long, ? extends Double> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2DoubleRBTreeMap(final SortedMap<Long, Double> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Long2DoubleRBTreeMap(final Long2DoubleMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2DoubleRBTreeMap(final Long2DoubleSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2DoubleRBTreeMap(final long[] k, final double v[], final Comparator<? super Long> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Long2DoubleRBTreeMap(final long[] k, final double v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2DoubleRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2DoubleMap.Entry>() {
				final Comparator<? super Long2DoubleMap.Entry> comparator = (Comparator<Long2DoubleMap.Entry>) (x,
						y) -> Long2DoubleRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	...
	private final class Submap extends AbstractLong2DoubleSortedMap implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2DoubleRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2DoubleRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2DoubleMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Double))
							return false;
						final Long2DoubleRBTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2DoubleRBTreeMap.this.containsKey(k);
		}
		...
		public double get(final long k) {
			final Long2DoubleRBTreeMap.Entry e;
			final long kk = k;
		...
		public double put(final long k, final double v) {
			...
			final double oldValue = Long2DoubleRBTreeMap.this.put(k, v);
		...
		public double remove(final long k) {
			...
			final double oldValue = Long2DoubleRBTreeMap.this.remove(k);
		...
		@Override
		public long firstLongKey() {
			Long2DoubleRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2DoubleRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2DoubleRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2DoubleRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
