 ...
 * @see HashCommon
 */
public class Long2ByteLinkedOpenHashMap extends AbstractLong2ByteSortedMap
	...
	public Long2ByteLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Long2ByteLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Long2ByteLinkedOpenHashMap(final Map<? extends Long, ? extends Byte> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Long2ByteLinkedOpenHashMap(final Map<? extends Long, ? extends Byte> m) {
	 ...
	 * @param f the load factor.
	 */
	public Long2ByteLinkedOpenHashMap(final Long2ByteMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Long2ByteLinkedOpenHashMap(final Long2ByteMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ByteLinkedOpenHashMap(final long[] k, final byte[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Long2ByteLinkedOpenHashMap(final long[] k, final byte[] v) {
	 ...
	 */
	public byte getAndMoveToFirst(final long k) {
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public byte getAndMoveToLast(final long k) {
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToFirst(final long k, final byte v) {
		int pos;
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToLast(final long k, final byte v) {
		int pos;
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				long curr;
				final long[] key = Long2ByteLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Long2ByteMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		@Override
		public ObjectSortedSet<Long2ByteMap.Entry> subSet(Long2ByteMap.Entry fromElement,
				Long2ByteMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0)))
				return Long2ByteLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			long curr;
			final long[] key = Long2ByteLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Long2ByteMap.Entry> iterator(final Long2ByteMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Long2ByteMap.Entry> fastIterator(final Long2ByteMap.Entry from) {
		...
		public void forEach(final Consumer<? super Long2ByteMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractLong2ByteMap.BasicEntry(key[curr], value[curr]));
			}
		}
	...
	@Override
	public FastSortedEntrySet long2ByteEntrySet() {
		if (entries == null)
