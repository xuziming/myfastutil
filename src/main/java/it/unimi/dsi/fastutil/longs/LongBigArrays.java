...
public final class LongBigArrays {
	...
	private static void insertionSortIndirect(final long[][] perm, final long[][] a, final long[][] b, final long from,
			final long to) {
	 ...
	 */
	public static void radixSortIndirect(final long[][] perm, final long[][] a, final long[][] b,
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final long[][] perm, final long[][] a, final long[][] b, final long from,
		...
		while (stackPos > 0) {
			...
			for (long i = first + length; i-- != first;)
				count[(int) ((BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)]++;
			...
			if (stable) {
				for (long i = first + length; i-- != first;)
					BigArrays.set(support,
							--pos[(int) ((BigArrays.get(k, BigArrays.get(perm, i))) >>> shift & DIGIT_MASK ^ signMask)],
