 ...
 *
 */
public class Long2ShortRBTreeMap extends AbstractLong2ShortSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Long2ShortRBTreeMap(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Long2ShortRBTreeMap(final Map<? extends Long, ? extends Short> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ShortRBTreeMap(final SortedMap<Long, Short> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Long2ShortRBTreeMap(final Long2ShortMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ShortRBTreeMap(final Long2ShortSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ShortRBTreeMap(final long[] k, final short v[], final Comparator<? super Long> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Long2ShortRBTreeMap(final long[] k, final short v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2ShortRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2ShortMap.Entry>() {
				final Comparator<? super Long2ShortMap.Entry> comparator = (Comparator<Long2ShortMap.Entry>) (x,
						y) -> Long2ShortRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	...
	private final class Submap extends AbstractLong2ShortSortedMap implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2ShortRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2ShortRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Long2ShortRBTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2ShortRBTreeMap.this.containsKey(k);
		}
		...
		public short get(final long k) {
			final Long2ShortRBTreeMap.Entry e;
			final long kk = k;
		...
		public short put(final long k, final short v) {
			...
			final short oldValue = Long2ShortRBTreeMap.this.put(k, v);
		...
		public short remove(final long k) {
			...
			final short oldValue = Long2ShortRBTreeMap.this.remove(k);
		...
		@Override
		public long firstLongKey() {
			Long2ShortRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2ShortRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2ShortRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2ShortRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Long2ShortRBTreeMap clone() {
		...
		try {
			c = (Long2ShortRBTreeMap) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
