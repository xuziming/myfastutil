 ...
 * @see HashCommon
 */
public class Long2ByteOpenCustomHashMap extends AbstractLong2ByteMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Long2ByteOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Long2ByteOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Long2ByteOpenCustomHashMap(final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Long2ByteOpenCustomHashMap(final Map<? extends Long, ? extends Byte> m, final float f,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Long2ByteOpenCustomHashMap(final Map<? extends Long, ? extends Byte> m,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Long2ByteOpenCustomHashMap(final Long2ByteMap m, final float f,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Long2ByteOpenCustomHashMap(final Long2ByteMap m,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ByteOpenCustomHashMap(final long[] k, final byte[] v, final float f,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Long2ByteOpenCustomHashMap(final long[] k, final byte[] v,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	...
	public byte getOrDefault(final long k, final byte defaultValue) {
		if ((strategy.equals((k), (0))))
	...
	public boolean remove(final long k, final byte v) {
		if ((strategy.equals((k), (0)))) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean replace(final long k, final byte oldValue, final byte v) {
		final int pos = find(k);
	...
	/** {@inheritDoc} */
	@Override
	public byte replace(final long k, final byte v) {
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final byte oldValue = value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsentNullable(final long k,
			final java.util.function.LongFunction<? extends Byte> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfPresent(final long k,
			final java.util.function.BiFunction<? super Long, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Byte newValue = remappingFunction.apply(Long.valueOf(k), Byte.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public byte compute(final long k,
			final java.util.function.BiFunction<? super Long, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Byte newValue = remappingFunction.apply(Long.valueOf(k), pos >= 0 ? Byte.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public byte merge(final long k, final byte v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Long2ByteOpenCustomHashMap.this.remove(wrapped.getLong(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ByteMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Long2ByteOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			long curr;
			final long[] key = Long2ByteOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Long2ByteMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractLong2ByteMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractLong2ByteMap.BasicEntry(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Long2ByteMap.Entry> consumer) {
