 ...
 * iterator}.
 */
public class Long2ShortAVLTreeMap extends AbstractLong2ShortSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Long2ShortAVLTreeMap(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Long2ShortAVLTreeMap(final Map<? extends Long, ? extends Short> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ShortAVLTreeMap(final SortedMap<Long, Short> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Long2ShortAVLTreeMap(final Long2ShortMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ShortAVLTreeMap(final Long2ShortSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ShortAVLTreeMap(final long[] k, final short v[], final Comparator<? super Long> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Long2ShortAVLTreeMap(final long[] k, final short v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry extends AbstractLong2ShortMap.BasicEntry implements Cloneable {
		 ...
		 * @param v a value.
		 */
		Entry(final long k, final short v) {
			super(k, v);
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).longValue())) && ((value) == ((e.getValue()).shortValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2ShortAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	 ...
	 * This class can iterate in both directions on a threaded tree.
	 */
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Long2ShortMap.Entry> {
		EntryIterator() {
		}
	...
	public ObjectSortedSet<Long2ShortMap.Entry> long2ShortEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2ShortMap.Entry>() {
				final Comparator<? super Long2ShortMap.Entry> comparator = (Comparator<Long2ShortMap.Entry>) (x,
						y) -> Long2ShortAVLTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
				...
				@Override
				public ObjectBidirectionalIterator<Long2ShortMap.Entry> iterator(final Long2ShortMap.Entry from) {
				...
				public ObjectSortedSet<Long2ShortMap.Entry> subSet(Long2ShortMap.Entry from, Long2ShortMap.Entry to) {
					return subMap(from.getLongKey(), to.getLongKey()).long2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2ShortMap.Entry> headSet(Long2ShortMap.Entry to) {
					return headMap(to.getLongKey()).long2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2ShortMap.Entry> tailSet(Long2ShortMap.Entry from) {
					return tailMap(from.getLongKey()).long2ShortEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractLong2ShortSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final long k) {
			return (bottom || Long2ShortAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2ShortAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Long2ShortAVLTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
					...
					@Override
					public ObjectSortedSet<Long2ShortMap.Entry> subSet(Long2ShortMap.Entry from,
							Long2ShortMap.Entry to) {
						return subMap(from.getLongKey(), to.getLongKey()).long2ShortEntrySet();
					}
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2ShortAVLTreeMap.this.containsKey(k);
		}
		...
		public short get(final long k) {
			final Long2ShortAVLTreeMap.Entry e;
			final long kk = k;
		...
		public short put(final long k, final short v) {
			...
			final short oldValue = Long2ShortAVLTreeMap.this.put(k, v);
		...
		public short remove(final long k) {
			...
			final short oldValue = Long2ShortAVLTreeMap.this.remove(k);
		...
		@Override
		public long firstLongKey() {
			Long2ShortAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2ShortAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2ShortAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2ShortAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry readTree(final java.io.ObjectInputStream s, final int n, final Entry pred, final Entry succ)
		...
		if (n == 1) {
			final Entry top = new Entry(s.readLong(), s.readShort());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
