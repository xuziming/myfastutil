 ...
 *
 */
public class Long2BooleanRBTreeMap extends AbstractLong2BooleanSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2BooleanRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2BooleanMap.Entry>() {
				final Comparator<? super Long2BooleanMap.Entry> comparator = (Comparator<Long2BooleanMap.Entry>) (x,
						y) -> Long2BooleanRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	...
	private final class Submap extends AbstractLong2BooleanSortedMap implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2BooleanRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2BooleanRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2BooleanMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Long2BooleanRBTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2BooleanRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public long firstLongKey() {
			Long2BooleanRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2BooleanRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2BooleanRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2BooleanRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
