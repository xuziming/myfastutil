 ...
 * iterator}.
 */
public class Long2BooleanAVLTreeMap extends AbstractLong2BooleanSortedMap implements java.io.Serializable, Cloneable {
	...
	private static final class Entry extends AbstractLong2BooleanMap.BasicEntry implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).longValue())) && ((value) == ((e.getValue()).booleanValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2BooleanAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Long2BooleanMap.Entry> long2BooleanEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2BooleanMap.Entry>() {
				final Comparator<? super Long2BooleanMap.Entry> comparator = (Comparator<Long2BooleanMap.Entry>) (x,
						y) -> Long2BooleanAVLTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
				...
				@Override
				public ObjectSortedSet<Long2BooleanMap.Entry> subSet(Long2BooleanMap.Entry from,
						Long2BooleanMap.Entry to) {
					return subMap(from.getLongKey(), to.getLongKey()).long2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2BooleanMap.Entry> headSet(Long2BooleanMap.Entry to) {
					return headMap(to.getLongKey()).long2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2BooleanMap.Entry> tailSet(Long2BooleanMap.Entry from) {
					return tailMap(from.getLongKey()).long2BooleanEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractLong2BooleanSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final long k) {
			return (bottom || Long2BooleanAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2BooleanAVLTreeMap.this.compare(k, to) < 0);
		}
		...
		public ObjectSortedSet<Long2BooleanMap.Entry> long2BooleanEntrySet() {
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2BooleanMap.Entry>() {
					...
					@Override
					public ObjectBidirectionalIterator<Long2BooleanMap.Entry> iterator(
							final Long2BooleanMap.Entry from) {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Long2BooleanAVLTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2BooleanAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public long firstLongKey() {
			Long2BooleanAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2BooleanAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2BooleanAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2BooleanAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
