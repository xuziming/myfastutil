...
public interface Long2BooleanMap extends Long2BooleanFunction, Map<Long, Boolean> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Long2BooleanMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Long2BooleanMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsent(final long key, final java.util.function.LongPredicate mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfAbsentNullable(final long key,
	 ...
	 * @since 8.0.0
	 */
	default boolean computeIfPresent(final long key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Boolean newValue = remappingFunction.apply(Long.valueOf(key), Boolean.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default boolean compute(final long key,
			final java.util.function.BiFunction<? super Long, ? super Boolean, ? extends Boolean> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final boolean oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Boolean newValue = remappingFunction.apply(Long.valueOf(key),
	 ...
	 * @since 8.0.0
	 */
	default boolean merge(final long key, final boolean value,
	...
	@Deprecated
	@Override
	default Boolean computeIfAbsent(final Long key,
	 ...
	 */
	interface Entry extends Map.Entry<Long, Boolean> {
		/**
