 ...
 */
public class LongRBTreeSet extends AbstractLongSortedSet implements java.io.Serializable, Cloneable, LongSortedSet {
	...
	private class SetIterator implements LongListIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			LongRBTreeSet.this.remove(curr.key);
			curr = null;
		}
	}
	...
	private final class Subset extends AbstractLongSortedSet implements java.io.Serializable, LongSortedSet {
		...
		final boolean in(final long k) {
			return (bottom || LongRBTreeSet.this.compare(k, from) >= 0)
					&& (top || LongRBTreeSet.this.compare(k, to) < 0);
		}
		...
		@Override
		public long firstLong() {
			LongRBTreeSet.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLong() {
			LongRBTreeSet.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && LongRBTreeSet.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && LongRBTreeSet.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	}
