 ...
 * @see HashCommon
 */
public class Long2ObjectLinkedOpenHashMap<V> extends AbstractLong2ObjectSortedMap<V>
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Long2ObjectMap.Entry<V>>
		...
		@Override
		public ObjectSortedSet<Long2ObjectMap.Entry<V>> subSet(Long2ObjectMap.Entry<V> fromElement,
				Long2ObjectMap.Entry<V> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0)))
				return Long2ObjectLinkedOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			long curr;
			final long[] key = Long2ObjectLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Long2ObjectMap.Entry<V>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractLong2ObjectMap.BasicEntry<V>(key[curr], value[curr]));
			}
		}
