 ...
 *
 */
public class Long2IntRBTreeMap extends AbstractLong2IntSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Long2IntRBTreeMap(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Long2IntRBTreeMap(final Map<? extends Long, ? extends Integer> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2IntRBTreeMap(final SortedMap<Long, Integer> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Long2IntRBTreeMap(final Long2IntMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2IntRBTreeMap(final Long2IntSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2IntRBTreeMap(final long[] k, final int v[], final Comparator<? super Long> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Long2IntRBTreeMap(final long[] k, final int v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2IntRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2IntMap.Entry>() {
				final Comparator<? super Long2IntMap.Entry> comparator = (Comparator<Long2IntMap.Entry>) (x,
						y) -> Long2IntRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	...
	private final class Submap extends AbstractLong2IntSortedMap implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2IntRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2IntRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2IntMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Integer))
							return false;
						final Long2IntRBTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2IntRBTreeMap.this.containsKey(k);
		}
		...
		public int get(final long k) {
			final Long2IntRBTreeMap.Entry e;
			final long kk = k;
		...
		public int put(final long k, final int v) {
			...
			final int oldValue = Long2IntRBTreeMap.this.put(k, v);
		...
		public int remove(final long k) {
			...
			final int oldValue = Long2IntRBTreeMap.this.remove(k);
		...
		@Override
		public long firstLongKey() {
			Long2IntRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2IntRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2IntRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2IntRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
