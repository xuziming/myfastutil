...
public class Long2IntOpenCustomHashMap extends AbstractLong2IntMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Long2IntOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Long2IntOpenCustomHashMap(final Map<? extends Long, ? extends Integer> m, final float f,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Long2IntOpenCustomHashMap(final Map<? extends Long, ? extends Integer> m,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Long2IntOpenCustomHashMap(final Long2IntMap m, final float f,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Long2IntOpenCustomHashMap(final Long2IntMap m,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2IntOpenCustomHashMap(final long[] k, final int[] v, final float f,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Long2IntOpenCustomHashMap(final long[] k, final int[] v,
			final it.unimi.dsi.fastutil.longs.LongHash.Strategy strategy) {
	...
	public int computeIfPresent(final long k,
		...
		if (pos < 0)
			return defRetValue;
		final Integer newValue = remappingFunction.apply(Long.valueOf(k), Integer.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	public int compute(final long k,
		...
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Integer newValue = remappingFunction.apply(Long.valueOf(k),
	...
	private class MapIterator {
		 ...
		 */
		boolean mustReturnNullKey = Long2IntOpenCustomHashMap.this.containsNullKey;
		/**
		...
		public void remove() {
			...
			if (last == n) {
				...
				Long2IntOpenCustomHashMap.this.remove(wrapped.getLong(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2IntMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Long2IntOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			long curr;
			final long[] key = Long2IntOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Long2IntMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractLong2IntMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractLong2IntMap.BasicEntry(key[pos], value[pos]));
		}
