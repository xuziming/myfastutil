...
public interface Long2ByteMap extends Long2ByteFunction, Map<Long, Byte> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Long2ByteMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Long2ByteMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final long key, final java.util.function.LongToIntFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsentNullable(final long key,
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfPresent(final long key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Byte newValue = remappingFunction.apply(Long.valueOf(key), Byte.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default byte compute(final long key,
			final java.util.function.BiFunction<? super Long, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Byte newValue = remappingFunction.apply(Long.valueOf(key), contained ? Byte.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default byte merge(final long key, final byte value,
	...
	@Deprecated
	@Override
	default Byte computeIfAbsent(final Long key,
	 ...
	 */
	interface Entry extends Map.Entry<Long, Byte> {
		/**
