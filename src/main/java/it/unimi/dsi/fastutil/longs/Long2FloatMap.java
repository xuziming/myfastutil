...
public interface Long2FloatMap extends Long2FloatFunction, Map<Long, Float> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Long2FloatMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Long2FloatMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default float getOrDefault(final long key, final float defaultValue) {
		final float v;
	 ...
	 * @since 8.0.0
	 */
	default float putIfAbsent(final long key, final float value) {
	 ...
	 * @since 8.0.0
	 */
	default boolean remove(final long key, final float value) {
		final float curValue = get(key);
		if (!(Float.floatToIntBits(curValue) == Float.floatToIntBits(value))
				|| (curValue == defaultReturnValue() && !containsKey(key)))
			return false;
		remove(key);
		return true;
	}
	 ...
	 * @since 8.0.0
	 */
	default boolean replace(final long key, final float oldValue, final float newValue) {
		final float curValue = get(key);
	 ...
	 * @since 8.0.0
	 */
	default float replace(final long key, final float value) {
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final long key, final java.util.function.LongToDoubleFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsentNullable(final long key,
			final java.util.function.LongFunction<? extends Float> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsentPartial(final long key, final Long2FloatFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final float v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default float computeIfPresent(final long key,
			final java.util.function.BiFunction<? super Long, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Float newValue = remappingFunction.apply(Long.valueOf(key), Float.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default float compute(final long key,
			final java.util.function.BiFunction<? super Long, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Float newValue = remappingFunction.apply(Long.valueOf(key), contained ? Float.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default float merge(final long key, final float value,
	...
	@Deprecated
	@Override
	default Float computeIfAbsent(final Long key,
			final java.util.function.Function<? super Long, ? extends Float> mappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Long, Float> {
		/**
