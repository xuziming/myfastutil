 ...
 * iterator}.
 */
public class Long2DoubleAVLTreeMap extends AbstractLong2DoubleSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Long2DoubleAVLTreeMap(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Long2DoubleAVLTreeMap(final Map<? extends Long, ? extends Double> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2DoubleAVLTreeMap(final SortedMap<Long, Double> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Long2DoubleAVLTreeMap(final Long2DoubleMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2DoubleAVLTreeMap(final Long2DoubleSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2DoubleAVLTreeMap(final long[] k, final double v[], final Comparator<? super Long> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Long2DoubleAVLTreeMap(final long[] k, final double v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2DoubleAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Long2DoubleMap.Entry> long2DoubleEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2DoubleMap.Entry>() {
				final Comparator<? super Long2DoubleMap.Entry> comparator = (Comparator<Long2DoubleMap.Entry>) (x,
						y) -> Long2DoubleAVLTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
				...
				public ObjectSortedSet<Long2DoubleMap.Entry> headSet(Long2DoubleMap.Entry to) {
					return headMap(to.getLongKey()).long2DoubleEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2DoubleMap.Entry> tailSet(Long2DoubleMap.Entry from) {
					return tailMap(from.getLongKey()).long2DoubleEntrySet();
				}
			};
		return entries;
	}
	...
	private final class Submap extends AbstractLong2DoubleSortedMap implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2DoubleAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2DoubleAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2DoubleMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Double))
							return false;
						final Long2DoubleAVLTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2DoubleAVLTreeMap.this.containsKey(k);
		}
		...
		public double get(final long k) {
			final Long2DoubleAVLTreeMap.Entry e;
			final long kk = k;
		...
		public double put(final long k, final double v) {
			...
			final double oldValue = Long2DoubleAVLTreeMap.this.put(k, v);
		...
		public double remove(final long k) {
			...
			final double oldValue = Long2DoubleAVLTreeMap.this.remove(k);
		...
		@Override
		public long firstLongKey() {
			Long2DoubleAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2DoubleAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2DoubleAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2DoubleAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
