...
public class Long2IntAVLTreeMap extends AbstractLong2IntSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2IntMap.Entry>() {
				final Comparator<? super Long2IntMap.Entry> comparator = (Comparator<Long2IntMap.Entry>) (x,
						y) -> Long2IntAVLTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	...
	private final class Submap extends AbstractLong2IntSortedMap implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2IntAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2IntAVLTreeMap.this.compare(k, to) < 0);
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2IntAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2IntAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
