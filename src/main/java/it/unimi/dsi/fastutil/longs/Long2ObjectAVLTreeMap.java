 ...
 * iterator}.
 */
public class Long2ObjectAVLTreeMap<V> extends AbstractLong2ObjectSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2ObjectAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2ObjectMap.Entry<V>>() {
				final Comparator<? super Long2ObjectMap.Entry<V>> comparator = (Comparator<Long2ObjectMap.Entry<V>>) (x,
						y) -> Long2ObjectAVLTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	...
	private final class Submap extends AbstractLong2ObjectSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2ObjectAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2ObjectAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2ObjectMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Long))
							return false;
						final Long2ObjectAVLTreeMap.Entry<V> f = findKey(((Long) (e.getKey())).longValue());
		...
		@Override
		public long firstLongKey() {
			Long2ObjectAVLTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2ObjectAVLTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2ObjectAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2ObjectAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Long2ObjectAVLTreeMap<V> clone() {
		...
		try {
			c = (Long2ObjectAVLTreeMap<V>) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
