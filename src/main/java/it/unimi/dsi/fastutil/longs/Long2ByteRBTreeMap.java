 ...
 *
 */
public class Long2ByteRBTreeMap extends AbstractLong2ByteSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Long2ByteRBTreeMap(final Comparator<? super Long> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Long2ByteRBTreeMap(final Map<? extends Long, ? extends Byte> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ByteRBTreeMap(final SortedMap<Long, Byte> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Long2ByteRBTreeMap(final Long2ByteMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Long2ByteRBTreeMap(final Long2ByteSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ByteRBTreeMap(final long[] k, final byte v[], final Comparator<? super Long> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Long2ByteRBTreeMap(final long[] k, final byte v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry extends AbstractLong2ByteMap.BasicEntry implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).longValue())) && ((value) == ((e.getValue()).byteValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2ByteRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Long2ByteMap.Entry> long2ByteEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2ByteMap.Entry>() {
				final Comparator<? super Long2ByteMap.Entry> comparator = (Comparator<Long2ByteMap.Entry>) (x,
						y) -> Long2ByteRBTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
				...
				public ObjectSortedSet<Long2ByteMap.Entry> subSet(Long2ByteMap.Entry from, Long2ByteMap.Entry to) {
					return subMap(from.getLongKey(), to.getLongKey()).long2ByteEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2ByteMap.Entry> headSet(Long2ByteMap.Entry to) {
					return headMap(to.getLongKey()).long2ByteEntrySet();
				}

				@Override
				public ObjectSortedSet<Long2ByteMap.Entry> tailSet(Long2ByteMap.Entry from) {
					return tailMap(from.getLongKey()).long2ByteEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractLong2ByteSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final long k) {
			return (bottom || Long2ByteRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2ByteRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2ByteMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Byte))
							return false;
						final Long2ByteRBTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2ByteRBTreeMap.this.containsKey(k);
		}
		...
		public byte get(final long k) {
			final Long2ByteRBTreeMap.Entry e;
			final long kk = k;
		...
		public byte put(final long k, final byte v) {
			...
			final byte oldValue = Long2ByteRBTreeMap.this.put(k, v);
		...
		public byte remove(final long k) {
			...
			final byte oldValue = Long2ByteRBTreeMap.this.remove(k);
		...
		@Override
		public long firstLongKey() {
			Long2ByteRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2ByteRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2ByteRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2ByteRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
