 ...
 * iterator}.
 */
public class Long2ByteAVLTreeMap extends AbstractLong2ByteSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Long2ByteAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Long2ByteMap.Entry>() {
				final Comparator<? super Long2ByteMap.Entry> comparator = (Comparator<Long2ByteMap.Entry>) (x,
						y) -> Long2ByteAVLTreeMap.this.actualComparator.compare(x.getLongKey(), y.getLongKey());
	...
	private final class Submap extends AbstractLong2ByteSortedMap implements java.io.Serializable {
		...
		final boolean in(final long k) {
			return (bottom || Long2ByteAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Long2ByteAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Long2ByteMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Byte))
							return false;
						final Long2ByteAVLTreeMap.Entry f = findKey(((Long) (e.getKey())).longValue());
		...
		public boolean containsKey(final long k) {
			return in(k) && Long2ByteAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public long firstLongKey() {
			Long2ByteAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public long lastLongKey() {
			Long2ByteAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Long2ByteAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Long2ByteAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
