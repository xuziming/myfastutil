...
public class LongHeapSemiIndirectPriorityQueue implements LongIndirectPriorityQueue {
	 ...
	 *                 natural order.
	 */
	public LongHeapSemiIndirectPriorityQueue(final long[] refArray, final int[] a, int size, final LongComparator c) {
		this(refArray, 0, c);
		this.heap = a;
		this.size = size;
	 ...
	 * @param size     the number of elements to be included in the queue.
	 */
	public LongHeapSemiIndirectPriorityQueue(final long[] refArray, final int[] a, int size) {
	...
	public void enqueue(int x) {
		...
		heap[size++] = x;
		LongSemiIndirectHeaps.upHeap(refArray, heap, size, size - 1, c);
	}
	...
	public int dequeue() {
		...
		heap[0] = heap[--size];
		if (size != 0)
			LongSemiIndirectHeaps.downHeap(refArray, heap, size, 0, c);
		return result;
	}
	...
	public void allChanged() {
		LongSemiIndirectHeaps.makeHeap(refArray, heap, size, c);
	}
	...
	public int front(final int[] a) {
		return c == null ? LongSemiIndirectHeaps.front(refArray, heap, size, a)
				: LongSemiIndirectHeaps.front(refArray, heap, size, a, c);
	}
