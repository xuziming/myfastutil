 ...
 * @see HashCommon
 */
public class Long2ByteOpenHashMap extends AbstractLong2ByteMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Long2ByteOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Long2ByteOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Long2ByteOpenHashMap(final Map<? extends Long, ? extends Byte> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Long2ByteOpenHashMap(final Map<? extends Long, ? extends Byte> m) {
	 ...
	 * @param f the load factor.
	 */
	public Long2ByteOpenHashMap(final Long2ByteMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Long2ByteOpenHashMap(final Long2ByteMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Long2ByteOpenHashMap(final long[] k, final byte[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Long2ByteOpenHashMap(final long[] k, final byte[] v) {
	...
	final class MapEntry implements Long2ByteMap.Entry, Map.Entry<Long, Byte> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()).longValue())) && ((value[index]) == ((e.getValue()).byteValue()));
		}
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final long key[] = Long2ByteOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Long2ByteOpenHashMap.this.remove(wrapped.getLong(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Long2ByteMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0)))
				return Long2ByteOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			long curr;
			final long[] key = Long2ByteOpenHashMap.this.key;
			int pos;
			// The starting point.
