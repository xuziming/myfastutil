...
public interface Long2ReferenceMap<V> extends Long2ReferenceFunction<V>, Map<Long, V> {
	 ...
	 */
	interface FastEntrySet<V> extends ObjectSet<Long2ReferenceMap.Entry<V>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Long2ReferenceMap.Entry<V>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsentPartial(final long key, final Long2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default V computeIfPresent(final long key,
	 ...
	 * @since 8.0.0
	 */
	default V compute(final long key,
			final java.util.function.BiFunction<? super Long, ? super V, ? extends V> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final V oldValue = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default V merge(final long key, final V value,
	...
	@Deprecated
	@Override
	default V computeIfAbsent(final Long key,
