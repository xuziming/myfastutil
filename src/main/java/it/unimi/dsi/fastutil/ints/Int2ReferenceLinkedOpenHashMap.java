 ...
 * @see HashCommon
 */
public class Int2ReferenceLinkedOpenHashMap<V> extends AbstractInt2ReferenceSortedMap<V>
	...
	public Int2ReferenceLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Int2ReferenceLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Int2ReferenceLinkedOpenHashMap(final Map<? extends Integer, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Int2ReferenceLinkedOpenHashMap(final Map<? extends Integer, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public Int2ReferenceLinkedOpenHashMap(final Int2ReferenceMap<V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Int2ReferenceLinkedOpenHashMap(final Int2ReferenceMap<V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2ReferenceLinkedOpenHashMap(final int[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Int2ReferenceLinkedOpenHashMap(final int[] k, final V[] v) {
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				int curr;
				final int[] key = Int2ReferenceLinkedOpenHashMap.this.key;
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Int2ReferenceMap.Entry<V>> {
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Int2ReferenceMap.Entry<V>>
		...
		@Override
		public ObjectSortedSet<Int2ReferenceMap.Entry<V>> subSet(Int2ReferenceMap.Entry<V> fromElement,
				Int2ReferenceMap.Entry<V> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0)))
				return Int2ReferenceLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2ReferenceLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Int2ReferenceMap.Entry<V>> iterator(final Int2ReferenceMap.Entry<V> from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Int2ReferenceMap.Entry<V>> fastIterator(final Int2ReferenceMap.Entry<V> from) {
		...
		public void forEach(final Consumer<? super Int2ReferenceMap.Entry<V>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractInt2ReferenceMap.BasicEntry<V>(key[curr], value[curr]));
			}
		}
