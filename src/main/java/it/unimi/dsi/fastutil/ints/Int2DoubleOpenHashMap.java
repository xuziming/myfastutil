...
public class Int2DoubleOpenHashMap extends AbstractInt2DoubleMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2DoubleOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Int2DoubleOpenHashMap(final int expected) {
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final int key[] = Int2DoubleOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2DoubleOpenHashMap.this.remove(wrapped.getInt(-pos - 1));
