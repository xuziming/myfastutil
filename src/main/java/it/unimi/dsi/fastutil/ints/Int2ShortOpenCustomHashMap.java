 ...
 * @see HashCommon
 */
public class Int2ShortOpenCustomHashMap extends AbstractInt2ShortMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2ShortOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ShortOpenCustomHashMap(final int expected, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ShortOpenCustomHashMap(final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ShortOpenCustomHashMap(final Map<? extends Integer, ? extends Short> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ShortOpenCustomHashMap(final Map<? extends Integer, ? extends Short> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ShortOpenCustomHashMap(final Int2ShortMap m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ShortOpenCustomHashMap(final Int2ShortMap m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2ShortOpenCustomHashMap(final int[] k, final short[] v, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Int2ShortOpenCustomHashMap(final int[] k, final short[] v,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsent(final int k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsentNullable(final int k,
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfPresent(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Short newValue = remappingFunction.apply(Integer.valueOf(k), Short.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public short compute(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Short newValue = remappingFunction.apply(Integer.valueOf(k), pos >= 0 ? Short.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public short merge(final int k, final short v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final int key[] = Int2ShortOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2ShortOpenCustomHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2ShortMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Int2ShortOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2ShortOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Int2ShortMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractInt2ShortMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractInt2ShortMap.BasicEntry(key[pos], value[pos]));
		}
