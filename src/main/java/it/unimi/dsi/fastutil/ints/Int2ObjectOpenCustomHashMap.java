...
public class Int2ObjectOpenCustomHashMap<V> extends AbstractInt2ObjectMap<V>
	...
	public Int2ObjectOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ObjectOpenCustomHashMap(final Map<? extends Integer, ? extends V> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ObjectOpenCustomHashMap(final Map<? extends Integer, ? extends V> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ObjectOpenCustomHashMap(final Int2ObjectMap<V> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ObjectOpenCustomHashMap(final Int2ObjectMap<V> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2ObjectOpenCustomHashMap(final int[] k, final V[] v, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Int2ObjectOpenCustomHashMap(final int[] k, final V[] v,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Int2ObjectOpenCustomHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			int curr;
			final int[] key = Int2ObjectOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Int2ObjectMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractInt2ObjectMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractInt2ObjectMap.BasicEntry<V>(key[pos], value[pos]));
		}
