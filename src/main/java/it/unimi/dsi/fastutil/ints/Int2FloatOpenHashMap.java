 ...
 * @see HashCommon
 */
public class Int2FloatOpenHashMap extends AbstractInt2FloatMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2FloatOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Int2FloatOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Int2FloatOpenHashMap(final Map<? extends Integer, ? extends Float> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Int2FloatOpenHashMap(final Map<? extends Integer, ? extends Float> m) {
	 ...
	 * @param f the load factor.
	 */
	public Int2FloatOpenHashMap(final Int2FloatMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Int2FloatOpenHashMap(final Int2FloatMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2FloatOpenHashMap(final int[] k, final float[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Int2FloatOpenHashMap(final int[] k, final float[] v) {
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final int key[] = Int2FloatOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2FloatOpenHashMap.this.remove(wrapped.getInt(-pos - 1));
