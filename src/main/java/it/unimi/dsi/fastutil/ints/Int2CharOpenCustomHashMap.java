 ...
 * @see HashCommon
 */
public class Int2CharOpenCustomHashMap extends AbstractInt2CharMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2CharOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Int2CharOpenCustomHashMap(final int expected, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2CharOpenCustomHashMap(final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2CharOpenCustomHashMap(final Map<? extends Integer, ? extends Character> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2CharOpenCustomHashMap(final Map<? extends Integer, ? extends Character> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2CharOpenCustomHashMap(final Int2CharMap m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Int2CharOpenCustomHashMap(final Int2CharMap m, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2CharOpenCustomHashMap(final int[] k, final char[] v, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Int2CharOpenCustomHashMap(final int[] k, final char[] v,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfAbsent(final int k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfAbsentNullable(final int k,
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfPresent(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Character newValue = remappingFunction.apply(Integer.valueOf(k), Character.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public char compute(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Character newValue = remappingFunction.apply(Integer.valueOf(k),
	...
	/** {@inheritDoc} */
	@Override
	public char merge(final int k, final char v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final int key[] = Int2CharOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2CharOpenCustomHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2CharMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Int2CharOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2CharOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Int2CharMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractInt2CharMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractInt2CharMap.BasicEntry(key[pos], value[pos]));
		}
