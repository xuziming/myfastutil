...
public interface Int2ReferenceMap<V> extends Int2ReferenceFunction<V>, Map<Integer, V> {
	 ...
	 */
	interface FastEntrySet<V> extends ObjectSet<Int2ReferenceMap.Entry<V>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Int2ReferenceMap.Entry<V>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsentPartial(final int key, final Int2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
