...
public interface Int2ByteMap extends Int2ByteFunction, Map<Integer, Byte> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Int2ByteMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Int2ByteMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final int key, final java.util.function.IntUnaryOperator mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsentNullable(final int key,
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfPresent(final int key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Byte newValue = remappingFunction.apply(Integer.valueOf(key), Byte.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default byte compute(final int key,
			final java.util.function.BiFunction<? super Integer, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Byte newValue = remappingFunction.apply(Integer.valueOf(key), contained ? Byte.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default byte merge(final int key, final byte value,
	...
	@Deprecated
	@Override
	default Byte computeIfAbsent(final Integer key,
	...
	@Deprecated
	@Override
	default Byte computeIfPresent(final Integer key,
			final java.util.function.BiFunction<? super Integer, ? super Byte, ? extends Byte> remappingFunction) {
	...
	@Deprecated
	@Override
	default Byte compute(final Integer key,
			final java.util.function.BiFunction<? super Integer, ? super Byte, ? extends Byte> remappingFunction) {
	...
	@Deprecated
	@Override
	default Byte merge(final Integer key, final Byte value,
			final java.util.function.BiFunction<? super Byte, ? super Byte, ? extends Byte> remappingFunction) {
