 ...
 * @see HashCommon
 */
public class Int2LongOpenCustomHashMap extends AbstractInt2LongMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2LongOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Int2LongOpenCustomHashMap(final int expected, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2LongOpenCustomHashMap(final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2LongOpenCustomHashMap(final Map<? extends Integer, ? extends Long> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2LongOpenCustomHashMap(final Map<? extends Integer, ? extends Long> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2LongOpenCustomHashMap(final Int2LongMap m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Int2LongOpenCustomHashMap(final Int2LongMap m, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2LongOpenCustomHashMap(final int[] k, final long[] v, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Int2LongOpenCustomHashMap(final int[] k, final long[] v,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	...
	private void insert(final int pos, final int k, final long v) {
		if (pos == n)
	...
	public long getOrDefault(final int k, final long defaultValue) {
		if ((strategy.equals((k), (0))))
	...
	/** {@inheritDoc} */
	@Override
	public long putIfAbsent(final int k, final long v) {
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
		insert(-pos - 1, k, v);
	...
	public boolean remove(final int k, final long v) {
		if ((strategy.equals((k), (0)))) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean replace(final int k, final long oldValue, final long v) {
		final int pos = find(k);
	...
	/** {@inheritDoc} */
	@Override
	public long replace(final int k, final long v) {
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final long oldValue = value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsent(final int k, final java.util.function.IntToLongFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsentNullable(final int k,
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfPresent(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Long newValue = remappingFunction.apply(Integer.valueOf(k), Long.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public long compute(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Long newValue = remappingFunction.apply(Integer.valueOf(k), pos >= 0 ? Long.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public long merge(final int k, final long v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2LongOpenCustomHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2LongMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Int2LongOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2LongOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Int2LongMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractInt2LongMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractInt2LongMap.BasicEntry(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Int2LongMap.Entry> consumer) {
