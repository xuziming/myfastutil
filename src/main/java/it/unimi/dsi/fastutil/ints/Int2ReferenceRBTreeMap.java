 ...
 *
 */
public class Int2ReferenceRBTreeMap<V> extends AbstractInt2ReferenceSortedMap<V>
	 ...
	 */
	public Int2ReferenceRBTreeMap(final Comparator<? super Integer> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Int2ReferenceRBTreeMap(final Map<? extends Integer, ? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Int2ReferenceRBTreeMap(final SortedMap<Integer, V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Int2ReferenceRBTreeMap(final Int2ReferenceMap<? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Int2ReferenceRBTreeMap(final Int2ReferenceSortedMap<V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2ReferenceRBTreeMap(final int[] k, final V v[], final Comparator<? super Integer> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Int2ReferenceRBTreeMap(final int[] k, final V v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry<V> extends AbstractInt2ReferenceMap.BasicEntry<V> implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).intValue())) && ((value) == ((e.getValue())));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Int2ReferenceRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Int2ReferenceMap.Entry<V>> int2ReferenceEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Int2ReferenceMap.Entry<V>>() {
				final Comparator<? super Int2ReferenceMap.Entry<V>> comparator = (Comparator<Int2ReferenceMap.Entry<V>>) (
						x, y) -> Int2ReferenceRBTreeMap.this.actualComparator.compare(x.getIntKey(), y.getIntKey());
				...
				@Override
				public ObjectBidirectionalIterator<Int2ReferenceMap.Entry<V>> iterator(
						final Int2ReferenceMap.Entry<V> from) {
				...
				public boolean contains(final Object o) {
					...
					if (e.getKey() == null || !(e.getKey() instanceof Integer))
						return false;
					final Entry<V> f = findKey(((Integer) (e.getKey())).intValue());
					return e.equals(f);
				}
				...
				@Override
				public ObjectSortedSet<Int2ReferenceMap.Entry<V>> subSet(Int2ReferenceMap.Entry<V> from,
						Int2ReferenceMap.Entry<V> to) {
					return subMap(from.getIntKey(), to.getIntKey()).int2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Int2ReferenceMap.Entry<V>> headSet(Int2ReferenceMap.Entry<V> to) {
					return headMap(to.getIntKey()).int2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Int2ReferenceMap.Entry<V>> tailSet(Int2ReferenceMap.Entry<V> from) {
					return tailMap(from.getIntKey()).int2ReferenceEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractInt2ReferenceSortedMap<V> implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final int k) {
			return (bottom || Int2ReferenceRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Int2ReferenceRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Int2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Integer))
							return false;
						final Int2ReferenceRBTreeMap.Entry<V> f = findKey(((Integer) (e.getKey())).intValue());
		...
		public boolean containsKey(final int k) {
			return in(k) && Int2ReferenceRBTreeMap.this.containsKey(k);
		}
		...
		public V get(final int k) {
			final Int2ReferenceRBTreeMap.Entry<V> e;
			final int kk = k;
		...
		public V put(final int k, final V v) {
			...
			final V oldValue = Int2ReferenceRBTreeMap.this.put(k, v);
		...
		public V remove(final int k) {
			...
			final V oldValue = Int2ReferenceRBTreeMap.this.remove(k);
		...
		@Override
		public int firstIntKey() {
			Int2ReferenceRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public int lastIntKey() {
			Int2ReferenceRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Int2ReferenceRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Int2ReferenceRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry<V> readTree(final java.io.ObjectInputStream s, final int n, final Entry<V> pred, final Entry<V> succ)
		...
		if (n == 1) {
			final Entry<V> top = new Entry<>(s.readInt(), (V) s.readObject());
			top.pred(pred);
			top.succ(succ);
			top.black(true);
			return top;
		...
		if (n == 2) {
			...
			final Entry<V> top = new Entry<>(s.readInt(), (V) s.readObject());
			top.black(true);
			top.right(new Entry<>(s.readInt(), (V) s.readObject()));
			top.right.pred(top);
			top.pred(pred);
			top.right.succ(succ);
			return top;
