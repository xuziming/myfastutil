...
public class Int2FloatRBTreeMap extends AbstractInt2FloatSortedMap implements java.io.Serializable, Cloneable {
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Int2FloatMap.Entry>() {
				final Comparator<? super Int2FloatMap.Entry> comparator = (Comparator<Int2FloatMap.Entry>) (x,
						y) -> Int2FloatRBTreeMap.this.actualComparator.compare(x.getIntKey(), y.getIntKey());
	...
	private final class Submap extends AbstractInt2FloatSortedMap implements java.io.Serializable {
		...
		final boolean in(final int k) {
			return (bottom || Int2FloatRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Int2FloatRBTreeMap.this.compare(k, to) < 0);
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Int2FloatRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Int2FloatRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
