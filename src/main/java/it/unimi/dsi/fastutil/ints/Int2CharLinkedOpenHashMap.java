...
public class Int2CharLinkedOpenHashMap extends AbstractInt2CharSortedMap
	...
	public Int2CharLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Int2CharLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Int2CharLinkedOpenHashMap(final Map<? extends Integer, ? extends Character> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Int2CharLinkedOpenHashMap(final Map<? extends Integer, ? extends Character> m) {
	 ...
	 * @param f the load factor.
	 */
	public Int2CharLinkedOpenHashMap(final Int2CharMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Int2CharLinkedOpenHashMap(final Int2CharMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2CharLinkedOpenHashMap(final int[] k, final char[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Int2CharLinkedOpenHashMap(final int[] k, final char[] v) {
	 ...
	 */
	public char getAndMoveToFirst(final int k) {
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public char getAndMoveToLast(final int k) {
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 */
	public char putAndMoveToFirst(final int k, final char v) {
		int pos;
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public char putAndMoveToLast(final int k, final char v) {
		int pos;
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				int curr;
				final int[] key = Int2CharLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Int2CharMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0)))
				return Int2CharLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2CharLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Int2CharMap.Entry> iterator(final Int2CharMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Int2CharMap.Entry> fastIterator(final Int2CharMap.Entry from) {
		...
		public void forEach(final Consumer<? super Int2CharMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractInt2CharMap.BasicEntry(key[curr], value[curr]));
			}
		}
