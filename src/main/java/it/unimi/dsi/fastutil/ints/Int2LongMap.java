...
public interface Int2LongMap extends Int2LongFunction, Map<Integer, Long> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Int2LongMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Int2LongMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final int key, final java.util.function.IntToLongFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsentNullable(final int key,
	 ...
	 * @since 8.0.0
	 */
	default long computeIfPresent(final int key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Long newValue = remappingFunction.apply(Integer.valueOf(key), Long.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default long compute(final int key,
			final java.util.function.BiFunction<? super Integer, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Long newValue = remappingFunction.apply(Integer.valueOf(key), contained ? Long.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default long merge(final int key, final long value,
	...
	@Deprecated
	@Override
	default Long computeIfAbsent(final Integer key,
	...
	@Deprecated
	@Override
	default Long computeIfPresent(final Integer key,
			final java.util.function.BiFunction<? super Integer, ? super Long, ? extends Long> remappingFunction) {
	...
	@Deprecated
	@Override
	default Long compute(final Integer key,
			final java.util.function.BiFunction<? super Integer, ? super Long, ? extends Long> remappingFunction) {
	...
	@Deprecated
	@Override
	default Long merge(final Integer key, final Long value,
			final java.util.function.BiFunction<? super Long, ? super Long, ? extends Long> remappingFunction) {
