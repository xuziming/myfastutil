 ...
 * @see HashCommon
 */
public class Int2ByteOpenCustomHashMap extends AbstractInt2ByteMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2ByteOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ByteOpenCustomHashMap(final int expected, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ByteOpenCustomHashMap(final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ByteOpenCustomHashMap(final Map<? extends Integer, ? extends Byte> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ByteOpenCustomHashMap(final Map<? extends Integer, ? extends Byte> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ByteOpenCustomHashMap(final Int2ByteMap m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ByteOpenCustomHashMap(final Int2ByteMap m, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2ByteOpenCustomHashMap(final int[] k, final byte[] v, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Int2ByteOpenCustomHashMap(final int[] k, final byte[] v,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	...
	private void insert(final int pos, final int k, final byte v) {
		if (pos == n)
	...
	/** {@inheritDoc} */
	@Override
	public byte putIfAbsent(final int k, final byte v) {
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
		insert(-pos - 1, k, v);
	...
	public boolean remove(final int k, final byte v) {
		if ((strategy.equals((k), (0)))) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean replace(final int k, final byte oldValue, final byte v) {
		final int pos = find(k);
	...
	/** {@inheritDoc} */
	@Override
	public byte replace(final int k, final byte v) {
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final byte oldValue = value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsent(final int k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsentNullable(final int k,
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfPresent(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Byte newValue = remappingFunction.apply(Integer.valueOf(k), Byte.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public byte compute(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Byte newValue = remappingFunction.apply(Integer.valueOf(k), pos >= 0 ? Byte.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public byte merge(final int k, final byte v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final int key[] = Int2ByteOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2ByteOpenCustomHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2ByteMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Int2ByteOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2ByteOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Int2ByteMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractInt2ByteMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractInt2ByteMap.BasicEntry(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Int2ByteMap.Entry> consumer) {
