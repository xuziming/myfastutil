 ...
 * @see HashCommon
 */
public class Int2ReferenceOpenCustomHashMap<V> extends AbstractInt2ReferenceMap<V>
	...
	public Int2ReferenceOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ReferenceOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ReferenceOpenCustomHashMap(final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ReferenceOpenCustomHashMap(final Map<? extends Integer, ? extends V> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ReferenceOpenCustomHashMap(final Map<? extends Integer, ? extends V> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ReferenceOpenCustomHashMap(final Int2ReferenceMap<V> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Int2ReferenceOpenCustomHashMap(final Int2ReferenceMap<V> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2ReferenceOpenCustomHashMap(final int[] k, final V[] v, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Int2ReferenceOpenCustomHashMap(final int[] k, final V[] v,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	...
	final class MapEntry implements Int2ReferenceMap.Entry<V>, Map.Entry<Integer, V> {
		...
		public boolean equals(final Object o) {
			...
			return (strategy.equals((key[index]), ((e.getKey()).intValue()))) && ((value[index]) == ((e.getValue())));
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2ReferenceOpenCustomHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Int2ReferenceOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2ReferenceOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Int2ReferenceMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractInt2ReferenceMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractInt2ReferenceMap.BasicEntry<V>(key[pos], value[pos]));
		}
