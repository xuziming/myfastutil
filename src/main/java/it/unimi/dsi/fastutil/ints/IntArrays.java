...
public final class IntArrays {
	...
	public static void quickSort(final int[] x, final int[] y, final int from, final int to) {
		...
		while (true) {
			int comparison, t;
			while (b <= c && (comparison = (t = (Integer.compare((x[b]), (v)))) == 0 ? (Integer.compare((y[b]), (w)))
					: t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			}
			while (c >= b && (comparison = (t = (Integer.compare((x[c]), (v)))) == 0 ? (Integer.compare((y[c]), (w)))
					: t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
	...
	protected static class ForkJoinQuickSort2 extends RecursiveAction {
		...
		protected void compute() {
			...
			while (true) {
				int comparison, t;
				while (b <= c
						&& (comparison = (t = (Integer.compare((x[b]), (v)))) == 0 ? (Integer.compare((y[b]), (w)))
								: t) <= 0) {
					if (comparison == 0)
						swap(x, y, a++, b);
					b++;
				...
				while (c >= b
						&& (comparison = (t = (Integer.compare((x[c]), (v)))) == 0 ? (Integer.compare((y[c]), (w)))
								: t) >= 0) {
					if (comparison == 0)
						swap(x, y, c, d--);
					c--;
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final int[] a, final int from, final int to,
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void parallelRadixSortIndirect(final int perm[], final int[] a, final int from, final int to,
			final boolean stable) {
		if (to - from < PARALLEL_RADIXSORT_NO_FORK) {
	...
	private static void insertionSortIndirect(final int[] perm, final int[] a, final int[] b, final int from,
			final int to) {
		for (int i = from; ++i < to;) {
			int t = perm[i];
			int j = i;
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final int[] a, final int[] b, final int from, final int to,
			final boolean stable) {
		if (to - from < RADIXSORT_NO_REC) {
