 ...
 * @see HashCommon
 */
public class Int2ByteLinkedOpenHashMap extends AbstractInt2ByteSortedMap
	...
	public Int2ByteLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Int2ByteLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Int2ByteLinkedOpenHashMap(final Map<? extends Integer, ? extends Byte> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Int2ByteLinkedOpenHashMap(final Map<? extends Integer, ? extends Byte> m) {
	 ...
	 * @param f the load factor.
	 */
	public Int2ByteLinkedOpenHashMap(final Int2ByteMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Int2ByteLinkedOpenHashMap(final Int2ByteMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2ByteLinkedOpenHashMap(final int[] k, final byte[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Int2ByteLinkedOpenHashMap(final int[] k, final byte[] v) {
	 ...
	 */
	public byte getAndMoveToFirst(final int k) {
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public byte getAndMoveToLast(final int k) {
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToFirst(final int k, final byte v) {
		int pos;
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToLast(final int k, final byte v) {
		int pos;
		if (((k) == (0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				int curr;
				final int[] key = Int2ByteLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Int2ByteMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if (((k) == (0)))
				return Int2ByteLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			int curr;
			final int[] key = Int2ByteLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Int2ByteMap.Entry> iterator(final Int2ByteMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Int2ByteMap.Entry> fastIterator(final Int2ByteMap.Entry from) {
		...
		public void forEach(final Consumer<? super Int2ByteMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractInt2ByteMap.BasicEntry(key[curr], value[curr]));
			}
		}
