 ...
 * @see HashCommon
 */
public class Int2FloatOpenCustomHashMap extends AbstractInt2FloatMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Int2FloatOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Int2FloatOpenCustomHashMap(final int expected, final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2FloatOpenCustomHashMap(final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2FloatOpenCustomHashMap(final Map<? extends Integer, ? extends Float> m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2FloatOpenCustomHashMap(final Map<? extends Integer, ? extends Float> m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Int2FloatOpenCustomHashMap(final Int2FloatMap m, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Int2FloatOpenCustomHashMap(final Int2FloatMap m,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Int2FloatOpenCustomHashMap(final int[] k, final float[] v, final float f,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Int2FloatOpenCustomHashMap(final int[] k, final float[] v,
			final it.unimi.dsi.fastutil.ints.IntHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfAbsent(final int k, final java.util.function.IntToDoubleFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfAbsentNullable(final int k,
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfPresent(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Float newValue = remappingFunction.apply(Integer.valueOf(k), Float.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), (0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public float compute(final int k,
			final java.util.function.BiFunction<? super Integer, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Float newValue = remappingFunction.apply(Integer.valueOf(k), pos >= 0 ? Float.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), (0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public float merge(final int k, final float v,
	...
	private class MapIterator {
		 ...
		 */
		boolean mustReturnNullKey = Int2FloatOpenCustomHashMap.this.containsNullKey;
		/**
		...
		public void remove() {
			...
			if (last == n) {
				...
				Int2FloatOpenCustomHashMap.this.remove(wrapped.getInt(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Int2FloatMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), (0))))
				return Int2FloatOpenCustomHashMap.this.containsNullKey
		...
		public void forEach(final Consumer<? super Int2FloatMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractInt2FloatMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == (0)))
					consumer.accept(new AbstractInt2FloatMap.BasicEntry(key[pos], value[pos]));
		}
