 ...
 * iterator}.
 */
public class Int2ShortAVLTreeMap extends AbstractInt2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	private static final class Entry extends AbstractInt2ShortMap.BasicEntry implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).intValue())) && ((value) == ((e.getValue()).shortValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Int2ShortAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Int2ShortMap.Entry> int2ShortEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Int2ShortMap.Entry>() {
				final Comparator<? super Int2ShortMap.Entry> comparator = (Comparator<Int2ShortMap.Entry>) (x,
						y) -> Int2ShortAVLTreeMap.this.actualComparator.compare(x.getIntKey(), y.getIntKey());
				...
				public ObjectSortedSet<Int2ShortMap.Entry> subSet(Int2ShortMap.Entry from, Int2ShortMap.Entry to) {
					return subMap(from.getIntKey(), to.getIntKey()).int2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Int2ShortMap.Entry> headSet(Int2ShortMap.Entry to) {
					return headMap(to.getIntKey()).int2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Int2ShortMap.Entry> tailSet(Int2ShortMap.Entry from) {
					return tailMap(from.getIntKey()).int2ShortEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractInt2ShortSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final int k) {
			return (bottom || Int2ShortAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Int2ShortAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Int2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Int2ShortAVLTreeMap.Entry f = findKey(((Integer) (e.getKey())).intValue());
		...
		public boolean containsKey(final int k) {
			return in(k) && Int2ShortAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public int firstIntKey() {
			Int2ShortAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public int lastIntKey() {
			Int2ShortAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Int2ShortAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Int2ShortAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
