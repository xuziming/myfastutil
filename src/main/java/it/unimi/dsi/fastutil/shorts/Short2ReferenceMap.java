...
public interface Short2ReferenceMap<V> extends Short2ReferenceFunction<V>, Map<Short, V> {
	 ...
	 */
	interface FastEntrySet<V> extends ObjectSet<Short2ReferenceMap.Entry<V>> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Short2ReferenceMap.Entry<V>> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsent(final short key, final java.util.function.IntFunction<? extends V> mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default V computeIfAbsentPartial(final short key, final Short2ReferenceFunction<? extends V> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final V v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default V computeIfPresent(final short key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final V newValue = remappingFunction.apply(Short.valueOf(key), (oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default V compute(final short key,
			final java.util.function.BiFunction<? super Short, ? super V, ? extends V> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final V oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final V newValue = remappingFunction.apply(Short.valueOf(key), contained ? (oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 *                          associated.
	 * @param value             the non-{@code null} value to be merged with the
	 *                          existing value associated with the key or, if no
	 *                          existing value is associated with the key, to be
	 ...
	 * @since 8.0.0
	 */
	default V merge(final short key, final V value,
	...
	@Deprecated
	@Override
	default V computeIfAbsent(final Short key,
	 ...
	 */
	interface Entry<V> extends Map.Entry<Short, V> {
		/**
