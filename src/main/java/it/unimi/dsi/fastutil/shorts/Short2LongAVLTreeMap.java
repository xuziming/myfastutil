 ...
 * iterator}.
 */
public class Short2LongAVLTreeMap extends AbstractShort2LongSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Short2LongAVLTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2LongAVLTreeMap(final Map<? extends Short, ? extends Long> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2LongAVLTreeMap(final SortedMap<Short, Long> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2LongAVLTreeMap(final Short2LongMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2LongAVLTreeMap(final Short2LongSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2LongAVLTreeMap(final short[] k, final long v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2LongAVLTreeMap(final short[] k, final long v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry extends AbstractShort2LongMap.BasicEntry implements Cloneable {
		 ...
		 * @param v a value.
		 */
		Entry(final short k, final long v) {
			super(k, v);
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).shortValue())) && ((value) == ((e.getValue()).longValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2LongAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	 ...
	 * This class can iterate in both directions on a threaded tree.
	 */
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Short2LongMap.Entry> {
		EntryIterator() {
		}
	...
	public ObjectSortedSet<Short2LongMap.Entry> short2LongEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2LongMap.Entry>() {
				final Comparator<? super Short2LongMap.Entry> comparator = (Comparator<Short2LongMap.Entry>) (x,
						y) -> Short2LongAVLTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
				...
				@Override
				public ObjectBidirectionalIterator<Short2LongMap.Entry> iterator(final Short2LongMap.Entry from) {
				...
				public ObjectSortedSet<Short2LongMap.Entry> subSet(Short2LongMap.Entry from, Short2LongMap.Entry to) {
					return subMap(from.getShortKey(), to.getShortKey()).short2LongEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2LongMap.Entry> headSet(Short2LongMap.Entry to) {
					return headMap(to.getShortKey()).short2LongEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2LongMap.Entry> tailSet(Short2LongMap.Entry from) {
					return tailMap(from.getShortKey()).short2LongEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractShort2LongSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final short k) {
			return (bottom || Short2LongAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2LongAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2LongMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Long))
							return false;
						final Short2LongAVLTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
					...
					@Override
					public ObjectSortedSet<Short2LongMap.Entry> subSet(Short2LongMap.Entry from,
							Short2LongMap.Entry to) {
						return subMap(from.getShortKey(), to.getShortKey()).short2LongEntrySet();
					}
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2LongAVLTreeMap.this.containsKey(k);
		}
		...
		public long get(final short k) {
			final Short2LongAVLTreeMap.Entry e;
			final short kk = k;
		...
		public long put(final short k, final long v) {
			...
			final long oldValue = Short2LongAVLTreeMap.this.put(k, v);
		...
		public long remove(final short k) {
			...
			final long oldValue = Short2LongAVLTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2LongAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2LongAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2LongAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2LongAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry readTree(final java.io.ObjectInputStream s, final int n, final Entry pred, final Entry succ)
		...
		if (n == 1) {
			final Entry top = new Entry(s.readShort(), s.readLong());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
