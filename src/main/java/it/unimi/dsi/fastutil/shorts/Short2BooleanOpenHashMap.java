...
public class Short2BooleanOpenHashMap extends AbstractShort2BooleanMap
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Short2BooleanOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Short2BooleanOpenHashMap(final Map<? extends Short, ? extends Boolean> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Short2BooleanOpenHashMap(final Map<? extends Short, ? extends Boolean> m) {
	 ...
	 * @param f the load factor.
	 */
	public Short2BooleanOpenHashMap(final Short2BooleanMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Short2BooleanOpenHashMap(final Short2BooleanMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2BooleanOpenHashMap(final short[] k, final boolean[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Short2BooleanOpenHashMap(final short[] k, final boolean[] v) {
	...
	final class MapEntry implements Short2BooleanMap.Entry, Map.Entry<Short, Boolean> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()).shortValue())) && ((value[index]) == ((e.getValue()).booleanValue()));
		}
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final short key[] = Short2BooleanOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Short2BooleanOpenHashMap.this.remove(wrapped.getShort(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2BooleanMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((short) 0)))
				return Short2BooleanOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2BooleanOpenHashMap.this.key;
			int pos;
			// The starting point.
