 ...
 */
public class Short2LongArrayMap extends AbstractShort2LongMap implements java.io.Serializable, Cloneable {
	 ...
	 *              {@code key}).
	 */
	public Short2LongArrayMap(final short[] key, final long[] value) {
		this.key = key;
		this.value = value;
		size = key.length;
	 ...
	 */
	public Short2LongArrayMap(final int capacity) {
	 ...
	 * @param m a map.
	 */
	public Short2LongArrayMap(final Short2LongMap m) {
		this(m.size());
		putAll(m);
	}
	 ...
	 * @param m a map.
	 */
	public Short2LongArrayMap(final Map<? extends Short, ? extends Long> m) {
		this(m.size());
		putAll(m);
	}
	 ...
	 */
	public Short2LongArrayMap(final short[] key, final long[] value, final int size) {
		this.key = key;
		this.value = value;
		this.size = size;
		if (key.length != value.length)
	...
	private final class EntrySet extends AbstractObjectSet<Short2LongMap.Entry> implements FastEntrySet {
		@Override
		public ObjectIterator<Short2LongMap.Entry> iterator() {
			return new ObjectIterator<Short2LongMap.Entry>() {
				...
				public Entry next() {
					...
					return new AbstractShort2LongMap.BasicEntry(key[curr = next], value[next++]);
				}
		...
		public boolean contains(Object o) {
			...
			return Short2LongArrayMap.this.containsKey(k)
					&& ((Short2LongArrayMap.this.get(k)) == (((Long) (e.getValue())).longValue()));
		}
		...
		public boolean remove(final Object o) {
			...
			final int oldPos = Short2LongArrayMap.this.findKey(k);
