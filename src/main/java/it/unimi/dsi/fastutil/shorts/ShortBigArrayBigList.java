...
public class ShortBigArrayBigList extends AbstractShortBigList
	 ...
	 */
	protected ShortBigArrayBigList(final short a[][], boolean dummy) {
		this.a = a;
	}
	...
	public ShortBigArrayBigList(final long capacity) {
		...
		if (capacity == 0)
			a = ShortBigArrays.EMPTY_BIG_ARRAY;
		else
			a = ShortBigArrays.newBigArray(capacity);
	}
	...
	public ShortBigArrayBigList() {
		a = ShortBigArrays.DEFAULT_EMPTY_BIG_ARRAY; // We delay allocation
	}
	...
	public ShortBigArrayBigList(final ShortCollection c) {
		this(c.size());
		for (ShortIterator i = c.iterator(); i.hasNext();)
			add(i.nextShort());
	}
	...
	public void ensureCapacity(final long capacity) {
		if (capacity <= a.length || a == ShortBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
			return;
	...
	private void grow(long capacity) {
		...
		if (capacity <= oldLength)
			return;
		if (a != ShortBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
	...
	public short removeShort(final long index) {
		if (index >= size)
			throw new IndexOutOfBoundsException(
					"Index (" + index + ") is greater than or equal to list size (" + size + ")");
		final short old = BigArrays.get(a, index);
		size--;
		if (index != size)
	...
	public short set(final long index, final short k) {
		if (index >= size)
			throw new IndexOutOfBoundsException(
					"Index (" + index + ") is greater than or equal to list size (" + size + ")");
		short old = BigArrays.get(a, index);
		BigArrays.set(a, index, k);
		return old;
	}
	...
	public void size(final long size) {
		...
		if (size > this.size)
			BigArrays.fill(a, this.size, size, ((short) 0));
		this.size = size;
	}
	...
	public ShortBigListIterator listIterator(final long index) {
		...
		return new ShortBigListIterator() {
			...
			public void add(short k) {
				ShortBigArrayBigList.this.add(pos++, k);
				last = -1;
			}
			...
			public void set(short k) {
				...
				ShortBigArrayBigList.this.set(last, k);
			}
			...
			public void remove() {
				if (last == -1)
					throw new IllegalStateException();
				ShortBigArrayBigList.this.removeShort(last);
				/*
	...
	public ShortBigArrayBigList clone() {
		ShortBigArrayBigList c = new ShortBigArrayBigList(size);
	...
	private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
		...
		for (int i = 0; i < size; i++)
			s.writeShort(BigArrays.get(a, i));
	}

	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		s.defaultReadObject();
		a = ShortBigArrays.newBigArray(size);
		for (int i = 0; i < size; i++)
			BigArrays.set(a, i, s.readShort());
	}
}
