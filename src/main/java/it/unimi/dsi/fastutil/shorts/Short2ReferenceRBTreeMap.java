 ...
 *
 */
public class Short2ReferenceRBTreeMap<V> extends AbstractShort2ReferenceSortedMap<V>
	 ...
	 */
	public Short2ReferenceRBTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2ReferenceRBTreeMap(final Map<? extends Short, ? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2ReferenceRBTreeMap(final SortedMap<Short, V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2ReferenceRBTreeMap(final Short2ReferenceMap<? extends V> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2ReferenceRBTreeMap(final Short2ReferenceSortedMap<V> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2ReferenceRBTreeMap(final short[] k, final V v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2ReferenceRBTreeMap(final short[] k, final V v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2ReferenceRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Short2ReferenceMap.Entry<V>> short2ReferenceEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2ReferenceMap.Entry<V>>() {
				final Comparator<? super Short2ReferenceMap.Entry<V>> comparator = (Comparator<Short2ReferenceMap.Entry<V>>) (
						x,
						y) -> Short2ReferenceRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
				...
				@Override
				public ObjectBidirectionalIterator<Short2ReferenceMap.Entry<V>> iterator(
						final Short2ReferenceMap.Entry<V> from) {
				...
				public boolean contains(final Object o) {
					...
					if (e.getKey() == null || !(e.getKey() instanceof Short))
						return false;
					final Entry<V> f = findKey(((Short) (e.getKey())).shortValue());
					return e.equals(f);
				}
				...
				@Override
				public ObjectSortedSet<Short2ReferenceMap.Entry<V>> subSet(Short2ReferenceMap.Entry<V> from,
						Short2ReferenceMap.Entry<V> to) {
					return subMap(from.getShortKey(), to.getShortKey()).short2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2ReferenceMap.Entry<V>> headSet(Short2ReferenceMap.Entry<V> to) {
					return headMap(to.getShortKey()).short2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2ReferenceMap.Entry<V>> tailSet(Short2ReferenceMap.Entry<V> from) {
					return tailMap(from.getShortKey()).short2ReferenceEntrySet();
				}
			};
		return entries;
	}
	...
	private final class Submap extends AbstractShort2ReferenceSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final short k) {
			return (bottom || Short2ReferenceRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2ReferenceRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Short))
							return false;
						final Short2ReferenceRBTreeMap.Entry<V> f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2ReferenceRBTreeMap.this.containsKey(k);
		}
		...
		public V get(final short k) {
			final Short2ReferenceRBTreeMap.Entry<V> e;
			final short kk = k;
		...
		public V put(final short k, final V v) {
			...
			final V oldValue = Short2ReferenceRBTreeMap.this.put(k, v);
		...
		public V remove(final short k) {
			...
			final V oldValue = Short2ReferenceRBTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2ReferenceRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2ReferenceRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2ReferenceRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2ReferenceRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry<V> readTree(final java.io.ObjectInputStream s, final int n, final Entry<V> pred, final Entry<V> succ)
		...
		if (n == 1) {
			final Entry<V> top = new Entry<>(s.readShort(), (V) s.readObject());
			top.pred(pred);
			top.succ(succ);
			top.black(true);
			return top;
		...
		if (n == 2) {
			...
			final Entry<V> top = new Entry<>(s.readShort(), (V) s.readObject());
			top.black(true);
			top.right(new Entry<>(s.readShort(), (V) s.readObject()));
			top.right.pred(top);
			top.pred(pred);
			top.right.succ(succ);
			return top;
