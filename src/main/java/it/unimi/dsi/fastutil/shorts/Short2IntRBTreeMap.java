 ...
 *
 */
public class Short2IntRBTreeMap extends AbstractShort2IntSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Short2IntRBTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2IntRBTreeMap(final Map<? extends Short, ? extends Integer> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2IntRBTreeMap(final SortedMap<Short, Integer> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2IntRBTreeMap(final Short2IntMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2IntRBTreeMap(final Short2IntSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2IntRBTreeMap(final short[] k, final int v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2IntRBTreeMap(final short[] k, final int v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry extends AbstractShort2IntMap.BasicEntry implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).shortValue())) && ((value) == ((e.getValue()).intValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2IntRBTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Short2IntMap.Entry> short2IntEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2IntMap.Entry>() {
				final Comparator<? super Short2IntMap.Entry> comparator = (Comparator<Short2IntMap.Entry>) (x,
						y) -> Short2IntRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
				...
				public ObjectSortedSet<Short2IntMap.Entry> subSet(Short2IntMap.Entry from, Short2IntMap.Entry to) {
					return subMap(from.getShortKey(), to.getShortKey()).short2IntEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2IntMap.Entry> headSet(Short2IntMap.Entry to) {
					return headMap(to.getShortKey()).short2IntEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2IntMap.Entry> tailSet(Short2IntMap.Entry from) {
					return tailMap(from.getShortKey()).short2IntEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractShort2IntSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final short k) {
			return (bottom || Short2IntRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2IntRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2IntMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Integer))
							return false;
						final Short2IntRBTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2IntRBTreeMap.this.containsKey(k);
		}
		...
		public int get(final short k) {
			final Short2IntRBTreeMap.Entry e;
			final short kk = k;
		...
		public int put(final short k, final int v) {
			...
			final int oldValue = Short2IntRBTreeMap.this.put(k, v);
		...
		public int remove(final short k) {
			...
			final int oldValue = Short2IntRBTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2IntRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2IntRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2IntRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2IntRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Short2IntRBTreeMap clone() {
		...
		try {
			c = (Short2IntRBTreeMap) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
