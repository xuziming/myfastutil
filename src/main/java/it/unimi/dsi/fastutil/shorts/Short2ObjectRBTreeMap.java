...
public class Short2ObjectRBTreeMap<V> extends AbstractShort2ObjectSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2ObjectRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2ObjectMap.Entry<V>>() {
				final Comparator<? super Short2ObjectMap.Entry<V>> comparator = (Comparator<Short2ObjectMap.Entry<V>>) (
						x, y) -> Short2ObjectRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	...
	private final class Submap extends AbstractShort2ObjectSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final short k) {
			return (bottom || Short2ObjectRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2ObjectRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2ObjectMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Short))
							return false;
						final Short2ObjectRBTreeMap.Entry<V> f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2ObjectRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public short firstShortKey() {
			Short2ObjectRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2ObjectRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2ObjectRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2ObjectRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Short2ObjectRBTreeMap<V> clone() {
		...
		try {
			c = (Short2ObjectRBTreeMap<V>) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
