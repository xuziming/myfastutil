 ...
 * iterator}.
 */
public class Short2ReferenceAVLTreeMap<V> extends AbstractShort2ReferenceSortedMap<V>
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2ReferenceMap.Entry<V>>() {
				final Comparator<? super Short2ReferenceMap.Entry<V>> comparator = (Comparator<Short2ReferenceMap.Entry<V>>) (
						x,
						y) -> Short2ReferenceAVLTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	...
	private final class Submap extends AbstractShort2ReferenceSortedMap<V> implements java.io.Serializable {
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Short))
							return false;
						final Short2ReferenceAVLTreeMap.Entry<V> f = findKey(((Short) (e.getKey())).shortValue());
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2ReferenceAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2ReferenceAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
