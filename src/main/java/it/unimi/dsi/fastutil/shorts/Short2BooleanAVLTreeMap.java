 ...
 * iterator}.
 */
public class Short2BooleanAVLTreeMap extends AbstractShort2BooleanSortedMap implements java.io.Serializable, Cloneable {
	...
	private void setActualComparator() {
		actualComparator = ShortComparators.asShortComparator(storedComparator);
	}
	 ...
	 */
	public Short2BooleanAVLTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2BooleanAVLTreeMap(final Map<? extends Short, ? extends Boolean> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2BooleanAVLTreeMap(final SortedMap<Short, Boolean> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2BooleanAVLTreeMap(final Short2BooleanMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2BooleanAVLTreeMap(final Short2BooleanSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2BooleanAVLTreeMap(final short[] k, final boolean v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2BooleanAVLTreeMap(final short[] k, final boolean v[]) {
		this(k, v, null);
	}
	...
	final int compare(final short k1, final short k2) {
		return actualComparator == null ? (Short.compare((k1), (k2))) : actualComparator.compare(k1, k2);
	}
	 ...
	 * It suffices for about 2<sup>32</sup> entries.
	 */
	...
	private static final class Entry extends AbstractShort2BooleanMap.BasicEntry implements Cloneable {
		 ...
		 * @param v a value.
		 */
		Entry(final short k, final boolean v) {
			super(k, v);
		...
		public boolean equals(final Object o) {
			if (!(o instanceof Map.Entry))
				return false;
			Map.Entry<Short, Boolean> e = (Map.Entry<Short, Boolean>) o;
			return ((key) == ((e.getKey()).shortValue())) && ((value) == ((e.getValue()).booleanValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2BooleanAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	 ...
	 * This class can iterate in both directions on a threaded tree.
	 */
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Short2BooleanMap.Entry> {
		EntryIterator() {
		}
	...
	public ObjectSortedSet<Short2BooleanMap.Entry> short2BooleanEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2BooleanMap.Entry>() {
				final Comparator<? super Short2BooleanMap.Entry> comparator = (Comparator<Short2BooleanMap.Entry>) (x,
						y) -> Short2BooleanAVLTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
				...
				@Override
				public ObjectBidirectionalIterator<Short2BooleanMap.Entry> iterator(final Short2BooleanMap.Entry from) {
				...
				public boolean contains(final Object o) {
					...
					if (e.getValue() == null || !(e.getValue() instanceof Boolean))
						return false;
					final Entry f = findKey(((Short) (e.getKey())).shortValue());
					return e.equals(f);
				}
				...
				@Override
				public ObjectSortedSet<Short2BooleanMap.Entry> subSet(Short2BooleanMap.Entry from,
						Short2BooleanMap.Entry to) {
					return subMap(from.getShortKey(), to.getShortKey()).short2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2BooleanMap.Entry> headSet(Short2BooleanMap.Entry to) {
					return headMap(to.getShortKey()).short2BooleanEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2BooleanMap.Entry> tailSet(Short2BooleanMap.Entry from) {
					return tailMap(from.getShortKey()).short2BooleanEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractShort2BooleanSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final short k) {
			return (bottom || Short2BooleanAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2BooleanAVLTreeMap.this.compare(k, to) < 0);
		}
		...
		public ObjectSortedSet<Short2BooleanMap.Entry> short2BooleanEntrySet() {
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2BooleanMap.Entry>() {
					...
					@Override
					public ObjectBidirectionalIterator<Short2BooleanMap.Entry> iterator(
							final Short2BooleanMap.Entry from) {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Short2BooleanAVLTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2BooleanAVLTreeMap.this.containsKey(k);
		}
		...
		public boolean get(final short k) {
			final Short2BooleanAVLTreeMap.Entry e;
			final short kk = k;
		...
		public boolean put(final short k, final boolean v) {
			...
			final boolean oldValue = Short2BooleanAVLTreeMap.this.put(k, v);
		...
		public boolean remove(final short k) {
			...
			final boolean oldValue = Short2BooleanAVLTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2BooleanAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2BooleanAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2BooleanAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2BooleanAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry readTree(final java.io.ObjectInputStream s, final int n, final Entry pred, final Entry succ)
		...
		if (n == 1) {
			final Entry top = new Entry(s.readShort(), s.readBoolean());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
