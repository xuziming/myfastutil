...
public interface Short2LongMap extends Short2LongFunction, Map<Short, Long> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Short2LongMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Short2LongMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final short key, final java.util.function.IntToLongFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsentNullable(final short key,
	 ...
	 * @since 8.0.0
	 */
	default long computeIfPresent(final short key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Long newValue = remappingFunction.apply(Short.valueOf(key), Long.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default long compute(final short key,
			final java.util.function.BiFunction<? super Short, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Long newValue = remappingFunction.apply(Short.valueOf(key), contained ? Long.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default long merge(final short key, final long value,
	...
	@Deprecated
	@Override
	default Long computeIfAbsent(final Short key,
	 ...
	 */
	interface Entry extends Map.Entry<Short, Long> {
		/**
