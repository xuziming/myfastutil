 ...
 *
 */
public class Short2BooleanRBTreeMap extends AbstractShort2BooleanSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Short2BooleanRBTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2BooleanRBTreeMap(final Map<? extends Short, ? extends Boolean> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2BooleanRBTreeMap(final SortedMap<Short, Boolean> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2BooleanRBTreeMap(final Short2BooleanMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2BooleanRBTreeMap(final Short2BooleanSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2BooleanRBTreeMap(final short[] k, final boolean v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2BooleanRBTreeMap(final short[] k, final boolean v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2BooleanRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2BooleanMap.Entry>() {
				final Comparator<? super Short2BooleanMap.Entry> comparator = (Comparator<Short2BooleanMap.Entry>) (x,
						y) -> Short2BooleanRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	...
	private final class Submap extends AbstractShort2BooleanSortedMap implements java.io.Serializable {
		...
		final boolean in(final short k) {
			return (bottom || Short2BooleanRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2BooleanRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2BooleanMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Boolean))
							return false;
						final Short2BooleanRBTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2BooleanRBTreeMap.this.containsKey(k);
		}
		...
		public boolean get(final short k) {
			final Short2BooleanRBTreeMap.Entry e;
			final short kk = k;
		...
		public boolean put(final short k, final boolean v) {
			...
			final boolean oldValue = Short2BooleanRBTreeMap.this.put(k, v);
		...
		public boolean remove(final short k) {
			...
			final boolean oldValue = Short2BooleanRBTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2BooleanRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2BooleanRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2BooleanRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2BooleanRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
