 ...
 * @see HashCommon
 */
public class Short2IntLinkedOpenHashMap extends AbstractShort2IntSortedMap
	...
	public Short2IntLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Short2IntLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Short2IntLinkedOpenHashMap(final Map<? extends Short, ? extends Integer> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Short2IntLinkedOpenHashMap(final Map<? extends Short, ? extends Integer> m) {
	 ...
	 * @param f the load factor.
	 */
	public Short2IntLinkedOpenHashMap(final Short2IntMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Short2IntLinkedOpenHashMap(final Short2IntMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2IntLinkedOpenHashMap(final short[] k, final int[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Short2IntLinkedOpenHashMap(final short[] k, final int[] v) {
	 ...
	 */
	public int getAndMoveToFirst(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public int getAndMoveToLast(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public int putAndMoveToFirst(final short k, final int v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public int putAndMoveToLast(final short k, final int v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				short curr;
				final short[] key = Short2IntLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Short2IntMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		@Override
		public ObjectSortedSet<Short2IntMap.Entry> subSet(Short2IntMap.Entry fromElement,
				Short2IntMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((short) 0)))
				return Short2IntLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2IntLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Short2IntMap.Entry> iterator(final Short2IntMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Short2IntMap.Entry> fastIterator(final Short2IntMap.Entry from) {
		...
		public void forEach(final Consumer<? super Short2IntMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractShort2IntMap.BasicEntry(key[curr], value[curr]));
			}
		}
	...
	@Override
	public FastSortedEntrySet short2IntEntrySet() {
		if (entries == null)
	...
	public Short2IntLinkedOpenHashMap clone() {
		...
		try {
			c = (Short2IntLinkedOpenHashMap) super.clone();
