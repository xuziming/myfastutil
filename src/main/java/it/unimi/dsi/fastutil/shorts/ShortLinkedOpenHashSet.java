...
public class ShortLinkedOpenHashSet extends AbstractShortSortedSet implements java.io.Serializable, Cloneable, Hash {
	...
	public ShortLinkedOpenHashSet(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash set.
	 */
	public ShortLinkedOpenHashSet(final int expected) {
	...
	private class SetIterator implements ShortListIterator {
		...
		SetIterator(short from) {
			...
			// The starting point.
			final short key[] = ShortLinkedOpenHashSet.this.key;
		...
		public void remove() {
			...
			if (pos == n) {
				...
				ShortLinkedOpenHashSet.this.key[n] = ((short) 0);
			} else {
				short curr;
				final short[] key = ShortLinkedOpenHashSet.this.key;
