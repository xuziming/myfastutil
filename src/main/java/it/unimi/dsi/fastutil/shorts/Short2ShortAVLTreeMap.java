 ...
 * iterator}.
 */
public class Short2ShortAVLTreeMap extends AbstractShort2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	private static final class Entry extends AbstractShort2ShortMap.BasicEntry implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).shortValue())) && ((value) == ((e.getValue()).shortValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2ShortAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Short2ShortMap.Entry> short2ShortEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2ShortMap.Entry>() {
				final Comparator<? super Short2ShortMap.Entry> comparator = (Comparator<Short2ShortMap.Entry>) (x,
						y) -> Short2ShortAVLTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
				...
				@Override
				public ObjectSortedSet<Short2ShortMap.Entry> subSet(Short2ShortMap.Entry from,
						Short2ShortMap.Entry to) {
					return subMap(from.getShortKey(), to.getShortKey()).short2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2ShortMap.Entry> headSet(Short2ShortMap.Entry to) {
					return headMap(to.getShortKey()).short2ShortEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2ShortMap.Entry> tailSet(Short2ShortMap.Entry from) {
					return tailMap(from.getShortKey()).short2ShortEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractShort2ShortSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final short k) {
			return (bottom || Short2ShortAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2ShortAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Short2ShortAVLTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2ShortAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public short firstShortKey() {
			Short2ShortAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2ShortAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2ShortAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2ShortAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
