 ...
 * iterator}.
 */
public class Short2CharAVLTreeMap extends AbstractShort2CharSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Short2CharAVLTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2CharAVLTreeMap(final Map<? extends Short, ? extends Character> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2CharAVLTreeMap(final SortedMap<Short, Character> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2CharAVLTreeMap(final Short2CharMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2CharAVLTreeMap(final Short2CharSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2CharAVLTreeMap(final short[] k, final char v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2CharAVLTreeMap(final short[] k, final char v[]) {
		this(k, v, null);
	}
	...
	private static final class Entry extends AbstractShort2CharMap.BasicEntry implements Cloneable {
		 ...
		 * @param v a value.
		 */
		Entry(final short k, final char v) {
			super(k, v);
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).shortValue())) && ((value) == ((e.getValue()).charValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2CharAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	 ...
	 * This class can iterate in both directions on a threaded tree.
	 */
	private class EntryIterator extends TreeIterator implements ObjectListIterator<Short2CharMap.Entry> {
		EntryIterator() {
		}
	...
	public ObjectSortedSet<Short2CharMap.Entry> short2CharEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2CharMap.Entry>() {
				final Comparator<? super Short2CharMap.Entry> comparator = (Comparator<Short2CharMap.Entry>) (x,
						y) -> Short2CharAVLTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
				...
				@Override
				public ObjectBidirectionalIterator<Short2CharMap.Entry> iterator(final Short2CharMap.Entry from) {
				...
				public ObjectSortedSet<Short2CharMap.Entry> subSet(Short2CharMap.Entry from, Short2CharMap.Entry to) {
					return subMap(from.getShortKey(), to.getShortKey()).short2CharEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2CharMap.Entry> headSet(Short2CharMap.Entry to) {
					return headMap(to.getShortKey()).short2CharEntrySet();
				}

				@Override
				public ObjectSortedSet<Short2CharMap.Entry> tailSet(Short2CharMap.Entry from) {
					return tailMap(from.getShortKey()).short2CharEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractShort2CharSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final short k) {
			return (bottom || Short2CharAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2CharAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2CharMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Character))
							return false;
						final Short2CharAVLTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
					...
					@Override
					public ObjectSortedSet<Short2CharMap.Entry> subSet(Short2CharMap.Entry from,
							Short2CharMap.Entry to) {
						return subMap(from.getShortKey(), to.getShortKey()).short2CharEntrySet();
					}
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2CharAVLTreeMap.this.containsKey(k);
		}
		...
		public char get(final short k) {
			final Short2CharAVLTreeMap.Entry e;
			final short kk = k;
		...
		public char put(final short k, final char v) {
			...
			final char oldValue = Short2CharAVLTreeMap.this.put(k, v);
		...
		public char remove(final short k) {
			...
			final char oldValue = Short2CharAVLTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2CharAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2CharAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2CharAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2CharAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry readTree(final java.io.ObjectInputStream s, final int n, final Entry pred, final Entry succ)
		...
		if (n == 1) {
			final Entry top = new Entry(s.readShort(), s.readChar());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
