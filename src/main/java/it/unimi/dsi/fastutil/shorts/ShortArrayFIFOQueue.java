...
public class ShortArrayFIFOQueue implements ShortPriorityQueue, Serializable {
	...
	public ShortArrayFIFOQueue(final int capacity) {
		...
		array = new short[Math.max(1, capacity)]; // Never build a queue with zero-sized backing array.
		length = array.length;
	}
	...
	private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
		...
		array = new short[length = HashCommon.nextPowerOfTwo(end + 1)];
		for (int i = 0; i < end; i++)
			array[i] = s.readShort();
	}
}
