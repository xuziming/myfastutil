...
public class ShortArrayList extends AbstractShortList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public void ensureCapacity(final int capacity) {
		if (capacity <= a.length || (a == ShortArrays.DEFAULT_EMPTY_ARRAY && capacity <= DEFAULT_INITIAL_CAPACITY))
			return;
	...
	private void grow(int capacity) {
		if (capacity <= a.length)
			return;
		if (a != ShortArrays.DEFAULT_EMPTY_ARRAY)
	...
	public void size(final int size) {
		if (size > a.length)
			a = ShortArrays.forceCapacity(a, size, this.size);
		if (size > this.size)
			Arrays.fill(a, this.size, size, ((short) 0));
		this.size = size;
	}
	...
	public boolean removeAll(final Collection<?> c) {
		...
		int j = 0;
		for (int i = 0; i < size; i++)
			if (!c.contains(Short.valueOf(a[i])))
				a[j++] = a[i];
		final boolean modified = size != j;
		size = j;
		return modified;
