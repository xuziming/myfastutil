 ...
 * @see HashCommon
 */
public class Short2DoubleOpenHashMap extends AbstractShort2DoubleMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Short2DoubleOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Short2DoubleOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Short2DoubleOpenHashMap(final Map<? extends Short, ? extends Double> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Short2DoubleOpenHashMap(final Map<? extends Short, ? extends Double> m) {
	 ...
	 * @param f the load factor.
	 */
	public Short2DoubleOpenHashMap(final Short2DoubleMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Short2DoubleOpenHashMap(final Short2DoubleMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2DoubleOpenHashMap(final short[] k, final double[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Short2DoubleOpenHashMap(final short[] k, final double[] v) {
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final short key[] = Short2DoubleOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Short2DoubleOpenHashMap.this.remove(wrapped.getShort(-pos - 1));
