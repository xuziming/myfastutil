 ...
 * @see HashCommon
 */
public class Short2ByteOpenCustomHashMap extends AbstractShort2ByteMap
	...
	public Short2ByteOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ByteOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ByteOpenCustomHashMap(final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ByteOpenCustomHashMap(final Map<? extends Short, ? extends Byte> m, final float f,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ByteOpenCustomHashMap(final Map<? extends Short, ? extends Byte> m,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ByteOpenCustomHashMap(final Short2ByteMap m, final float f,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ByteOpenCustomHashMap(final Short2ByteMap m,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2ByteOpenCustomHashMap(final short[] k, final byte[] v, final float f,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Short2ByteOpenCustomHashMap(final short[] k, final byte[] v,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsent(final short k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfAbsentNullable(final short k,
	...
	/** {@inheritDoc} */
	@Override
	public byte computeIfPresent(final short k,
			final java.util.function.BiFunction<? super Short, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Byte newValue = remappingFunction.apply(Short.valueOf(k), Byte.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((short) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public byte compute(final short k,
			final java.util.function.BiFunction<? super Short, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Byte newValue = remappingFunction.apply(Short.valueOf(k), pos >= 0 ? Byte.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), ((short) 0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public byte merge(final short k, final byte v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Short2ByteOpenCustomHashMap.this.remove(wrapped.getShort(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2ByteMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((short) 0))))
				return Short2ByteOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2ByteOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Short2ByteMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractShort2ByteMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((short) 0)))
					consumer.accept(new AbstractShort2ByteMap.BasicEntry(key[pos], value[pos]));
		}
