 ...
 * entirely. The <em>ratio</em> of a front-coded list defines how often this
 ...
 * collection. See the documentation of the family of {@code get()} methods.
 *
 * <p>
 ...
 * other arrays are stored as follows: let {@code common} the length of the
 * maximum common prefix between the array and its predecessor. Then we store
 * the array length decremented by {@code common}, followed by {@code common},
 * followed by the array elements whose index is greater than or equal to
 * {@code common}. For instance, if we store {@code foo}, {@code foobar},
 * {@code football} and {@code fool} in a front-coded character-array list with
...
public class ShortArrayFrontCodedList extends AbstractObjectList<short[]>
	...
	/**
	 * Creates a new front-coded list containing the arrays returned by the given
	 * iterator.
	 *
	 ...
	 */
	private static int readInt(final short a[][], long pos) {
	 ...
	 * @return the number of elements coding {@code length}.
	 */
	private static int writeInt(final short a[][], int length, long pos) {
		if (length < (1 << 15)) {
	...
	private int length(final int index) {
		...
		if (delta == 0)
			return length;
		// First of all, we recover the array length and the maximum amount of copied
		// elements.
		int common;
	 ...
	 *
	 * @param index  an index.
	 * @param a      the array that will store the result (we assume that it can
	 *               hold the result).
	...
	private int extract(final int index, final short a[], final int offset, final int length) {
		...
		final long startPos = p[index / ratio]; // The position into the array of the first entire word before the
												// index-th.
		...
		if (currLen < length)
			copyFromBig(array, pos + count(arrayLength) + count(common), a, currLen + offset,
	...
	/**
	 * Stores in the given array elements from an array stored in this front-coded
	 * list.
	 *
	 * @param index  an index.
	 ...
	 * @param offset an offset into {@code a} where elements will be store.
	 * @param length a maximum number of elements to store in {@code a}.
	 * @return if {@code a} can hold the extracted elements, the number of extracted
	 *         elements; otherwise, the number of remaining elements with the sign
	 *         changed.
	 */
	 ...
	 *
	 * @param index an index.
	 * @param a     the array that will store the content of the result (we assume
	...
	public ObjectListIterator<short[]> listIterator(final int start) {
		...
		return new ObjectListIterator<short[]>() {
			...
			int i = 0;
			long pos = 0;
			boolean inSync; // Whether the current value in a is the string just before the next to be
							// produced.
			{
				if (start != 0) {
					if (start == n)
			...
			public short[] next() {
				...
				if (i % ratio == 0) {
					pos = p[i / ratio];
					length = readInt(array, pos);
					s = ShortArrays.ensureCapacity(s, length, 0);
					...
					if (inSync) {
						length = readInt(array, pos);
						common = readInt(array, pos + count(length));
						s = ShortArrays.ensureCapacity(s, length + common, common);
						copyFromBig(array, pos + count(length) + count(common), s, common, length);
						pos += count(length) + count(common) + length;
						length += common;
					} else {
						s = ShortArrays.ensureCapacity(s, length = length(i), 0);
						extract(i, s, 0, length);
					}
				}
				i++;
				return ShortArrays.copy(s, 0, length);
			}
	...
	/**
	 * Computes the pointer array using the currently set ratio, number of elements
