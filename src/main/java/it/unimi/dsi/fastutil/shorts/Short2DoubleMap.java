...
public interface Short2DoubleMap extends Short2DoubleFunction, Map<Short, Double> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Short2DoubleMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Short2DoubleMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final short key, final java.util.function.IntToDoubleFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsentNullable(final short key,
	 ...
	 * @since 8.0.0
	 */
	default double computeIfPresent(final short key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Double newValue = remappingFunction.apply(Short.valueOf(key), Double.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default double compute(final short key,
			final java.util.function.BiFunction<? super Short, ? super Double, ? extends Double> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final double oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Double newValue = remappingFunction.apply(Short.valueOf(key),
	 ...
	 * @since 8.0.0
	 */
	default double merge(final short key, final double value,
	...
	@Deprecated
	@Override
	default Double computeIfAbsent(final Short key,
	 ...
	 */
	interface Entry extends Map.Entry<Short, Double> {
		/**
