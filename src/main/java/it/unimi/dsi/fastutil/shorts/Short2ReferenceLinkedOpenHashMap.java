 ...
 * @see HashCommon
 */
public class Short2ReferenceLinkedOpenHashMap<V> extends AbstractShort2ReferenceSortedMap<V>
	...
	public Short2ReferenceLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Short2ReferenceLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Short2ReferenceLinkedOpenHashMap(final Map<? extends Short, ? extends V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Short2ReferenceLinkedOpenHashMap(final Map<? extends Short, ? extends V> m) {
	 ...
	 * @param f the load factor.
	 */
	public Short2ReferenceLinkedOpenHashMap(final Short2ReferenceMap<V> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Short2ReferenceLinkedOpenHashMap(final Short2ReferenceMap<V> m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2ReferenceLinkedOpenHashMap(final short[] k, final V[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Short2ReferenceLinkedOpenHashMap(final short[] k, final V[] v) {
	 ...
	 */
	public V getAndMoveToFirst(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public V getAndMoveToLast(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 */
	public V putAndMoveToFirst(final short k, final V v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public V putAndMoveToLast(final short k, final V v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				short curr;
				final short[] key = Short2ReferenceLinkedOpenHashMap.this.key;
	...
	private class EntryIterator extends MapIterator implements ObjectListIterator<Short2ReferenceMap.Entry<V>> {
	...
	private class FastEntryIterator extends MapIterator implements ObjectListIterator<Short2ReferenceMap.Entry<V>> {
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Short2ReferenceMap.Entry<V>>
		...
		@Override
		public ObjectSortedSet<Short2ReferenceMap.Entry<V>> subSet(Short2ReferenceMap.Entry<V> fromElement,
				Short2ReferenceMap.Entry<V> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((short) 0)))
				return Short2ReferenceLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2ReferenceLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Short2ReferenceMap.Entry<V>> iterator(final Short2ReferenceMap.Entry<V> from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Short2ReferenceMap.Entry<V>> fastIterator(final Short2ReferenceMap.Entry<V> from) {
		...
		public void forEach(final Consumer<? super Short2ReferenceMap.Entry<V>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractShort2ReferenceMap.BasicEntry<V>(key[curr], value[curr]));
			}
		}
