...
public interface Short2FloatMap extends Short2FloatFunction, Map<Short, Float> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Short2FloatMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Short2FloatMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final short key, final java.util.function.IntToDoubleFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsentNullable(final short key,
	 ...
	 * @since 8.0.0
	 */
	default float computeIfPresent(final short key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Float newValue = remappingFunction.apply(Short.valueOf(key), Float.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default float compute(final short key,
			final java.util.function.BiFunction<? super Short, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Float newValue = remappingFunction.apply(Short.valueOf(key), contained ? Float.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default float merge(final short key, final float value,
	...
	@Deprecated
	@Override
	default Float computeIfAbsent(final Short key,
	 ...
	 */
	interface Entry extends Map.Entry<Short, Float> {
		/**
