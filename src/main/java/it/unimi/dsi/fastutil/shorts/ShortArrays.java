...
public final class ShortArrays {
	 ...
	 */
	public static final short[] DEFAULT_EMPTY_ARRAY = {};
	 ...
	 */
	public static short[] forceCapacity(final short[] array, final int length, final int preserve) {
	...
	private static int med3(final short x[], final int a, final int b, final int c, ShortComparator comp) {
	...
	public static void quickSort(final short[] x, final short[] y, final int from, final int to) {
		...
		final short v = x[m], w = y[m];
		// Establish Invariant: v* (<v)* (>v)* v*
		int a = from, b = a, c = to - 1, d = c;
		while (true) {
			int comparison, t;
			while (b <= c
					&& (comparison = (t = (Short.compare((x[b]), (v)))) == 0 ? (Short.compare((y[b]), (w))) : t) <= 0) {
				if (comparison == 0)
					swap(x, y, a++, b);
				b++;
			}
			while (c >= b
					&& (comparison = (t = (Short.compare((x[c]), (v)))) == 0 ? (Short.compare((y[c]), (w))) : t) >= 0) {
				if (comparison == 0)
					swap(x, y, c, d--);
				c--;
			}
	...
	protected static class ForkJoinQuickSort2 extends RecursiveAction {
		...
		protected void compute() {
			...
			while (true) {
				int comparison, t;
				while (b <= c && (comparison = (t = (Short.compare((x[b]), (v)))) == 0 ? (Short.compare((y[b]), (w)))
						: t) <= 0) {
					if (comparison == 0)
						swap(x, y, a++, b);
					b++;
				}
				while (c >= b && (comparison = (t = (Short.compare((x[c]), (v)))) == 0 ? (Short.compare((y[c]), (w)))
						: t) >= 0) {
					if (comparison == 0)
						swap(x, y, c, d--);
					c--;
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final short a[], final int from, final int to) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final short a[]) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final short a[], final int from, final int to, ShortComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void unstableSort(final short a[], ShortComparator comp) {
	 ...
	 *             specified range.
	 */
	public static void mergeSort(final short a[], final int from, final int to, ShortComparator comp,
			final short supp[]) {
		int len = to - from;
		// Insertion sort on smallest arrays
		if (len < MERGESORT_NO_REC) {
	 ...
	 * @param comp the comparator to determine the sorting order.
	 */
	public static void mergeSort(final short a[], ShortComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final short a[], final int from, final int to) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final short a[]) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final short a[], final int from, final int to, ShortComparator comp) {
	 ...
	 * @since 8.3.0
	 */
	public static void stableSort(final short a[], ShortComparator comp) {
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final short[] a, final int from, final int to,
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void parallelRadixSortIndirect(final int perm[], final short[] a, final int from, final int to,
			final boolean stable) {
		if (to - from < PARALLEL_RADIXSORT_NO_FORK) {
	...
	private static void insertionSortIndirect(final int[] perm, final short[] a, final short[] b, final int from,
			final int to) {
		for (int i = from; ++i < to;) {
			int t = perm[i];
			int j = i;
	 ...
	 * @param stable whether the sorting algorithm should be stable.
	 */
	public static void radixSortIndirect(final int[] perm, final short[] a, final short[] b, final int from,
	...
	public static void radixSort(final short[][] a, final int from, final int to) {
		...
		while (stackPos > 0) {
			...
			final int level = levelStack[stackPos];
			final int signMask = level % DIGITS_PER_ELEMENT == 0 ? 1 << DIGIT_BITS - 1 : 0;
			final short[] k = a[level / DIGITS_PER_ELEMENT]; // This is the key array
			final int shift = (DIGITS_PER_ELEMENT - 1 - level % DIGITS_PER_ELEMENT) * DIGIT_BITS; // This is the shift
																									// that extract the
																									// right byte from a
																									// key
	...
	private static final class ArrayHashStrategy implements Hash.Strategy<short[]>, java.io.Serializable {
