 ...
 * @see HashCommon
 */
public class Short2ReferenceOpenCustomHashMap<V> extends AbstractShort2ReferenceMap<V>
	...
	public Short2ReferenceOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ReferenceOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ReferenceOpenCustomHashMap(final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ReferenceOpenCustomHashMap(final Map<? extends Short, ? extends V> m, final float f,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ReferenceOpenCustomHashMap(final Map<? extends Short, ? extends V> m,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ReferenceOpenCustomHashMap(final Short2ReferenceMap<V> m, final float f,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Short2ReferenceOpenCustomHashMap(final Short2ReferenceMap<V> m,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2ReferenceOpenCustomHashMap(final short[] k, final V[] v, final float f,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Short2ReferenceOpenCustomHashMap(final short[] k, final V[] v,
			final it.unimi.dsi.fastutil.shorts.ShortHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfAbsent(final short k, final java.util.function.IntFunction<? extends V> mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfPresent(final short k,
			final java.util.function.BiFunction<? super Short, ? super V, ? extends V> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final V newValue = remappingFunction.apply(Short.valueOf(k), (value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((short) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public V compute(final short k,
			final java.util.function.BiFunction<? super Short, ? super V, ? extends V> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final V newValue = remappingFunction.apply(Short.valueOf(k), pos >= 0 ? (value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), ((short) 0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public V merge(final short k, final V v,
	...
	final class MapEntry implements Short2ReferenceMap.Entry<V>, Map.Entry<Short, V> {
		...
		public boolean equals(final Object o) {
			...
			return (strategy.equals((key[index]), ((e.getKey()).shortValue()))) && ((value[index]) == ((e.getValue())));
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Short2ReferenceOpenCustomHashMap.this.remove(wrapped.getShort(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((short) 0))))
				return Short2ReferenceOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2ReferenceOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Short2ReferenceMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractShort2ReferenceMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((short) 0)))
					consumer.accept(new AbstractShort2ReferenceMap.BasicEntry<V>(key[pos], value[pos]));
		}
