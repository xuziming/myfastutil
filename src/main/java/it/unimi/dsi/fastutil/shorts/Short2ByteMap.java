...
public interface Short2ByteMap extends Short2ByteFunction, Map<Short, Byte> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Short2ByteMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Short2ByteMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default byte putIfAbsent(final short key, final byte value) {
	 ...
	 * @since 8.0.0
	 */
	default boolean remove(final short key, final byte value) {
	 ...
	 * @since 8.0.0
	 */
	default boolean replace(final short key, final byte oldValue, final byte newValue) {
		final byte curValue = get(key);
	 ...
	 * @since 8.0.0
	 */
	default byte replace(final short key, final byte value) {
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final short key, final java.util.function.IntUnaryOperator mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsentNullable(final short key,
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsentPartial(final short key, final Short2ByteFunction mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final byte v = get(key), drv = defaultReturnValue();
		if (v != drv || containsKey(key))
			return v;
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfPresent(final short key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Byte newValue = remappingFunction.apply(Short.valueOf(key), Byte.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default byte compute(final short key,
			final java.util.function.BiFunction<? super Short, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Byte newValue = remappingFunction.apply(Short.valueOf(key), contained ? Byte.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default byte merge(final short key, final byte value,
	...
	@Deprecated
	@Override
	default Byte computeIfAbsent(final Short key,
	 ...
	 */
	interface Entry extends Map.Entry<Short, Byte> {
		/**
