 ...
 *
 */
public class Short2LongRBTreeMap extends AbstractShort2LongSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Short2LongRBTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2LongRBTreeMap(final Map<? extends Short, ? extends Long> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2LongRBTreeMap(final SortedMap<Short, Long> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2LongRBTreeMap(final Short2LongMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2LongRBTreeMap(final Short2LongSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2LongRBTreeMap(final short[] k, final long v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2LongRBTreeMap(final short[] k, final long v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2LongRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2LongMap.Entry>() {
				final Comparator<? super Short2LongMap.Entry> comparator = (Comparator<Short2LongMap.Entry>) (x,
						y) -> Short2LongRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	...
	private final class Submap extends AbstractShort2LongSortedMap implements java.io.Serializable {
		...
		final boolean in(final short k) {
			return (bottom || Short2LongRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2LongRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2LongMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Long))
							return false;
						final Short2LongRBTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2LongRBTreeMap.this.containsKey(k);
		}
		...
		public long get(final short k) {
			final Short2LongRBTreeMap.Entry e;
			final short kk = k;
		...
		public long put(final short k, final long v) {
			...
			final long oldValue = Short2LongRBTreeMap.this.put(k, v);
		...
		public long remove(final short k) {
			...
			final long oldValue = Short2LongRBTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2LongRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2LongRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2LongRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2LongRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Short2LongRBTreeMap clone() {
		...
		try {
			c = (Short2LongRBTreeMap) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
