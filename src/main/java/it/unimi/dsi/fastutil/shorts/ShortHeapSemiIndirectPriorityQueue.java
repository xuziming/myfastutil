...
public class ShortHeapSemiIndirectPriorityQueue implements ShortIndirectPriorityQueue {
	 ...
	 *                 natural order.
	 */
	public ShortHeapSemiIndirectPriorityQueue(final short[] refArray, final int[] a, int size,
	...
	public void enqueue(int x) {
		...
		heap[size++] = x;
		ShortSemiIndirectHeaps.upHeap(refArray, heap, size, size - 1, c);
	}
	...
	public int dequeue() {
		...
		heap[0] = heap[--size];
		if (size != 0)
			ShortSemiIndirectHeaps.downHeap(refArray, heap, size, 0, c);
		return result;
	}
	...
	public void allChanged() {
		ShortSemiIndirectHeaps.makeHeap(refArray, heap, size, c);
	}
	...
	public int front(final int[] a) {
		return c == null ? ShortSemiIndirectHeaps.front(refArray, heap, size, a)
				: ShortSemiIndirectHeaps.front(refArray, heap, size, a, c);
	}
