 ...
 *
 */
public class Short2DoubleRBTreeMap extends AbstractShort2DoubleSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Short2DoubleRBTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2DoubleRBTreeMap(final Map<? extends Short, ? extends Double> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2DoubleRBTreeMap(final SortedMap<Short, Double> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2DoubleRBTreeMap(final Short2DoubleMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2DoubleRBTreeMap(final Short2DoubleSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2DoubleRBTreeMap(final short[] k, final double v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2DoubleRBTreeMap(final short[] k, final double v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2DoubleRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2DoubleMap.Entry>() {
				final Comparator<? super Short2DoubleMap.Entry> comparator = (Comparator<Short2DoubleMap.Entry>) (x,
						y) -> Short2DoubleRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	...
	private final class Submap extends AbstractShort2DoubleSortedMap implements java.io.Serializable {
		...
		final boolean in(final short k) {
			return (bottom || Short2DoubleRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2DoubleRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2DoubleMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Double))
							return false;
						final Short2DoubleRBTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2DoubleRBTreeMap.this.containsKey(k);
		}
		...
		public double get(final short k) {
			final Short2DoubleRBTreeMap.Entry e;
			final short kk = k;
		...
		public double put(final short k, final double v) {
			...
			final double oldValue = Short2DoubleRBTreeMap.this.put(k, v);
		...
		public double remove(final short k) {
			...
			final double oldValue = Short2DoubleRBTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2DoubleRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2DoubleRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2DoubleRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2DoubleRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
