 ...
 * @see HashCommon
 */
public class Short2ByteLinkedOpenHashMap extends AbstractShort2ByteSortedMap
	...
	public Short2ByteLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Short2ByteLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Short2ByteLinkedOpenHashMap(final Map<? extends Short, ? extends Byte> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Short2ByteLinkedOpenHashMap(final Map<? extends Short, ? extends Byte> m) {
	 ...
	 * @param f the load factor.
	 */
	public Short2ByteLinkedOpenHashMap(final Short2ByteMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Short2ByteLinkedOpenHashMap(final Short2ByteMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2ByteLinkedOpenHashMap(final short[] k, final byte[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Short2ByteLinkedOpenHashMap(final short[] k, final byte[] v) {
	 ...
	 */
	public byte getAndMoveToFirst(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public byte getAndMoveToLast(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToFirst(final short k, final byte v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public byte putAndMoveToLast(final short k, final byte v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				short curr;
				final short[] key = Short2ByteLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Short2ByteMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		@Override
		public ObjectSortedSet<Short2ByteMap.Entry> subSet(Short2ByteMap.Entry fromElement,
				Short2ByteMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((short) 0)))
				return Short2ByteLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2ByteLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Short2ByteMap.Entry> iterator(final Short2ByteMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Short2ByteMap.Entry> fastIterator(final Short2ByteMap.Entry from) {
		...
		public void forEach(final Consumer<? super Short2ByteMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractShort2ByteMap.BasicEntry(key[curr], value[curr]));
			}
		}
