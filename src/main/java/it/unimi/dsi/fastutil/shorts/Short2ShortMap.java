...
public interface Short2ShortMap extends Short2ShortFunction, Map<Short, Short> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Short2ShortMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Short2ShortMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final short key, final java.util.function.IntUnaryOperator mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsentNullable(final short key,
	 ...
	 * @since 8.0.0
	 */
	default short computeIfPresent(final short key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Short newValue = remappingFunction.apply(Short.valueOf(key), Short.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default short compute(final short key,
			final java.util.function.BiFunction<? super Short, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Short newValue = remappingFunction.apply(Short.valueOf(key), contained ? Short.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default short merge(final short key, final short value,
			final java.util.function.BiFunction<? super Short, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
	...
	@Deprecated
	@Override
	default Short computeIfAbsent(final Short key,
	 ...
	 */
	interface Entry extends Map.Entry<Short, Short> {
		/**
