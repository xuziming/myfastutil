 ...
 * @see HashCommon
 */
public class Short2ByteOpenHashMap extends AbstractShort2ByteMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Short2ByteOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Short2ByteOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Short2ByteOpenHashMap(final Map<? extends Short, ? extends Byte> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Short2ByteOpenHashMap(final Map<? extends Short, ? extends Byte> m) {
	 ...
	 * @param f the load factor.
	 */
	public Short2ByteOpenHashMap(final Short2ByteMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Short2ByteOpenHashMap(final Short2ByteMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2ByteOpenHashMap(final short[] k, final byte[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Short2ByteOpenHashMap(final short[] k, final byte[] v) {
	...
	final class MapEntry implements Short2ByteMap.Entry, Map.Entry<Short, Byte> {
		...
		public boolean equals(final Object o) {
			...
			return ((key[index]) == ((e.getKey()).shortValue())) && ((value[index]) == ((e.getValue()).byteValue()));
		}
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final short key[] = Short2ByteOpenHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Short2ByteOpenHashMap.this.remove(wrapped.getShort(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Short2ByteMap.Entry> implements FastEntrySet {
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((short) 0)))
				return Short2ByteOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2ByteOpenHashMap.this.key;
			int pos;
			// The starting point.
