 ...
 *
 */
public class Short2ShortRBTreeMap extends AbstractShort2ShortSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2ShortRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2ShortMap.Entry>() {
				final Comparator<? super Short2ShortMap.Entry> comparator = (Comparator<Short2ShortMap.Entry>) (x,
						y) -> Short2ShortRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	...
	private final class Submap extends AbstractShort2ShortSortedMap implements java.io.Serializable {
		...
		final boolean in(final short k) {
			return (bottom || Short2ShortRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2ShortRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2ShortMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Short))
							return false;
						final Short2ShortRBTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2ShortRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public short firstShortKey() {
			Short2ShortRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2ShortRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2ShortRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2ShortRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
