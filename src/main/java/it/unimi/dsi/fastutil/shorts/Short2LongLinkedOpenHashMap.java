 ...
 * @see HashCommon
 */
public class Short2LongLinkedOpenHashMap extends AbstractShort2LongSortedMap
	...
	public Short2LongLinkedOpenHashMap(final int expected, final float f) {
		if (f <= 0 || f > 1)
	 ...
	 * @param expected the expected number of elements in the hash map.
	 */
	public Short2LongLinkedOpenHashMap(final int expected) {
	 ...
	 * @param f the load factor.
	 */
	public Short2LongLinkedOpenHashMap(final Map<? extends Short, ? extends Long> m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a {@link Map} to be copied into the new hash map.
	 */
	public Short2LongLinkedOpenHashMap(final Map<? extends Short, ? extends Long> m) {
	 ...
	 * @param f the load factor.
	 */
	public Short2LongLinkedOpenHashMap(final Short2LongMap m, final float f) {
		this(m.size(), f);
		putAll(m);
	}
	 ...
	 * @param m a type-specific map to be copied into the new hash map.
	 */
	public Short2LongLinkedOpenHashMap(final Short2LongMap m) {
		this(m, DEFAULT_LOAD_FACTOR);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2LongLinkedOpenHashMap(final short[] k, final long[] v, final float f) {
		this(k.length, f);
	 ...
	 *                                  lengths.
	 */
	public Short2LongLinkedOpenHashMap(final short[] k, final long[] v) {
	 ...
	 */
	public long getAndMoveToFirst(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
				return value[n];
	 ...
	 *         default return value} if no value was present for the given key.
	 */
	public long getAndMoveToLast(final short k) {
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
				return value[n];
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public long putAndMoveToFirst(final short k, final long v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToFirst(n);
	 ...
	 *         return value} if no value was present for the given key.
	 */
	public long putAndMoveToLast(final short k, final long v) {
		int pos;
		if (((k) == ((short) 0))) {
			if (containsNullKey) {
				moveIndexToLast(n);
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (pos == n) {
				...
				short curr;
				final short[] key = Short2LongLinkedOpenHashMap.this.key;
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Short2LongMap.Entry> implements FastSortedEntrySet {
		@Override
		...
		@Override
		public ObjectSortedSet<Short2LongMap.Entry> subSet(Short2LongMap.Entry fromElement,
				Short2LongMap.Entry toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((short) 0)))
				return Short2LongLinkedOpenHashMap.this.containsNullKey && ((value[n]) == (v));
			short curr;
			final short[] key = Short2LongLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		 ...
		 */
		@Override
		public ObjectListIterator<Short2LongMap.Entry> iterator(final Short2LongMap.Entry from) {
		 ...
		 */
		@Override
		public ObjectListIterator<Short2LongMap.Entry> fastIterator(final Short2LongMap.Entry from) {
		...
		public void forEach(final Consumer<? super Short2LongMap.Entry> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractShort2LongMap.BasicEntry(key[curr], value[curr]));
			}
		}
