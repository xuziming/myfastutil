 ...
 *
 */
public class Short2CharRBTreeMap extends AbstractShort2CharSortedMap implements java.io.Serializable, Cloneable {
	 ...
	 */
	public Short2CharRBTreeMap(final Comparator<? super Short> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public Short2CharRBTreeMap(final Map<? extends Short, ? extends Character> m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2CharRBTreeMap(final SortedMap<Short, Character> m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 */
	public Short2CharRBTreeMap(final Short2CharMap m) {
		this();
		putAll(m);
	}
	 ...
	 */
	public Short2CharRBTreeMap(final Short2CharSortedMap m) {
		this(m.comparator());
		putAll(m);
	}
	 ...
	 *                                  lengths.
	 */
	public Short2CharRBTreeMap(final short[] k, final char v[], final Comparator<? super Short> c) {
		this(c);
	 ...
	 *                                  lengths.
	 */
	public Short2CharRBTreeMap(final short[] k, final char v[]) {
		this(k, v, null);
	}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Short2CharRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Short2CharMap.Entry>() {
				final Comparator<? super Short2CharMap.Entry> comparator = (Comparator<Short2CharMap.Entry>) (x,
						y) -> Short2CharRBTreeMap.this.actualComparator.compare(x.getShortKey(), y.getShortKey());
	...
	private final class Submap extends AbstractShort2CharSortedMap implements java.io.Serializable {
		...
		final boolean in(final short k) {
			return (bottom || Short2CharRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Short2CharRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Short2CharMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Character))
							return false;
						final Short2CharRBTreeMap.Entry f = findKey(((Short) (e.getKey())).shortValue());
		...
		public boolean containsKey(final short k) {
			return in(k) && Short2CharRBTreeMap.this.containsKey(k);
		}
		...
		public char get(final short k) {
			final Short2CharRBTreeMap.Entry e;
			final short kk = k;
		...
		public char put(final short k, final char v) {
			...
			final char oldValue = Short2CharRBTreeMap.this.put(k, v);
		...
		public char remove(final short k) {
			...
			final char oldValue = Short2CharRBTreeMap.this.remove(k);
		...
		@Override
		public short firstShortKey() {
			Short2CharRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public short lastShortKey() {
			Short2CharRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Short2CharRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Short2CharRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
