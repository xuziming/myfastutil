 ...
 * @see HashCommon
 */
public class Char2ObjectLinkedOpenHashMap<V> extends AbstractChar2ObjectSortedMap<V>
	...
	private final class MapEntrySet extends AbstractObjectSortedSet<Char2ObjectMap.Entry<V>>
		...
		@Override
		public ObjectSortedSet<Char2ObjectMap.Entry<V>> subSet(Char2ObjectMap.Entry<V> fromElement,
				Char2ObjectMap.Entry<V> toElement) {
			throw new UnsupportedOperationException();
		}
		...
		public boolean contains(final Object o) {
			...
			if (((k) == ((char) 0)))
				return Char2ObjectLinkedOpenHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			char curr;
			final char[] key = Char2ObjectLinkedOpenHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2ObjectMap.Entry<V>> consumer) {
			for (int i = size, curr, next = first; i-- != 0;) {
				curr = next;
				next = (int) link[curr];
				consumer.accept(new AbstractChar2ObjectMap.BasicEntry<V>(key[curr], value[curr]));
			}
		}
