 ...
 * @see HashCommon
 */
public class Char2IntOpenCustomHashMap extends AbstractChar2IntMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Char2IntOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Char2IntOpenCustomHashMap(final int expected, final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2IntOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2IntOpenCustomHashMap(final Map<? extends Character, ? extends Integer> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2IntOpenCustomHashMap(final Map<? extends Character, ? extends Integer> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2IntOpenCustomHashMap(final Char2IntMap m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2IntOpenCustomHashMap(final Char2IntMap m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2IntOpenCustomHashMap(final char[] k, final int[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2IntOpenCustomHashMap(final char[] k, final int[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsent(final char k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfAbsentNullable(final char k,
	...
	/** {@inheritDoc} */
	@Override
	public int computeIfPresent(final char k,
			final java.util.function.BiFunction<? super Character, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Integer newValue = remappingFunction.apply(Character.valueOf(k), Integer.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((char) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public int compute(final char k,
			final java.util.function.BiFunction<? super Character, ? super Integer, ? extends Integer> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Integer newValue = remappingFunction.apply(Character.valueOf(k),
	...
	/** {@inheritDoc} */
	@Override
	public int merge(final char k, final int v,
	...
	private class MapIterator {
		 ...
		 */
		boolean mustReturnNullKey = Char2IntOpenCustomHashMap.this.containsNullKey;
		/**
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2IntOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2IntMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((char) 0))))
				return Char2IntOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			char curr;
			final char[] key = Char2IntOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2IntMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2IntMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2IntMap.BasicEntry(key[pos], value[pos]));
		}
