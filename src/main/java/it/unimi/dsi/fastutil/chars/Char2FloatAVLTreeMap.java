 ...
 * iterator}.
 */
public class Char2FloatAVLTreeMap extends AbstractChar2FloatSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2FloatAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2FloatMap.Entry>() {
				final Comparator<? super Char2FloatMap.Entry> comparator = (Comparator<Char2FloatMap.Entry>) (x,
						y) -> Char2FloatAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
	...
	private final class Submap extends AbstractChar2FloatSortedMap implements java.io.Serializable {
		...
		final boolean in(final char k) {
			return (bottom || Char2FloatAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2FloatAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2FloatMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Float))
							return false;
						final Char2FloatAVLTreeMap.Entry f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2FloatAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2FloatAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2FloatAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2FloatAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2FloatAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
