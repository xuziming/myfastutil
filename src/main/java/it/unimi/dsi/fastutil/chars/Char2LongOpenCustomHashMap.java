 ...
 * @see HashCommon
 */
public class Char2LongOpenCustomHashMap extends AbstractChar2LongMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Char2LongOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Char2LongOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2LongOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2LongOpenCustomHashMap(final Map<? extends Character, ? extends Long> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2LongOpenCustomHashMap(final Map<? extends Character, ? extends Long> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2LongOpenCustomHashMap(final Char2LongMap m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2LongOpenCustomHashMap(final Char2LongMap m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2LongOpenCustomHashMap(final char[] k, final long[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2LongOpenCustomHashMap(final char[] k, final long[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsent(final char k, final java.util.function.IntToLongFunction mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfAbsentNullable(final char k,
	...
	/** {@inheritDoc} */
	@Override
	public long computeIfPresent(final char k,
			final java.util.function.BiFunction<? super Character, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Long newValue = remappingFunction.apply(Character.valueOf(k), Long.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((char) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public long compute(final char k,
			final java.util.function.BiFunction<? super Character, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Long newValue = remappingFunction.apply(Character.valueOf(k), pos >= 0 ? Long.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), ((char) 0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public long merge(final char k, final long v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2LongOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2LongMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((char) 0))))
				return Char2LongOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			char curr;
			final char[] key = Char2LongOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2LongMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2LongMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2LongMap.BasicEntry(key[pos], value[pos]));
		}
