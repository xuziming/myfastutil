 ...
 *
 */
public class Char2CharRBTreeMap extends AbstractChar2CharSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2CharRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2CharMap.Entry>() {
				final Comparator<? super Char2CharMap.Entry> comparator = (Comparator<Char2CharMap.Entry>) (x,
						y) -> Char2CharRBTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
	...
	private final class Submap extends AbstractChar2CharSortedMap implements java.io.Serializable {
		...
		final boolean in(final char k) {
			return (bottom || Char2CharRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2CharRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2CharMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Character))
							return false;
						final Char2CharRBTreeMap.Entry f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2CharRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2CharRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2CharRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2CharRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2CharRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
