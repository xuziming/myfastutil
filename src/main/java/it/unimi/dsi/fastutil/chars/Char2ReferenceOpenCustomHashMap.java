...
public class Char2ReferenceOpenCustomHashMap<V> extends AbstractChar2ReferenceMap<V>
	...
	public Char2ReferenceOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ReferenceOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ReferenceOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ReferenceOpenCustomHashMap(final Map<? extends Character, ? extends V> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ReferenceOpenCustomHashMap(final Map<? extends Character, ? extends V> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ReferenceOpenCustomHashMap(final Char2ReferenceMap<V> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ReferenceOpenCustomHashMap(final Char2ReferenceMap<V> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2ReferenceOpenCustomHashMap(final char[] k, final V[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2ReferenceOpenCustomHashMap(final char[] k, final V[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	final class MapEntry implements Char2ReferenceMap.Entry<V>, Map.Entry<Character, V> {
		...
		public boolean equals(final Object o) {
			...
			return (strategy.equals((key[index]), ((e.getKey()).charValue()))) && ((value[index]) == ((e.getValue())));
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2ReferenceOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2ReferenceMap.Entry<V>> implements FastEntrySet<V> {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((char) 0))))
				return Char2ReferenceOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			char curr;
			final char[] key = Char2ReferenceOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2ReferenceMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2ReferenceMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2ReferenceMap.BasicEntry<V>(key[pos], value[pos]));
		}
