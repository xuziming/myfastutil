...
public interface Char2ShortMap extends Char2ShortFunction, Map<Character, Short> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Char2ShortMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Char2ShortMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsent(final char key, final java.util.function.IntUnaryOperator mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default short computeIfAbsentNullable(final char key,
	 ...
	 * @since 8.0.0
	 */
	default short computeIfPresent(final char key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Short newValue = remappingFunction.apply(Character.valueOf(key), Short.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default short compute(final char key,
			final java.util.function.BiFunction<? super Character, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final short oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Short newValue = remappingFunction.apply(Character.valueOf(key),
				contained ? Short.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default short merge(final char key, final short value,
	...
	@Deprecated
	@Override
	default Short computeIfAbsent(final Character key,
	...
	@Deprecated
	@Override
	default Short computeIfPresent(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Short, ? extends Short> remappingFunction) {
	...
	@Deprecated
	@Override
	default Short compute(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Short, ? extends Short> remappingFunction) {
	...
	@Deprecated
	@Override
	default Short merge(final Character key, final Short value,
			final java.util.function.BiFunction<? super Short, ? super Short, ? extends Short> remappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Character, Short> {
		/**
