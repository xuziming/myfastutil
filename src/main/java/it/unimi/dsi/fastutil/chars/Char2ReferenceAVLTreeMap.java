 ...
 * iterator}.
 */
public class Char2ReferenceAVLTreeMap<V> extends AbstractChar2ReferenceSortedMap<V>
	...
	private static final class Entry<V> extends AbstractChar2ReferenceMap.BasicEntry<V> implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).charValue())) && ((value) == ((e.getValue())));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2ReferenceAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Char2ReferenceMap.Entry<V>> char2ReferenceEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2ReferenceMap.Entry<V>>() {
				final Comparator<? super Char2ReferenceMap.Entry<V>> comparator = (Comparator<Char2ReferenceMap.Entry<V>>) (
						x, y) -> Char2ReferenceAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
				...
				@Override
				public ObjectBidirectionalIterator<Char2ReferenceMap.Entry<V>> iterator(
						final Char2ReferenceMap.Entry<V> from) {
				...
				@Override
				public ObjectSortedSet<Char2ReferenceMap.Entry<V>> subSet(Char2ReferenceMap.Entry<V> from,
						Char2ReferenceMap.Entry<V> to) {
					return subMap(from.getCharKey(), to.getCharKey()).char2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2ReferenceMap.Entry<V>> headSet(Char2ReferenceMap.Entry<V> to) {
					return headMap(to.getCharKey()).char2ReferenceEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2ReferenceMap.Entry<V>> tailSet(Char2ReferenceMap.Entry<V> from) {
					return tailMap(from.getCharKey()).char2ReferenceEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractChar2ReferenceSortedMap<V> implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final char k) {
			return (bottom || Char2ReferenceAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2ReferenceAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Character))
							return false;
						final Char2ReferenceAVLTreeMap.Entry<V> f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2ReferenceAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2ReferenceAVLTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2ReferenceAVLTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2ReferenceAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2ReferenceAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	private Entry<V> readTree(final java.io.ObjectInputStream s, final int n, final Entry<V> pred, final Entry<V> succ)
		...
		if (n == 1) {
			final Entry<V> top = new Entry<>(s.readChar(), (V) s.readObject());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
		if (n == 2) {
			 ...
			 */
			final Entry<V> top = new Entry<>(s.readChar(), (V) s.readObject());
			top.right(new Entry<>(s.readChar(), (V) s.readObject()));
			top.right.pred(top);
			top.balance(1);
			top.pred(pred);
			top.right.succ(succ);
