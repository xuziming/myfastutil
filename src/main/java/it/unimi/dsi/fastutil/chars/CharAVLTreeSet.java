 ...
 * Moreover, the iterator returned by {@code iterator()} can be safely cast to a
 * type-specific {@linkplain java.util.ListIterator list iterator}.
 */
public class CharAVLTreeSet extends AbstractCharSortedSet implements java.io.Serializable, Cloneable, CharSortedSet {
	...
	/**
	 * This set's actual comparator; it may differ from {@link #storedComparator}
	...
	private void setActualComparator() {
		actualComparator = CharComparators.asCharComparator(storedComparator);
	}
	 ...
	 */
	public CharAVLTreeSet(final Comparator<? super Character> c) {
		this();
		storedComparator = c;
		setActualComparator();
	}
	 ...
	 */
	public CharAVLTreeSet(final Collection<? extends Character> c) {
		this();
		addAll(c);
	}
	 ...
	 */
	public CharAVLTreeSet(final SortedSet<Character> s) {
		this(s.comparator());
		addAll(s);
	}
	 ...
	 */
	public CharAVLTreeSet(final CharCollection c) {
		this();
		addAll(c);
	}
	 ...
	 */
	public CharAVLTreeSet(final CharSortedSet s) {
		this(s.comparator());
		addAll(s);
	}
	 ...
	 */
	public CharAVLTreeSet(final CharIterator i) {
		while (i.hasNext())
			add(i.nextChar());
	}
	 ...
	 * @param c      a {@link Comparator} (even better, a type-specific comparator).
	 */
	public CharAVLTreeSet(final char[] a, final int offset, final int length, final Comparator<? super Character> c) {
		this(c);
	 ...
	 * @param length the number of elements to use.
	 */
	public CharAVLTreeSet(final char[] a, final int offset, final int length) {
	 ...
	 */
	public CharAVLTreeSet(final char[] a) {
		this();
		int i = a.length;
		while (i-- != 0)
			add(a[i]);
	 ...
	 * @param c a {@link Comparator} (even better, a type-specific comparator).
	 */
	public CharAVLTreeSet(final char[] a, final Comparator<? super Character> c) {
		this(c);
		int i = a.length;
		while (i-- != 0)
			add(a[i]);
	 ...
	 *         key exists.
	 */
	private Entry findKey(final char k) {
		Entry e = tree;
		int cmp;
	 ...
	 * about balance, predecessor status and successor status.
	 *
	 * <p>
	...
	private static final class Entry implements Cloneable {
		...
		/**
		 * The bits in this mask hold the node balance info. You can get it just by
		 * casting to byte.
		 */
		 ...
		 * System.err.print("  "); System.err.println(key + " (" + level() + ")"); if
	...
	private class SetIterator implements CharListIterator {
		 ...
		 * that this value is not meaningful when this {@link SetIterator} has been
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			CharAVLTreeSet.this.remove(curr.key);
			curr = null;
		}
	}
	...
	public CharSortedSet headSet(final char to) {
		return new Subset(((char) 0), true, to, false);
	}

	@Override
	public CharSortedSet tailSet(final char from) {
		return new Subset(from, false, ((char) 0), true);
	}
	 ...
	 * (which can be set to -&infin; or &infin;). Since the subset is a view on the
	 * set, at a given moment it could happen that the limits of the range are not
	 * any longer in the main set. Thus, things such as
	 * {@link java.util.SortedSet#first()} or {@link java.util.SortedSet#size()}
	 * must be always computed on-the-fly.
	 */
	private final class Subset extends AbstractCharSortedSet implements java.io.Serializable, CharSortedSet {
		 ...
		 * @param bottom if true, the first parameter is ignored and the range starts
		 *               from -&infin;.
		 * @param to     the end of the subset range.
		 * @param top    if true, the third parameter is ignored and the range goes to
		 *               &infin;.
		 */
		public Subset(final char from, final boolean bottom, final char to, final boolean top) {
			if (!bottom && !top && CharAVLTreeSet.this.compare(from, to) > 0)
				throw new IllegalArgumentException(
						"Start element (" + from + ") is larger than end element (" + to + ")");
			this.from = from;
			this.bottom = bottom;
			this.to = to;
			this.top = top;
		...
		final boolean in(final char k) {
			return (bottom || CharAVLTreeSet.this.compare(k, from) >= 0)
					&& (top || CharAVLTreeSet.this.compare(k, to) < 0);
		}

		@Override
		public boolean contains(final char k) {
			return in(k) && CharAVLTreeSet.this.contains(k);
		}

		@Override
		public boolean add(final char k) {
			if (!in(k))
				throw new IllegalArgumentException("Element (" + k + ") out of range ["
		 ...
		 *
		 * @return the first entry of this subset, or {@code null} if the subset is
		 *         empty.
		 */
		public CharAVLTreeSet.Entry firstEntry() {
			if (tree == null)
				return null;
			// If this subset goes to -infinity, we return the main set first entry;
			// otherwise, we locate the start of the set.
			...
			// Finally, if this subset doesn't go to infinity, we check that the resulting
		 ...
		 *
		 * @return the last entry of this subset, or {@code null} if the subset is
		 *         empty.
		 */
		public CharAVLTreeSet.Entry lastEntry() {
			if (tree == null)
				return null;
			// If this subset goes to infinity, we return the main set last entry;
			// otherwise, we locate the end of the set.
			CharAVLTreeSet.Entry e;
			if (top)
				e = lastEntry;
			else {
		...
		@Override
		public char firstChar() {
			CharAVLTreeSet.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastChar() {
			CharAVLTreeSet.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		 ...
		 * of the subset we just overwrite the next or previous entry with {@code null}.
		 */
		private final class SubsetIterator extends SetIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && CharAVLTreeSet.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && CharAVLTreeSet.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	}
	 ...
	 * @param pred the entry containing the key that preceeds the first key in the
	 *             tree.
	...
	private Entry readTree(final java.io.ObjectInputStream s, final int n, final Entry pred, final Entry succ)
		...
		if (n == 1) {
			final Entry top = new Entry(s.readChar());
			top.pred(pred);
			top.succ(succ);
			return top;
		}
		...
		if (n == (n & -n))
			top.balance(1); // Quick test for determining whether n is a power of 2.
		return top;
	}
