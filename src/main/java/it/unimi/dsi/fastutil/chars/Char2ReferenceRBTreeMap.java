 ...
 *
 */
public class Char2ReferenceRBTreeMap<V> extends AbstractChar2ReferenceSortedMap<V>
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2ReferenceRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2ReferenceMap.Entry<V>>() {
				final Comparator<? super Char2ReferenceMap.Entry<V>> comparator = (Comparator<Char2ReferenceMap.Entry<V>>) (
						x, y) -> Char2ReferenceRBTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
	...
	private final class Submap extends AbstractChar2ReferenceSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final char k) {
			return (bottom || Char2ReferenceRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2ReferenceRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2ReferenceMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Character))
							return false;
						final Char2ReferenceRBTreeMap.Entry<V> f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2ReferenceRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2ReferenceRBTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2ReferenceRBTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2ReferenceRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2ReferenceRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
