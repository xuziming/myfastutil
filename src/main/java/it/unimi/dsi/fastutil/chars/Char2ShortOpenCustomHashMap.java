...
public class Char2ShortOpenCustomHashMap extends AbstractChar2ShortMap
	...
	public Char2ShortOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ShortOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ShortOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ShortOpenCustomHashMap(final Map<? extends Character, ? extends Short> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ShortOpenCustomHashMap(final Map<? extends Character, ? extends Short> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ShortOpenCustomHashMap(final Char2ShortMap m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ShortOpenCustomHashMap(final Char2ShortMap m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2ShortOpenCustomHashMap(final char[] k, final short[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2ShortOpenCustomHashMap(final char[] k, final short[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsent(final char k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfAbsentNullable(final char k,
	...
	/** {@inheritDoc} */
	@Override
	public short computeIfPresent(final char k,
			final java.util.function.BiFunction<? super Character, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Short newValue = remappingFunction.apply(Character.valueOf(k), Short.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((char) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public short compute(final char k,
			final java.util.function.BiFunction<? super Character, ? super Short, ? extends Short> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Short newValue = remappingFunction.apply(Character.valueOf(k),
				pos >= 0 ? Short.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), ((char) 0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public short merge(final char k, final short v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final char key[] = Char2ShortOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2ShortOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2ShortMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((char) 0))))
				return Char2ShortOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			char curr;
			final char[] key = Char2ShortOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2ShortMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2ShortMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2ShortMap.BasicEntry(key[pos], value[pos]));
		}
