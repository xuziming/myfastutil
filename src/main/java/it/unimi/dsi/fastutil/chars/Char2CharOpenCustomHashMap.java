 ...
 * @see HashCommon
 */
public class Char2CharOpenCustomHashMap extends AbstractChar2CharMap implements java.io.Serializable, Cloneable, Hash {
	...
	public Char2CharOpenCustomHashMap(final int expected, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2CharOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2CharOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2CharOpenCustomHashMap(final Map<? extends Character, ? extends Character> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2CharOpenCustomHashMap(final Map<? extends Character, ? extends Character> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2CharOpenCustomHashMap(final Char2CharMap m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2CharOpenCustomHashMap(final Char2CharMap m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2CharOpenCustomHashMap(final char[] k, final char[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2CharOpenCustomHashMap(final char[] k, final char[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	private int find(final char k) {
		if ((strategy.equals((k), ((char) 0))))
			return containsNullKey ? n : -(n + 1);
		char curr;
		final char[] key = this.key;
		int pos;
		// The starting point.
		if (((curr = key[pos = (it.unimi.dsi.fastutil.HashCommon.mix(strategy.hashCode(k))) & mask]) == ((char) 0)))
			return -(pos + 1);
		if ((strategy.equals((k), (curr))))
			return pos;
		// There's always an unused entry.
		while (true) {
			if (((curr = key[pos = (pos + 1) & mask]) == ((char) 0)))
				return -(pos + 1);
			if ((strategy.equals((k), (curr))))
				return pos;
		}
	...
	private void insert(final int pos, final char k, final char v) {
		if (pos == n)
	...
	public char getOrDefault(final char k, final char defaultValue) {
		if ((strategy.equals((k), ((char) 0))))
	...
	/** {@inheritDoc} */
	@Override
	public char putIfAbsent(final char k, final char v) {
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
		insert(-pos - 1, k, v);
	...
	public boolean remove(final char k, final char v) {
		if ((strategy.equals((k), ((char) 0)))) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean replace(final char k, final char oldValue, final char v) {
		final int pos = find(k);
	...
	/** {@inheritDoc} */
	@Override
	public char replace(final char k, final char v) {
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final char oldValue = value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfAbsent(final char k, final java.util.function.IntUnaryOperator mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfAbsentNullable(final char k,
			final java.util.function.IntFunction<? extends Character> mappingFunction) {
		java.util.Objects.requireNonNull(mappingFunction);
		final int pos = find(k);
		if (pos >= 0)
			return value[pos];
	...
	/** {@inheritDoc} */
	@Override
	public char computeIfPresent(final char k,
			final java.util.function.BiFunction<? super Character, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Character newValue = remappingFunction.apply(Character.valueOf(k), Character.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((char) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public char compute(final char k,
			final java.util.function.BiFunction<? super Character, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Character newValue = remappingFunction.apply(Character.valueOf(k),
				pos >= 0 ? Character.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), ((char) 0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public char merge(final char k, final char v,
			final java.util.function.BiFunction<? super Character, ? super Character, ? extends Character> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0) {
	...
	final class MapEntry implements Char2CharMap.Entry, Map.Entry<Character, Character> {
		...
		public boolean equals(final Object o) {
			...
			return (strategy.equals((key[index]), ((e.getKey()).charValue())))
					&& ((value[index]) == ((e.getValue()).charValue()));
		}
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2CharOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2CharMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((char) 0))))
				return Char2CharOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			char curr;
			final char[] key = Char2CharOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2CharMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2CharMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2CharMap.BasicEntry(key[pos], value[pos]));
		}

		/** {@inheritDoc} */
		@Override
		public void fastForEach(final Consumer<? super Char2CharMap.Entry> consumer) {
