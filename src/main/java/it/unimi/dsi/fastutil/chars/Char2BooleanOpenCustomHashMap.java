...
public class Char2BooleanOpenCustomHashMap extends AbstractChar2BooleanMap
	...
	public Char2BooleanOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Char2BooleanOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2BooleanOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2BooleanOpenCustomHashMap(final Map<? extends Character, ? extends Boolean> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2BooleanOpenCustomHashMap(final Map<? extends Character, ? extends Boolean> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2BooleanOpenCustomHashMap(final Char2BooleanMap m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2BooleanOpenCustomHashMap(final Char2BooleanMap m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2BooleanOpenCustomHashMap(final char[] k, final boolean[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2BooleanOpenCustomHashMap(final char[] k, final boolean[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean computeIfAbsent(final char k, final java.util.function.IntPredicate mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public boolean computeIfAbsentNullable(final char k,
	...
	/** {@inheritDoc} */
	@Override
	public boolean computeIfPresent(final char k,
			final java.util.function.BiFunction<? super Character, ? super Boolean, ? extends Boolean> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		if (pos < 0)
			return defRetValue;
		final Boolean newValue = remappingFunction.apply(Character.valueOf(k), Boolean.valueOf(value[pos]));
		if (newValue == null) {
			if ((strategy.equals((k), ((char) 0))))
				removeNullEntry();
			else
	...
	/** {@inheritDoc} */
	@Override
	public boolean compute(final char k,
			final java.util.function.BiFunction<? super Character, ? super Boolean, ? extends Boolean> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Boolean newValue = remappingFunction.apply(Character.valueOf(k),
	...
	/** {@inheritDoc} */
	@Override
	public boolean merge(final char k, final boolean v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final char key[] = Char2BooleanOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2BooleanOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2BooleanMap.Entry> implements FastEntrySet {
		@Override
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((char) 0))))
				return Char2BooleanOpenCustomHashMap.this.containsNullKey && ((value[n]) == (v));
			char curr;
			final char[] key = Char2BooleanOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2BooleanMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2BooleanMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2BooleanMap.BasicEntry(key[pos], value[pos]));
		}
