 ...
 *
 */
public class Char2ByteRBTreeMap extends AbstractChar2ByteSortedMap implements java.io.Serializable, Cloneable {
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2ByteRBTreeMap.this.remove(curr.key);
			curr = null;
		}
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2ByteMap.Entry>() {
				final Comparator<? super Char2ByteMap.Entry> comparator = (Comparator<Char2ByteMap.Entry>) (x,
						y) -> Char2ByteRBTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
	...
	private final class Submap extends AbstractChar2ByteSortedMap implements java.io.Serializable {
		...
		final boolean in(final char k) {
			return (bottom || Char2ByteRBTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2ByteRBTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2ByteMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Byte))
							return false;
						final Char2ByteRBTreeMap.Entry f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2ByteRBTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2ByteRBTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2ByteRBTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2ByteRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2ByteRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
