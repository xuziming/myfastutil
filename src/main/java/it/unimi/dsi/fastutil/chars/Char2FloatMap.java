...
public interface Char2FloatMap extends Char2FloatFunction, Map<Character, Float> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Char2FloatMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Char2FloatMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsent(final char key, final java.util.function.IntToDoubleFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default float computeIfAbsentNullable(final char key,
	 ...
	 * @since 8.0.0
	 */
	default float computeIfPresent(final char key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Float newValue = remappingFunction.apply(Character.valueOf(key), Float.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default float compute(final char key,
			final java.util.function.BiFunction<? super Character, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final float oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Float newValue = remappingFunction.apply(Character.valueOf(key),
				contained ? Float.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default float merge(final char key, final float value,
	...
	@Deprecated
	@Override
	default Float computeIfAbsent(final Character key,
	...
	@Deprecated
	@Override
	default Float computeIfPresent(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Float, ? extends Float> remappingFunction) {
	...
	@Deprecated
	@Override
	default Float compute(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Float, ? extends Float> remappingFunction) {
	...
	@Deprecated
	@Override
	default Float merge(final Character key, final Float value,
			final java.util.function.BiFunction<? super Float, ? super Float, ? extends Float> remappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Character, Float> {
		/**
