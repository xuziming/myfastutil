...
public class Char2FloatOpenCustomHashMap extends AbstractChar2FloatMap
	...
	public Char2FloatOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Char2FloatOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2FloatOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2FloatOpenCustomHashMap(final Map<? extends Character, ? extends Float> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2FloatOpenCustomHashMap(final Map<? extends Character, ? extends Float> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2FloatOpenCustomHashMap(final Char2FloatMap m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2FloatOpenCustomHashMap(final Char2FloatMap m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2FloatOpenCustomHashMap(final char[] k, final float[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2FloatOpenCustomHashMap(final char[] k, final float[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	public boolean containsValue(final float v) {
		...
		if (containsNullKey && (Float.floatToIntBits(value[n]) == Float.floatToIntBits(v)))
			return true;
		for (int i = n; i-- != 0;)
			if (!((key[i]) == ((char) 0)) && (Float.floatToIntBits(value[i]) == Float.floatToIntBits(v)))
				return true;
		return false;
	}
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfAbsentNullable(final char k,
	...
	/** {@inheritDoc} */
	@Override
	public float computeIfPresent(final char k,
			final java.util.function.BiFunction<? super Character, ? super Float, ? extends Float> remappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public float compute(final char k,
			final java.util.function.BiFunction<? super Character, ? super Float, ? extends Float> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final int pos = find(k);
		final Float newValue = remappingFunction.apply(Character.valueOf(k),
				pos >= 0 ? Float.valueOf(value[pos]) : null);
		if (newValue == null) {
			if (pos >= 0) {
				if ((strategy.equals((k), ((char) 0))))
					removeNullEntry();
	...
	/** {@inheritDoc} */
	@Override
	public float merge(final char k, final float v,
	...
	private class MapIterator {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2FloatOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2FloatMap.Entry> implements FastEntrySet {
		...
		public void forEach(final Consumer<? super Char2FloatMap.Entry> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2FloatMap.BasicEntry(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2FloatMap.BasicEntry(key[pos], value[pos]));
		}
