...
public interface Char2LongMap extends Char2LongFunction, Map<Character, Long> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Char2LongMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Char2LongMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	...
	default long putIfAbsent(final char key, final long value) {
		final long v = get(key), drv = defaultReturnValue();
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsent(final char key, final java.util.function.IntToLongFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default long computeIfAbsentNullable(final char key,
	 ...
	 * @since 8.0.0
	 */
	default long computeIfPresent(final char key,
			final java.util.function.BiFunction<? super Character, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Long newValue = remappingFunction.apply(Character.valueOf(key), Long.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default long compute(final char key,
			final java.util.function.BiFunction<? super Character, ? super Long, ? extends Long> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final long oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Long newValue = remappingFunction.apply(Character.valueOf(key),
				contained ? Long.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default long merge(final char key, final long value,
	...
	@Deprecated
	@Override
	default Long computeIfAbsent(final Character key,
	...
	@Deprecated
	@Override
	default Long computeIfPresent(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Long, ? extends Long> remappingFunction) {
	...
	@Deprecated
	@Override
	default Long compute(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Long, ? extends Long> remappingFunction) {
	...
	@Deprecated
	@Override
	default Long merge(final Character key, final Long value,
			final java.util.function.BiFunction<? super Long, ? super Long, ? extends Long> remappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Character, Long> {
		/**
