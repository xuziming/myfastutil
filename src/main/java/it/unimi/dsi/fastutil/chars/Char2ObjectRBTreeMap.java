...
public class Char2ObjectRBTreeMap<V> extends AbstractChar2ObjectSortedMap<V>
		...
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2ObjectMap.Entry<V>>() {
				final Comparator<? super Char2ObjectMap.Entry<V>> comparator = (Comparator<Char2ObjectMap.Entry<V>>) (x,
						y) -> Char2ObjectRBTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
	...
	private final class Submap extends AbstractChar2ObjectSortedMap<V> implements java.io.Serializable {
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2ObjectRBTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2ObjectRBTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
