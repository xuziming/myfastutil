 ...
 * iterator}.
 */
public class Char2CharAVLTreeMap extends AbstractChar2CharSortedMap implements java.io.Serializable, Cloneable {
	...
	private static final class Entry extends AbstractChar2CharMap.BasicEntry implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).charValue())) && ((value) == ((e.getValue()).charValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2CharAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Char2CharMap.Entry> char2CharEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2CharMap.Entry>() {
				final Comparator<? super Char2CharMap.Entry> comparator = (Comparator<Char2CharMap.Entry>) (x,
						y) -> Char2CharAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
				...
				public ObjectSortedSet<Char2CharMap.Entry> subSet(Char2CharMap.Entry from, Char2CharMap.Entry to) {
					return subMap(from.getCharKey(), to.getCharKey()).char2CharEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2CharMap.Entry> headSet(Char2CharMap.Entry to) {
					return headMap(to.getCharKey()).char2CharEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2CharMap.Entry> tailSet(Char2CharMap.Entry from) {
					return tailMap(from.getCharKey()).char2CharEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractChar2CharSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final char k) {
			return (bottom || Char2CharAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2CharAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2CharMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Character))
							return false;
						final Char2CharAVLTreeMap.Entry f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2CharAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2CharAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2CharAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2CharAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2CharAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
