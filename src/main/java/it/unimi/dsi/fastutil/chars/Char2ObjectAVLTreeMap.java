 ...
 * iterator}.
 */
public class Char2ObjectAVLTreeMap<V> extends AbstractChar2ObjectSortedMap<V>
	...
	private static final class Entry<V> extends AbstractChar2ObjectMap.BasicEntry<V> implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).charValue())) && java.util.Objects.equals(value, (e.getValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2ObjectAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Char2ObjectMap.Entry<V>> char2ObjectEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2ObjectMap.Entry<V>>() {
				final Comparator<? super Char2ObjectMap.Entry<V>> comparator = (Comparator<Char2ObjectMap.Entry<V>>) (x,
						y) -> Char2ObjectAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
				...
				@Override
				public ObjectBidirectionalIterator<Char2ObjectMap.Entry<V>> iterator(
						final Char2ObjectMap.Entry<V> from) {
				...
				@Override
				public ObjectSortedSet<Char2ObjectMap.Entry<V>> subSet(Char2ObjectMap.Entry<V> from,
						Char2ObjectMap.Entry<V> to) {
					return subMap(from.getCharKey(), to.getCharKey()).char2ObjectEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2ObjectMap.Entry<V>> headSet(Char2ObjectMap.Entry<V> to) {
					return headMap(to.getCharKey()).char2ObjectEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2ObjectMap.Entry<V>> tailSet(Char2ObjectMap.Entry<V> from) {
					return tailMap(from.getCharKey()).char2ObjectEntrySet();
				}
			};
		return entries;
	}
	...
	private final class Submap extends AbstractChar2ObjectSortedMap<V> implements java.io.Serializable {
		...
		final boolean in(final char k) {
			return (bottom || Char2ObjectAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2ObjectAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2ObjectMap.Entry<V>>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getKey() == null || !(e.getKey() instanceof Character))
							return false;
						final Char2ObjectAVLTreeMap.Entry<V> f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2ObjectAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2ObjectAVLTreeMap.Entry<V> e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2ObjectAVLTreeMap.Entry<V> e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2ObjectAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2ObjectAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
	...
	public Char2ObjectAVLTreeMap<V> clone() {
		...
		try {
			c = (Char2ObjectAVLTreeMap<V>) super.clone();
		} catch (CloneNotSupportedException cantHappen) {
			throw new InternalError();
		}
		c.keys = null;
