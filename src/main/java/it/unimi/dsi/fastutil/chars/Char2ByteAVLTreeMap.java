 ...
 * iterator}.
 */
public class Char2ByteAVLTreeMap extends AbstractChar2ByteSortedMap implements java.io.Serializable, Cloneable {
	...
	private static final class Entry extends AbstractChar2ByteMap.BasicEntry implements Cloneable {
		...
		public boolean equals(final Object o) {
			...
			return ((key) == ((e.getKey()).charValue())) && ((value) == ((e.getValue()).byteValue()));
		}
	...
	private class TreeIterator {
		...
		public void remove() {
			...
			updatePrevious();
			updateNext();
			Char2ByteAVLTreeMap.this.remove(curr.key);
			curr = null;
		}
	...
	public ObjectSortedSet<Char2ByteMap.Entry> char2ByteEntrySet() {
		if (entries == null)
			entries = new AbstractObjectSortedSet<Char2ByteMap.Entry>() {
				final Comparator<? super Char2ByteMap.Entry> comparator = (Comparator<Char2ByteMap.Entry>) (x,
						y) -> Char2ByteAVLTreeMap.this.actualComparator.compare(x.getCharKey(), y.getCharKey());
				...
				public ObjectSortedSet<Char2ByteMap.Entry> subSet(Char2ByteMap.Entry from, Char2ByteMap.Entry to) {
					return subMap(from.getCharKey(), to.getCharKey()).char2ByteEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2ByteMap.Entry> headSet(Char2ByteMap.Entry to) {
					return headMap(to.getCharKey()).char2ByteEntrySet();
				}

				@Override
				public ObjectSortedSet<Char2ByteMap.Entry> tailSet(Char2ByteMap.Entry from) {
					return tailMap(from.getCharKey()).char2ByteEntrySet();
				}
			};
		return entries;
	}
	 ...
	 */
	private final class Submap extends AbstractChar2ByteSortedMap implements java.io.Serializable {
		private static final long serialVersionUID = -7046029254386353129L;
		...
		final boolean in(final char k) {
			return (bottom || Char2ByteAVLTreeMap.this.compare(k, from) >= 0)
					&& (top || Char2ByteAVLTreeMap.this.compare(k, to) < 0);
		}
			...
			if (entries == null)
				entries = new AbstractObjectSortedSet<Char2ByteMap.Entry>() {
					...
					public boolean contains(final Object o) {
						...
						if (e.getValue() == null || !(e.getValue() instanceof Byte))
							return false;
						final Char2ByteAVLTreeMap.Entry f = findKey(((Character) (e.getKey())).charValue());
		...
		public boolean containsKey(final char k) {
			return in(k) && Char2ByteAVLTreeMap.this.containsKey(k);
		}
		...
		@Override
		public char firstCharKey() {
			Char2ByteAVLTreeMap.Entry e = firstEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		@Override
		public char lastCharKey() {
			Char2ByteAVLTreeMap.Entry e = lastEntry();
			if (e == null)
				throw new NoSuchElementException();
			return e.key;
		}
		...
		private class SubmapIterator extends TreeIterator {
			...
			void updatePrevious() {
				prev = prev.prev();
				if (!bottom && prev != null && Char2ByteAVLTreeMap.this.compare(prev.key, from) < 0)
					prev = null;
			}
			...
			void updateNext() {
				next = next.next();
				if (!top && next != null && Char2ByteAVLTreeMap.this.compare(next.key, to) >= 0)
					next = null;
			}
		}
