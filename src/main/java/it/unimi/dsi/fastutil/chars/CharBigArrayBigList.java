...
public class CharBigArrayBigList extends AbstractCharBigList implements RandomAccess, Cloneable, java.io.Serializable {
	...
	public CharBigArrayBigList(final long capacity) {
		...
		if (capacity == 0)
			a = CharBigArrays.EMPTY_BIG_ARRAY;
		else
	...
	public CharBigArrayBigList(final CharCollection c) {
		this(c.size());
		for (CharIterator i = c.iterator(); i.hasNext();)
			add(i.nextChar());
	}
	...
	public void ensureCapacity(final long capacity) {
		if (capacity <= a.length || a == CharBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
			return;
	...
	private void grow(long capacity) {
		...
		if (capacity <= oldLength)
			return;
		if (a != CharBigArrays.DEFAULT_EMPTY_BIG_ARRAY)
	...
	public void size(final long size) {
		...
		if (size > this.size)
			BigArrays.fill(a, this.size, size, ((char) 0));
		this.size = size;
	}
