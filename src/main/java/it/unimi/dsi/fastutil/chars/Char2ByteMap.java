...
public interface Char2ByteMap extends Char2ByteFunction, Map<Character, Byte> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Char2ByteMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Char2ByteMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsent(final char key, final java.util.function.IntUnaryOperator mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfAbsentNullable(final char key,
	 ...
	 * @since 8.0.0
	 */
	default byte computeIfPresent(final char key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Byte newValue = remappingFunction.apply(Character.valueOf(key), Byte.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default byte compute(final char key,
			final java.util.function.BiFunction<? super Character, ? super Byte, ? extends Byte> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final byte oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Byte newValue = remappingFunction.apply(Character.valueOf(key),
				contained ? Byte.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default byte merge(final char key, final byte value,
	...
	@Deprecated
	@Override
	default Byte computeIfAbsent(final Character key,
	...
	@Deprecated
	@Override
	default Byte computeIfPresent(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Byte, ? extends Byte> remappingFunction) {
	...
	@Deprecated
	@Override
	default Byte compute(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Byte, ? extends Byte> remappingFunction) {
	...
	@Deprecated
	@Override
	default Byte merge(final Character key, final Byte value,
			final java.util.function.BiFunction<? super Byte, ? super Byte, ? extends Byte> remappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Character, Byte> {
		/**
