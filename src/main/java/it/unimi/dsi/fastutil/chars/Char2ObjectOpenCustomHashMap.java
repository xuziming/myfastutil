 ...
 * @see HashCommon
 */
public class Char2ObjectOpenCustomHashMap<V> extends AbstractChar2ObjectMap<V>
	...
	public Char2ObjectOpenCustomHashMap(final int expected, final float f,
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ObjectOpenCustomHashMap(final int expected,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ObjectOpenCustomHashMap(final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ObjectOpenCustomHashMap(final Map<? extends Character, ? extends V> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ObjectOpenCustomHashMap(final Map<? extends Character, ? extends V> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ObjectOpenCustomHashMap(final Char2ObjectMap<V> m, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m.size(), f, strategy);
		putAll(m);
	}
	 ...
	 * @param strategy the strategy.
	 */
	public Char2ObjectOpenCustomHashMap(final Char2ObjectMap<V> m,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
		this(m, DEFAULT_LOAD_FACTOR, strategy);
	}
	 ...
	 *                                  lengths.
	 */
	public Char2ObjectOpenCustomHashMap(final char[] k, final V[] v, final float f,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	 ...
	 *                                  lengths.
	 */
	public Char2ObjectOpenCustomHashMap(final char[] k, final V[] v,
			final it.unimi.dsi.fastutil.chars.CharHash.Strategy strategy) {
	...
	public boolean containsValue(final Object v) {
		...
		if (containsNullKey && java.util.Objects.equals(value[n], v))
			return true;
		for (int i = n; i-- != 0;)
			if (!((key[i]) == ((char) 0)) && java.util.Objects.equals(value[i], v))
				return true;
		return false;
	}
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfAbsent(final char k, final java.util.function.IntFunction<? extends V> mappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public V computeIfPresent(final char k,
			final java.util.function.BiFunction<? super Character, ? super V, ? extends V> remappingFunction) {
	...
	/** {@inheritDoc} */
	@Override
	public V merge(final char k, final V v,
	...
	private class MapIterator {
		...
		public int nextEntry() {
			...
			final char key[] = Char2ObjectOpenCustomHashMap.this.key;
			for (;;) {
				if (--pos < 0) {
		...
		public void remove() {
			...
			if (last == n) {
				...
				Char2ObjectOpenCustomHashMap.this.remove(wrapped.getChar(-pos - 1));
	...
	private final class MapEntrySet extends AbstractObjectSet<Char2ObjectMap.Entry<V>> implements FastEntrySet<V> {
		...
		public boolean contains(final Object o) {
			...
			if ((strategy.equals((k), ((char) 0))))
				return Char2ObjectOpenCustomHashMap.this.containsNullKey && java.util.Objects.equals(value[n], v);
			char curr;
			final char[] key = Char2ObjectOpenCustomHashMap.this.key;
			int pos;
			// The starting point.
		...
		public void forEach(final Consumer<? super Char2ObjectMap.Entry<V>> consumer) {
			if (containsNullKey)
				consumer.accept(new AbstractChar2ObjectMap.BasicEntry<V>(key[n], value[n]));
			for (int pos = n; pos-- != 0;)
				if (!((key[pos]) == ((char) 0)))
					consumer.accept(new AbstractChar2ObjectMap.BasicEntry<V>(key[pos], value[pos]));
		}
