...
public interface Char2DoubleMap extends Char2DoubleFunction, Map<Character, Double> {
	 ...
	 */
	interface FastEntrySet extends ObjectSet<Char2DoubleMap.Entry> {
		/**
		 ...
		 * @since 8.1.0
		 */
		default void fastForEach(final Consumer<? super Char2DoubleMap.Entry> consumer) {
			forEach(consumer);
		}
	}
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsent(final char key, final java.util.function.IntToDoubleFunction mappingFunction) {
	 ...
	 * @since 8.0.0
	 */
	default double computeIfAbsentNullable(final char key,
	 ...
	 * @since 8.0.0
	 */
	default double computeIfPresent(final char key,
		...
		if (oldValue == drv && !containsKey(key))
			return drv;
		final Double newValue = remappingFunction.apply(Character.valueOf(key), Double.valueOf(oldValue));
		if (newValue == null) {
			remove(key);
			return drv;
		}
	 ...
	 * @since 8.0.0
	 */
	default double compute(final char key,
			final java.util.function.BiFunction<? super Character, ? super Double, ? extends Double> remappingFunction) {
		java.util.Objects.requireNonNull(remappingFunction);
		final double oldValue = get(key), drv = defaultReturnValue();
		final boolean contained = oldValue != drv || containsKey(key);
		final Double newValue = remappingFunction.apply(Character.valueOf(key),
				contained ? Double.valueOf(oldValue) : null);
		if (newValue == null) {
			if (contained)
				remove(key);
			return drv;
	 ...
	 * @since 8.0.0
	 */
	default double merge(final char key, final double value,
	...
	@Deprecated
	@Override
	default Double computeIfAbsent(final Character key,
	...
	@Deprecated
	@Override
	default Double computeIfPresent(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Double, ? extends Double> remappingFunction) {
	...
	@Deprecated
	@Override
	default Double compute(final Character key,
			final java.util.function.BiFunction<? super Character, ? super Double, ? extends Double> remappingFunction) {
	...
	@Deprecated
	@Override
	default Double merge(final Character key, final Double value,
			final java.util.function.BiFunction<? super Double, ? super Double, ? extends Double> remappingFunction) {
	 ...
	 */
	interface Entry extends Map.Entry<Character, Double> {
		/**
